...
The reference precondition limits are four clauses, four predicates per clause and four total predicates per Step. The composed `RuntimeStepControlWeight` selectors price the hint `total + Opening` at `n = min(hint, 4)`, with zero cost at zero. They use the component-wise maximum of generated asset-heavy mixed and pure-asset models at `n`, plus the observation-heavy model at `n - 1` only when `n >= 2`. The control identity binds all three inputs. This parameter is at least the number of Step predicate visits; Opening visits use saved results, while Current visits evaluate host state. Opening capture is priced separately. Reference host/state evidence and selector tests cover these staged components; direct aggregate profiles, complete admission and full production regeneration remain unqualified.

Composed Opening control includes separate generated traversals of the full bounded Contract when there are zero Opening predicate results or zero amount-snapshot entries. Predicate capture still visits Steps, clauses and skipped Current predicates; amount capture still classifies each Step's authored amounts. Each zero-result traversal is reserved only for head contexts and charged only during Opening, replacing its corresponding nonzero capture charge. Inclusive direct Opening owners receive no additive traversal.

Nonzero amount capture uses the component-wise maximum of generated Preservable, Target and one-share mixed models. Target input is capped at the 12-Step domain and share-mixed input at 23 entries; the Preservable model covers all 24 entries. These are alternative complete capture models, never additive traversal charges. The control identity binds all three input models, their selected result and the zero-result alternatives. The scoped binding consumes typed, Native, unfunded and mixed capture evidence; complete phase/admission reconciliation and final production regeneration remain open.

Nonzero Opening predicate capture selects the component-wise maximum of full-Contract mixed and pure maximum-account models at the captured count, plus one-account/Oracle-heavy capture at `count - 1` only for counts of at least two. The first two models cover 0..48; the Oracle parameter covers 1..47. Zero selects only the independent traversal. V10 binds every applicable input and selected result; maximum and actual use the same selector. Generated fits, recorded proof and sampled timings remain distinct evidence classes; these staged components do not certify complete production admission.
...
| `MaxCrossingActorsPerBlock` | Configured per-pass candidate ceiling; executed prepass materialization is instead bounded by the shared materialization family rotation inside the Actor Control envelope. The previously released cohort executed `1`, `3` and `8` candidates across the three-block cycle (mean four); the retained placed pair-owner fallback executes `2`, `6` and `8` for the steady cycle after the opening leaf, moving the cycle `12 → 16` at unchanged cycle ProofSize with a measured runtime-bound RefTime premium per additional candidate, and the executed ceiling of eight is grant-bound rather than counter-bound (`actors/EXP-0103`, `actors/EXP-0104`, `actors/EXP-0110`) |
...
Wakeups, Crossing, and broad fanout contribute `14%`, `10%`, and `20%` of maximum block Weight to one `44%` shared materialization envelope, but the Actor Control one-third ProofSize share (`676,150`) is the binding root envelope for materialization. On the pinned generated binding the three family minimum quanta total `667,364` of the `668,608` available materialization ProofSize, so the reservation set stays effective and first service order decides each family's grant. A persisted cursor rotates the first family, reserves every later minimum, lends the remainder to the first family, and may return trailing unused reservations through one counter-preserving second grant. The runtime-bound `375,842,000 / 5,982` coordinator owner includes ten reads and one write for cursor state and bounded remaining-work classification. Executed Crossing prepass materialization therefore follows the rotation: the previously released cohort executed one, three and eight candidates across the three-block cycle (mean four), while the retained placed pair-owner fallback executes `2`, `6` and `8` for the steady cycle after the opening leaf — `12 → 16` at unchanged cycle ProofSize with the measured runtime-bound RefTime premium per additional candidate — under the `MaxCrossingTransitionsPerBlock` ceiling (`actors/EXP-0103`, `actors/EXP-0110`). Inside that rotation the executed yields are reservation-fit events: one Crossing admission reserves `180,960` (worker base, common probe, selection owner, single placed owner, and fault reserve) and settles `179,431`; the rotated-last grant settles the `608,478` aggregate owner plus two `15,106` classification probes and leaves `10,503`, below the next probe, so the transition counter stops nothing the grant would otherwise allow; the rotated-first grant (`197,310`) stays below the aggregate reservation and reached its three candidates of the previously released cohort through single owners across the first service and the bounded lending re-service (`actors/EXP-0104`). Fixed scheduler base, coordinator, mandatory cleanup, and the shared envelope are subtracted once; the residual is the exact Actor execution floor.

Funding authorization remains Contract policy, while every dynamic amount reads current sovereign Available balance at its execution attempt; no Actor-only funding accumulator, opening snapshot, or machine-fee ledger exists. Fresh-genesis Actors storage includes fragment-local resources, compact activation authority, split run state, bounded control topology, one transient block-resource state, and one latest finalized non-authoritative telemetry snapshot; the pre-`1.0` reference line defines no deployed-state migration.
...
`template/runtime/src/weights/pallet_deos_actors.rs` owns complete generated methods and storage annotations. The accepted production binding is generated Actors Weight `46a89aa48b6814b0d02b3b423b99a134cc6b84f08cbb1c3dfaf4bf4074c18346`, production Wasm `91c23b2f77b57265e8e32ec8645610a32b7d4763b0ced31ab56ef64e8c0f5a2d`, and metadata `75868ea75fd85e7b79db12650a7407e89e249f5564b2157d55ac46cd26cb5d12`. `scripts/actors-assurance.sh` reports and preserves those identities while running exact named heavy profiles. Architecture records only load-bearing admission values; benchmark-host timing never becomes a chain-throughput claim.
...
The current production policy has a `44%` shared materialization envelope, exact generated minima for all three families, a steady Crossing cycle of 16 candidates across three blocks (`2`, `6` and `8`) against the previously released cycle of `12` (`1`, `3` and `8`) at unchanged cycle ProofSize with a measured runtime-bound RefTime premium per additional candidate and an executed ceiling of eight per block (`actors/EXP-0110`), 9,000 User memberships per feed, 10,000 total memberships per feed, a 10,000-entry FIFO, 512 wakeup scans, and 64 broad-fanout pages. Crossing branch billing selects one mutually exclusive generated execution owner after the common, branch and admission-selection owners; the component rows below are not summed as independent whole-branch maxima.

| Crossing path | Generated owner RefTime / ProofSize | Counter cap per block | 10,000-candidate counter-cap bound |
| --- | ---: | ---: | ---: |
| No-match transition | `32,895,000 / 6,060` transition; staged probes remain separate generated owners | 0 | Not a candidate drain |
| Post-installation skip pair | `52,522,000 / 6,156` probe; `74,033,000 / 6,156` execution | 8 | 1,250 blocks |
| Rearm pair | `86,815,000 / 10,482` probe; `476,185,000 / 162,782` execution | 8 | 1,250 blocks |
| Coalesced fire pair | `334,335,000 / 10,482` fire-pair probe; `657,217,000 / 162,782` execution | 8 | 1,250 blocks |
| Placed fire pair | `334,335,000 / 10,482` fire-pair probe; `652,398,000 / 162,782` execution | 8 | 1,250 blocks |
| Maximum placed batch | `998,886,000 / 81,886` execution after staged probes | 8 | 1,250 blocks |
| Non-tail placed batch | Component-wise maximum of `795,994,000 / 81,886` emptied-tail and `801,232,000 / 81,886` trimmed-tail execution | 8 | 1,250 blocks |
| Terminal fire | Generated fire probe; `693,745,000 / 162,782` cleanup execution | 8 | 1,250 blocks |
| One same-threshold page | Funded scalar owner `560,485,000 / 162,782`; homogeneous batches select their dedicated owner | 8 | 1,250 blocks |
| Maximum homogeneous herd | Maximum placed-batch path under the shared envelope | 8 | 1,250 blocks |

The ledger separates zero-match transition setup, sparse occupied-leaf search, source-page traversal, rearm, coalesced fire, placed fire, and terminal cleanup. The current search owner still couples a newly found sparse threshold to its first candidate. EXP-0095 binds these selected values to the production Weight source and Wasm; larger candidate-count cohorts or sparse-search amortization would require a later geometry campaign and complete evidence refresh.

On the declared 48-member funded User Crossing-to-Transfer witness, the retained pair-owner fallback was compared against the sealed previously released artifact end to end: the last Cycle completes at block `14` against `16` (completion horizon `14 → 12`), Trigger-to-completion mean `9.46 → 8.67` with p95 and maximum `14 → 12`, and Trigger-to-materialization mean `6.38 → 4.94` with p95 `11 → 9`. Both cohorts materialize all 48 members exactly once, commit 48 Steps from 48 distinct committing Actors (`S = A = 48`) with 48 Transfer effects, and complete 48 Cycles with zero censoring and zero faults; over the fixed window charged Control work falls `13.0%` in RefTime and `10.1%` in ProofSize while effect totals are identical, and the maximum queue span deepens `22 → 29` as the stated mechanism and cost of earlier placement (`actors/EXP-0115`, `actors/EXP-0116`). The measured improvement is qualified to this declared funded User witness on the retained binding; it is not a herd-scale, steady-state, wall-clock, fairness, or universal Actor-throughput claim.
...
The inert `EVENT_COMPLETE_TRANSITION_OWNERS` cutover inventory separately closes every Oracle writer that can invalidate current dependency state: feed registration, pause/resume lifecycle mutation, deactivation, and changed or equal-value publication. Every row binds the collision-free typed key `EventCompleteDependencySource::OracleFeed(feed)`, its present transaction boundary, and the exact post-write/pre-event publication point. Source-backed runtime evidence fails when a new `Feeds` or `Observations` writer is unclassified. Age expiry remains timed; production callers do not yet invoke the inert dependency publication boundary. Lifecycle and observation owners not already enclosed transactionally are explicitly marked as requiring that enclosure at cutover.
...
Deactivating a feed used by a latched Crossing can prevent Opening at the live FIFO head and stop later Actors despite spare Weight. Opening retains paid readiness and commits no Pipeline fee or cycle. Authorized Mutable owner close, independently due terminal semantics, or an explicit runtime upgrade can resolve the obstruction within their existing authority; observation loss grants no new Immutable close permission. See Actors specification §5.2/§8.5 and `scheduler_paged_zero_step_user_crossing_unavailable` for the measured refusal and Mutable-close recovery path.
