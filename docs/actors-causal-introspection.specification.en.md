...
The result contains one activation input, Contract-level funding/completion/window constraints, and ordered Step projections. Each Step carries exact index, bounded DNF clause/Predicate positions, current-read amount surfaces, typed host dependencies, error policy, and at most one structural atomic effect bundle. A zero-Step Contract produces an empty Step list and retains its control-only completion explanation.
...
All predicates and dynamic amounts use current authoritative reads. DNF is preserved as clauses and atoms; the extractor cannot flatten it into an apparent conjunction or skip evaluated atoms based on optimistic short-circuiting. `Fixed`, current-Available `Percent`, and input limits retain their canonical semantics; `Percent(100%)` means all currently Available balance. Funding coverage/authorization is a dependency, not proof of available funds.
