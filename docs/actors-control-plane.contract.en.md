# DEOS Actors Control-Plane Contract

> Off-chain contract for representing, reviewing, forecasting, composing, and indexing bounded Actor Contracts without expanding consensus state.
...
## Canonical Actor Contract Artifact

A canonical Actor Contract artifact is chain-bound and has these required fields:
...
Projection acceptance requires `decode(contractScale) -> projection -> encode == contractScale`. Unknown variants, missing fields, overflow, noncanonical numbers, or metadata lookup failure reject the artifact instead of producing a partial editable Actor Contract.

Trigger projection preserves the scalar runtime union directly: `Manual`, `AddressEvent`, `ObservationChange { feed }`, or `Cadenced { every_ticks }`. No admission/source cross-product, OR-set, secondary source, or compatibility alias exists. Observation projection preserves the complete typed feed identity and introduces no threshold, callback, amount, or revision promise. Typed authoring rejects `PercentageOfTrigger` under every trigger. AddressEvent plans additionally require the scalar asset filter to cover every authored snapshot surface, with `Any` required when an Unstake receipt asset cannot be derived locally. Authoring normalizes whitelist members by canonical SCALE bytes and rejects semantic duplicates and empty or oversized lists.
...
The reactive delivery projection MUST fail closed unless DEOS Oracle revision, Actors dirty state, active-list topology, subscriber-page topology, and finalized block come from one snapshot. It reports Clean, PendingFanout, FanoutInProgress, or AwaitingCleanup; exact `dirty_since` age; latest/fanout revisions; active-list head/tail/count/cursor and zero-based selected/cursor positions; next subscriber page; occupied and current-revision page counts; and remaining fanout service units. One service unit represents one occupied-page attempt or a pending restart/cleanup transition when no page cursor exists; completing a final page may clear or restart the feed within that page unit.
...
`DeosRouterPreExecutionReserves` identifies local pool reserves observed before Router execution. The inspection surface MUST state that this is not an external fair-price, manipulation-resistance, MEV-protection, or ordering proof. Authored slippage and System reference-deviation guards remain separate from an execution-time Precondition.

## Linear Authoring

The canonical authoring model is a typed ordered `Step[]`, not a graph or a generic DSL. Each Step exposes an absent unconditional `precondition` or one present bounded DNF `Precondition`, one Task with typed parameters and amount resolutions when present, and one error policy. `RetryLater` exposes a required `maxAttempts` integer from metadata-derived `2..=MaxRetryAttempts`; the opening unsuccessful attempt counts as `1`, so at least one retry remains possible. Fieldless `StopCycle` exposes no synthetic parameter. Off-chain Step keys may stabilize editing, but lowering removes them and preserves only exact runtime order.
...
The canonical reactive-authoring fixture uses `ObservationChange → Precondition((ObservationBelow AND BalanceAbove)) → SwapIn → RetryLater`. The same explicit plan supports `Persistent` and `CloseAfterProductiveCycle`; every feed, threshold, asset, amount, slippage bound, freshness window, and retry limit remains authored policy rather than a runtime bucket default.
...
A structural diff compares decoded typed trees only when both artifacts share `genesisHash` and `metadataHash`. It reports additions, removals, moves, and field changes by stable structural path. Array position is semantic for Steps. Canonical Precondition clause and Predicate ordering comes from runtime admission; tooling must not reorder Steps for presentation.
...
## Static Actor Contract Analysis
...
Trigger analysis reports the exact scalar trigger kind, `every_ticks` for cadence, filters for AddressEvent, and the exact ObservationChange feed projection. `ExternallySignalledAdmission` and `PeriodicAdmission` remain factual structural findings; they never claim scheduler position, signal presence, queue admission, or runtime execution. Analyzer version `10` removes source-count and admission-mode projections, retains `TriggerAmountCompatibilityViolation`, and projects exact `Persistent | CloseAfterProductiveCycle` policy from metadata. Typed authoring rejects uncovered trigger-amount filters and unknown completion variants before encoding.

Every ordered Step reports its exact Precondition topology and cost bound, execute/skip admission, lossless Task and amount semantics, failure policy, retry limit, adapters, economic surfaces, Temporary-failure reachability, and Continuation eligibility. Success is `advance` or `complete-cycle`; failure is `advance`, `terminate`, or Mutable-only `stutter-current`. Analysis never merges success and failure or invents a successor.

`SplitTransferDepositPreflight` requires every non-zero allocation to be depositable; one rejection fails the task atomically as Temporary. Optional finalized-block `ActorMinimumBalanceEvidence` emits `SplitTransferLegBelowKnownMinimum` only for a fixed leg whose observed empty recipient would receive `0 < floor(total × share) < minimum`. Missing evidence produces no viability claim.

System swap rows emit `SystemReferenceDeviationGuard`. Its fresh EMA or reserve fallback is a local execution guard, may reflect manipulated pool state, and proves neither fair price nor transaction-order protection. User swap rows omit this finding.

DNF analysis states that several Predicates may be true while the Task executes at most once. It remains one linear Step rather than multiple edges. Forward dependencies connect earlier Task writes to Predicate reads without claiming a branch, and costs use the total configured Predicate count regardless of ordering or truth position.

`StopCycle` analysis reports no adapter, amount, asset, recipient, economic effect, Temporary failure, or Continuation eligibility. Completion-policy authoring distinguishes a persistent strategy from one-shot productive closure and states that only a committed effectful task qualifies; false latest-state Precondition results, skipped Steps, failed rollback, suspension, abort, retry exhaustion, and bare `StopCycle` do not consume the one-shot lifecycle. Its successful control is cycle completion at the current index; a false Precondition remains ordinary advance, while Predicate-evaluation or fee-collection failure follows its owning runtime boundary. `StopCycleFailureMayFallThrough` identifies `ContinueNextStep`, the exact row, and whether its suffix contains economic effects. Authoring shows the same warning. Every suffix envelope remains a structural maximum unless separately proven path-sensitive evidence applies.

Authoring presentation distinguishes a false Precondition, resolution skip, funding unavailability, Temporary Task failure, and Permanent Task failure. The stored `AbortCycle` variant is shown as abort-on-task-failure behavior: ordinary Precondition and resolution skips advance, funding unavailability advances for Abort/Continue but suspends under Mutable Retry, and only task or pre-execution errors follow the failure-policy termination path.

Forward data dependencies report an earlier asset write read by a later Predicate, Task, or amount surface. They describe behavioral predication and live/frozen data flow, not workflow branches. Without supplied runtime observations, analysis MUST NOT claim current balances, adapter health, active Continuation state, execution outcome, or scheduler position.

For every cursor from zero through `ContractSteps` length, the analyzer emits one suffix envelope with remaining-Step count, maximum RefTime and ProofSize, evaluation and execution fee upper bounds, lifecycle and funding-snapshot-opening overhead, adapters, asset surfaces, committed effect classes, and retryable indexes. Suffix zero is the full-contract envelope; remaining-Step count decreases exactly once per cursor. Every result stays bound to the artifact and model identities used to derive it.
...
`ActorFeedbackModel` consumes identified `ActorContractStaticAnalysis` results rather than decoding Actor Contracts or recreating Task semantics. It builds a deterministic bounded graph over actors, typed observation feeds, shared assets, exact actor-account signal recipients, and declared future parameter actuators. Observation-to-actor edges come only from exact trigger or Predicate feed identity; actor effects come from existing analyzed Steps plus explicit typed effect matchers.
...
Canonical-chain truth includes current bounded Actor Contract/state, dispatch outcomes, events, and runtime metadata at an identified block. Actor Contract artifacts, diffs, forecasts, simulations, annotations, and long version/cycle/funding histories are local or materialized truth.
...
| 2 | DEOS Fee Sink phase-one allocation | `Cadenced { every_ticks: 120 }` → native `SplitTransfer` to staking ingress and liquidity ingress | Expressible | One atomic bounded fan-out owns the 50/50 runtime policy. The 500 ms reference tick yields a stable 60-second period; fee collection remains ledger-only. |
...
| 4 | DEOS Liquidity Actor provisioning | One AddressEvent Actor or one Cadenced Actor → optional swap → `AddLiquidity` or `DonateLiquidity` | Partial | Typed tasks and retry semantics exist, but one Actor cannot combine independent triggers. A strategy needing both uses separate Actors until concrete evidence justifies composition. |
| 5 | Periodic DCA | `Cadenced { every_ticks }` → `SwapIn(Fixed or PercentageOfCurrent)` | Expressible | One-step DCA needs no Continuation. Each tick pays one task envelope and any User evaluation/execution fees; market outcome remains adapter-dependent. |
...
| 7 | Resilient swap-to-liquidity pipeline | `SwapIn → AddLiquidity → Transfer`, with bounded `RetryLater { maxAttempts }` only where Temporary failure is meaningful | Expressible | Successful prefixes remain committed. Suspension stores one scalar cursor and required frozen suffix snapshots; retry re-evaluates Current Predicates without replaying the prefix. |
| 8 | Stake then maturity-gated unstake | Stake plan plus a later timer/block-gated Unstake plan | Partial | Basic stake and unstake tasks exist, but reward, maturity, health, or exchange-rate Predicates are absent. Separate actors add custody, scheduling, latency, and operational cost. |
| 9 | Oracle-price take-profit or stop-loss | Fresh typed price observation → swap or successful stop | Expressible | Observation Predicates compare one explicitly authored feed identity and raw scalar threshold. Unavailable, uninitialized, or stale truth skips the task; local-pool provenance does not imply external fair price. |
...
| 11 | Descending buy buckets | Independent one-shot actors: price change → `All(ObservationBelow(level), BalanceAbove(spend))` → `SwapIn` → bounded retry | Expressible | DEOS Router produces local pre-execution samples, DEOS Oracle owns current truth, and Actors owns reaction. Non-fresh truth skips; retry exhaustion closes only that bucket. |
...
| 15 | Non-price block-height release | Cadence → `BlockNumberAbove` → one-shot `Transfer` | Expressible | FRAME System/runtime block number owns canonical current truth. Before the threshold the Precondition is false and the Step skips; transfer failure follows the authored policy. |
...
- Runtime-provider rejection for artifact/Actor Contract mismatch, wrong mode/run state, unavailable fee budget, stale code/state identity, and any write escaping the outer rollback.
