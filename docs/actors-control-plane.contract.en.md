...
| 5 | Periodic DCA | `Cadenced { every_ticks }` → `SwapIn(Fixed or Percent)` | Expressible | One-step DCA needs no Continuation. Each tick pays one task envelope and any User evaluation/execution fees; market outcome remains adapter-dependent. |
