...
The replacement contract now has code-level primitives without yet changing active storage: `WakeupPointer { block, page_id, slot }`, optional-slot `WakeupPage` values with live count, scan cursor, and bidirectional page links, plus `WakeupBucketState` head/tail/allocation metadata. `WakeupPageSize = 64` currently provides a provisional compile-time bound only; production-Wasm page-size evidence must select the final value before these types replace `WakeupIndex` and `ScheduledWakeupBlock`.
