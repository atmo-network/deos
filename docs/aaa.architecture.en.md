...
`aaa_id` exists only as each storage-map key. Dormant identity carries no timestamp. Activation or a pre-first-cycle schedule update derives first eligibility from current block, window start, cadence, and actor-stable jitter. The typed lifecycle forbids contradictory pause state. Scheduler admission, cycle execution, control extrinsics, lifecycle transitions, liveness, wakeup, ingress, and try-state paths compose explicit `ActorHot + ActorProgram` snapshots through pallet-private helpers and never depend on the temporary `AaaInstances` compatibility facade. That facade now exposes read composition only for legacy benchmarks/tests; it is not consensus storage, has no metadata prefix, and owns no mutation, construction, removal, or invariant logic.
