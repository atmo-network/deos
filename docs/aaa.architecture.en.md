...
Dormant identities retain their deterministic sovereign address, owner, Mutable System class, and genesis provider but have no `AaaInstances`, readiness, inbox, queue, wakeup, funding, fee, or cycle state. Custody-only accounts receive only a runtime-declared genesis provider so assets can enter safely; they have no generic AAA identity or control surface.
...
- Identity and ownership (`ActorClass::User { owner_slot } | System`, sovereign account, owner, mutability), making a System owner slot unrepresentable; `aaa_id` exists only as the storage-map key and is not duplicated inside active or dormant values
- Lifecycle control (`ActiveLifecycle::Active | Paused(PauseReason)`, optional auto-close target, schedule, schedule window); the typed enum forbids contradictory pause boolean/reason combinations
- Execution state (run plan, close plan, cycle nonce, canonical `first_eligible_at`, last admitted cycle block, failure counter, manual-trigger flag); dormant identity carries no timestamp, and activation or a pre-first-cycle schedule update derives first eligibility from current block, window start, cadence, and actor-stable jitter
- Funding lives separately in `ActorFunding`; `AaaInstances` no longer embeds policy, tracked assets, or per-asset batches
- Admission caches (`cycle_weight_upper`, `cycle_fee_upper`, plus close-tail derivation inputs)
- Creation block retained only as the first delayed-timer anchor; the former `updated_at` field was removed because no ordering, admission, lifecycle, or read-model contract consumed it
...
Primary storage in pallet implementation follows two ownership classes: Section 13's stable behavioral stores constrain compatibility, while bounded scheduler and ingress machinery remains replaceable implementation state whose semantics come from the owning contracts. The former synchronized `AaaReadiness` mirror has been removed; until ActorHot decomposition lands, `AaaInstances` remains the single readiness source.

- `NextAaaId`: monotonic AAA id allocator
- `AaaInstances`: active/paused actor identity, lifecycle, schedule, plan, and cached admission bounds; funding fields have moved out, while later S4 work still splits hot state from cold program bytes
- `ActorFunding`: active-only funding-source policy, bounded tracked-asset set, `funding_snapshots[asset] = FundingBatch { amount, pending_amount }`, and canonical `has_pending_funding`; ingress, promotion, and policy mutation touch this store without rewriting `AaaInstances`, promotion skips batch traversal when false, and try-state reconciles the indication against every bounded batch
- `DormantAaaIdentities`: identity-only records with no executable or scheduler state
- `ActorIdentityCount`: transactionally maintained O(1) cardinality across `AaaInstances` plus `DormantAaaIdentities`, bounded by `MaxActorIdentities`
- `ActiveAaaCount`: transactionally maintained O(1) active/paused cardinality used by activation and operational-cap checks; try-runtime reconciles it against `AaaInstances`
...
- Runtime `WeightInfo` regenerated with frame omni bencher 0.22 / benchmark CLI 58 against the shipped identity/active counts, dormant lifecycle, queue-cleanup, wakeup-retry, sweep, funding-policy update, fixed hook bases, close-plan-update, User fee collection, saturated ingress-aware asset transfer, and User fee-bearing close-tail topology; dedicated lifecycle classes measure dormant System creation at `45,258,000` RefTime with seven reads/five writes and `12,272` proof bytes, activation at `41,836,000` RefTime with five reads/four writes, 660 measured/12,272 charged proof bytes, and deactivation at `40,997,000` RefTime with five reads/nine writes and `81,487` charged proof bytes; generated storage annotations contain no retired active-list stores
...
- Successful-cycle funding rotation has a generated `funding_batch_promotion(a)` class over `a = 1..10`; cycle admission always reserves the runtime maximum, with `19,679,541 + 2,370,734a` RefTime, one read/write, 4,426 charged proof bytes, and measured proof growth `496 + 37a`; the one-byte fixed increase is the canonical pending indication
- Mutable plan replacement benchmarks a prepopulated `MaxFundingTrackedAssets` batch map and removes every stale entry in the measured call; the regenerated `update_execution_plan` envelope charges two reads/two writes across program and funding ownership with 1,146 measured and 12,272 estimated proof bytes
- Producer-owned transaction-extension ingress declares the generated full matched-notification envelope and refunds against a separate generated unmatched-event base; the base conservatively benchmarks the real negative recipient resolution together with a populated `SovereignIndex` proof witness, charging `14,877,000` RefTime, one database read, and 3,521 proof bytes, and refund tests retain that complete base after an unmatched successful call. The maximum measures `AnySource` authority evaluation, existing-batch checked accumulation, funding observability, inbox mutation, and nine saturated wakeup buckets at 17 reads, 14 writes, 725,743 measured proof bytes, and a charged 743,463-byte proof estimate
- Compatibility ingress uses a generated one-read probe with 1,489 charged proof bytes before ring-length access, then a trigger-only durable-ring drain unit under saturated wakeup retry at 18 reads, 16 writes, 725,639 measured proof bytes, and a 743,463-byte charged proof estimate; enqueue-time funding reservation is paid synchronously by the originating producer, and the retired event-vector scan class no longer appears in production weights
...
- The diagnostic-only `scheduler_cooldown_ineligible_idle` case admits a Timer actor once, verifies its sole cooldown wakeup and empty current/next queues, then measures the production scheduler loop at an ineligible intermediate block: 50 steps/20 repeats record `17.95 µs`, four queue/index reads, four writes, 490 measured proof bytes, and no `AaaInstances` access. This falsifies repeated actor probing for `every_blocks=1/cooldown_blocks=10`: `frame-omni-bencher v1 benchmark pallet --runtime target/release/wbuild/deos-runtime/deos_runtime.compact.compressed.wasm --pallet pallet_aaa --extrinsic scheduler_cooldown_ineligible_idle --steps 50 --repeat 20 --heap-pages 4096 --output /tmp/pallet_aaa_scheduler_cooldown_ineligible_idle.rs`
