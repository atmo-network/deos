...
`automation/analysis.ts` consumes canonical artifact inspection plus the existing forecast aggregator. It emits identity-bound `StaticStructuralProjection` rows, economic/failure/data-dependency surfaces, factual unscored findings, and one bounded suffix envelope for every cursor. Each row preserves exact `Always`/`All`/`Any` mode, atomic count, full-observation semantics, fixed false successor, and whole-group atomic-error behavior. `StopCycle` remains adapter-free and exposes only skip-advance or admitted termination.

`automation/authoring.ts` owns the typed linear draft and immutable add/replace/remove/reorder operations. It validates runtime bounds and class rules, rejects empty `All`/`Any` before encoding, lowers every current condition/task/amount/policy directly to metadata-shaped `ProgramInput`, and delegates exact SCALE and `planId` production to `plan-artifact.ts`. Authoring-only row keys disappear during lowering; no graph, recipe identity, generic call, or successor field enters the artifact.

`AutomationWidget` keeps actor monitoring and composition in separate views. Composition renders stable numbered rows with explicit `Always`, `All conditions`, or `Any condition` mode plus task-parameter, amount, and error-policy controls for every current primitive. Removing the final grouped atom returns the row to `Always`; `StopCycle` renders terminal completion without adding an edge. The widget creates artifacts but has no submission action.
