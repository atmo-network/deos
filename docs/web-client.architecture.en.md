...
- `StatisticsWidget` — protocol/system statistics, native staking read-model facts, plus recent system traces
...
- Use explicit provenance metadata on ambiguous surfaces such as live runtime quotes, recent chart samples, live session event feeds, bounded client asset projections, native staking runtime-view cards, and future materialized/archive providers
...
- `StatisticsWidget` now follows it more explicitly as well: tight panes collapse the snapshot/liquidity stat grids, delegates bounded native staking facts, per-operator/governance custody pending-unlock details, and collapsible signed native nomination reward, collator LP lifecycle, and governance-only NativeVotePower custody action sections to `StakingWidget`, lets the route-mix detail cards stack before those labels/details become cramped, and stacks the lower route/liquidity split instead of clinging to one roomy desktop matrix
