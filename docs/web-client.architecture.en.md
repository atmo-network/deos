...
The step editor requires canonical positive base-unit market bounds. `SwapExactOut.maxAmountIn`, `AddLiquidity.minLpOut`, and both `RemoveLiquidity` asset minima lower to exact runtime `u128` fields. Zero, non-canonical, or overflowing values fail before artifact construction; dynamic amount modes cannot replace these fixed bounds.
