...
struct OutcomeTotals { executed_steps: u32, skipped_conditions: u32, skipped_resolution: u32, skipped_funding_unavailable: u32, failed_steps: u32 }
```
...
All operations MUST go through typed adapters. The external host-runtime embedding checklist and adapter failure atomicity matrix are maintained in [package-owned AAA Runtime Embedding Guide](../template/pallets/aaa/EMBEDDING.md) as the implementation-facing companion to this normative adapter contract.
...
`can_deposit`/`minimum_balance` are REQUIRED for ED-safe split-transfer normalization (Section 6.2). The unit `AssetOps` fallback MUST fail every mutation and report no deposit viability; an unconfigured capability MUST NOT fabricate successful asset effects.
...
8. Funding-batch behavior for inbound ingress is normative (Section 2.6) and MUST remain independent from trigger-filter matching.
