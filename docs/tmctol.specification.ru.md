...
`Защита`: Толерантность к проскальзыванию, зависящая от глубины резервов, предотвращает извлечение стоимости во время инициализации пула или больших дисбалансов. Соответствующая реализации система SHOULD выводить допуск Zap-свопа из глубины нативного резерва, зажимать его в явных нижней/верхней границах и избегать одного плоского потолка для всех случаев, когда более глубокие пулы уже делают более строгие границы материально безопаснее. Точная стратегия реализации относится к парным архитектурным/runtime-документам. Механизм обеспечивает достижение распределениями TOL максимальной глубины ликвидности.
...
`Таксономия порогов`: Нельзя смешивать четыре разные границы и две разные метрики сжатия.

1. `Порог инверсии эластичности`

```
dF/dS = 0
```

В рамке растущего предложения, используемой в тесте симулятора, эффективный стресс-пол можно записать так:
...
Следовательно, инверсия наступает когда:
...
`Интерпретация`: Это только граница, где пол перестаёт ухудшаться при расширении предложения. В самой точке пол плоский, а потолок всё ещё растёт линейно, поэтому инверсия не означает, что пол уже сжимает коридор.

2. `Порог паритета относительного сжатия`

Этот порог отвечает на вопрос, сжимается ли коридор в терминах отношения цен при расширении предложения.
...
Следовательно, относительный паритет требует:

```
d ln k / d ln S = 2 + ε_C
```

При больших `S`, когда вклад `P₀` мал, `ε_C ≈ 1`, поэтому относительный паритет приблизительно равен:

```
d ln k / d ln S ≈ 3
```

`Интерпретация`: Инверсия соответствует росту `k` как `S²`; паритет относительного сжатия требует роста `k`, уже более близкого к `S³`.

3. `Порог сжатия абсолютного зазора`

Если сжатие измеряется как арифметический спред:

```
G_abs(S) = C(S) - F(S)
```

то условия паритета и сжатия таковы:

```
dG_abs/dS = dC/dS - dF/dS
паритет абсолютного зазора: dF/dS = dC/dS
сжатие абсолютного зазора: dF/dS > dC/dS
```

`Интерпретация`: Пол может расти и при этом всё ещё расти медленнее потолка в абсолютных ценовых единицах. В таком режиме пол восстанавливается, но абсолютный зазор всё ещё расширяется.

4. `Порог арбитражного разворота / overtaking`
...
`Интерпретация`: Это сильнее инверсии и сильнее обеих метрик сжатия. Здесь пол уже догнал или превысил потолок настолько, что меняется доминирующая структура стимулов.

`Уточнение метрики и оси анализа`:

- Раздел `3.4 Двунаправленное сжатие` — это утверждение о burn-time траектории: при устойчивом сжигании `dC/dt < 0`, а `dF/dt > 0`, поэтому коридор сжимается напрямую
- Настоящая таксономия порогов использует другую рамку: что происходит при росте `S`
- Таблица прогрессии выше выражает `Spread` как мультипликатор (`13,6×`, `3,4×`, `0,54×`), поэтому её естественное чтение — это относительное сжатие, если явно не назван абсолютный зазор
- Поэтому пост-инверсионный рост пола сам по себе ещё не означает сжатие коридора; это зависит от выбранной метрики — мультипликативной (`C/F`) или абсолютной (`C-F`)

`Карта режимов`:

- `d ln k / d ln S < 2`: пол падает
- `d ln k / d ln S = 2`: порог инверсии; пол плоский
- `2 < d ln k / d ln S < 2 + ε_C`: пол снова растёт, но относительное сжатие ещё не началось
- `d ln k / d ln S = 2 + ε_C`: паритет относительного сжатия; `C/F` плоский
- `d ln k / d ln S > 2 + ε_C`: относительное сжатие; `C/F` сужается
- `dF/dS = dC/dS`: паритет абсолютного зазора; `C - F` плоский
- `dF/dS > dC/dS`: сжатие абсолютного зазора; `C - F` сужается
- `F(S) ≥ C(S)`: арбитражный разворот / overtaking

`Уточнение области теста`: Тест симулятора `Supply Elasticity Inversion Point` проверяет только первый порог. Он показывает, что ухудшение пола может остановиться и затем развернуться после критической границы; сам по себе он не доказывает относительное сжатие, не доказывает сжатие абсолютного зазора и не доказывает немедленное пересечение пола и потолка.
...
- Параметры запуска кривой задаются в `create_curve`
- Контракт запуска TMCTOL по умолчанию считает эти параметры неизменяемыми после запуска
- Форки MAY намеренно расширить эту поверхность, но тогда они берут измененный экономический риск на себя, а не наследуют стандартную поверхность гарантий TMCTOL
