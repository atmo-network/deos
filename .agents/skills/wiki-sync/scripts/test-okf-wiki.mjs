...
test("current bundle and migration preservation baseline pass", () => {
  ...
  assert.match(
    output,
    /92 concepts, 46 page IDs, 46 graph nodes, 206 typed edges/,
  );
});
...
test("migration baseline fails closed when one prior typed edge is lost", () => {
  ...
  assert.match(
    validate(wiki, 1, true),
    /migration baseline: missing prior typed edge/,
  );
});
...
test("graph rejects duplicate, invalid, and dangling typed edges", async (t) => {
  await t.test("duplicate identity", () => {
    const { wiki } = sandbox();
    mutateJson(join(wiki, "_meta/graph.json"), (graph) =>
...
test("conformant unknown type and block-scalar extension are tolerated", () => {
  const { wiki } = sandbox();
  mutatePage(wiki, "concepts/generated-wiki.en.md", (text) =>
    text
...
test("invalid unquoted colon scalar and duplicate YAML key are rejected", async (t) => {
  await t.test("colon scalar", () => {
    const { wiki } = sandbox();
    mutatePage(wiki, "concepts/generated-wiki.en.md", (text) =>
      text.replace(/^description:.*$/m, "description: Topic: details"),
    );
...
test("frontend manifest path drift fails", () => {
  const { wiki } = sandbox();
  mutateJson(join(wiki, "_meta/locales.json"), (locales) => {
    locales.pages["generated-wiki"].en = "wrong.en.md";
  });
