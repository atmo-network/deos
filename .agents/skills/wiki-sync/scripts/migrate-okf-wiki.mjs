...
for (let index = 2; index < process.argv.length; index += 1) {
  ...
  if (argument === '--write') write = true;
    ...
    usage();
    process.exit(0);
  } else if (argument === '--wiki-dir') wikiDir = resolve(process.argv[++index] ?? '');
...
function normalizeMetadata(meta, path) {
  if ('page_type' in meta && 'type' in meta) throw new Error(`${path}: both page_type and type are present`);
  if ('summary' in meta && 'description' in meta) throw new Error(`${path}: both summary and description are present`);
  let changed = false;
  const normalized = {};
  for (const [key, value] of Object.entries(meta)) {
    const canonicalKey = key === 'page_type' ? 'type' : key === 'summary' ? 'description' : key;
...
console.log(`${write ? 'Migrated' : 'Canonical OKF projection confirmed for'} ${paths.length} localized concepts${write ? ` (${changed} changed)` : ''}.`);
