...
Audit canonical OKF wiki metadata, source provenance, locale mirrors, confidence,
and navigation/graph reachability. Short, low-confidence, stale-source, and graph
leaf pages remain non-blocking consolidation candidates.

Options:
  --wiki-dir <path>              Override the wiki directory (default: ./wiki)
  --wiki-dir=<path>              Override the wiki directory (default: ./wiki)
  --min-body-lines <n>           Short-page threshold (default: 18)
  --low-confidence-threshold <n> Low-confidence threshold (default: 0.85)
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --low-confidence-threshold)
                [[ $# -ge 2 ]] || { log_error "Missing value for --low-confidence-threshold"; exit 1; }
                LOW_CONFIDENCE_THRESHOLD="$2"; shift ;;
            -h|--help) usage; exit 0 ;;
...
main() {
    ...
    node "$SCRIPT_DIR/audit-wiki-consolidation.mjs" \
        "$WIKI_DIR" "$MIN_BODY_LINES" "$LOW_CONFIDENCE_THRESHOLD" "$PROJECT_ROOT"
