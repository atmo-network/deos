...
const sourceWiki = resolve(
  dirname(defaultReferencePath),
  "../../../..",
  "wiki",
);
const sandboxes = [];
...
function response(body, { json = false, status = 200 } = {}) {
  const bytes = Buffer.isBuffer(body)
    ? body
    : Buffer.from(json ? JSON.stringify(body) : body);
  return {
    ...
    if (url.includes("/commits?"))
      return response(
        [
          {
            sha: commit,
            commit: { committer: { date: "2026-08-16T00:00:00Z" } },
          },
        ],
        { json: true },
      );
...
async function latest(candidate, options) {
  return fetchLatestUpstream({
    fetchImpl: fakeFetchFor(candidate, options),
    token: options?.token ?? "",
    lock: pinned.lock,
  });
}
...
function changedSameVersion() {
  return Buffer.from(
    source.toString("utf8").replace("OKF is an open,", "OKF is a portable,"),
  );
}
...
function changedVersion(version) {
  return Buffer.from(
    source
      .toString("utf8")
      .replace("**Version 0.2**", `**Version ${version}**`),
  );
}
...
test("one Markdown atom binds source, body, upstream authority, and canonical adoption", () => {
  ...
  assert.equal(
    parseVersion(pinned.source).text,
    pinned.lock.adoption.okf_version,
  );
...
test("lifecycle classification exposes current and review-pending states", async (t) => {
  await t.test("current", async () => {
    ...
    assert.deepEqual(classify(pinned, value), {
      state: "current",
      change_kind: "none",
      adoptable: false,
    });
  ...
  await t.test("same content at a newer commit remains current", async () => {
    ...
    assert.deepEqual(classify(pinned, value), {
      state: "current",
      change_kind: "metadata-only",
      adoptable: false,
    });
  });
  await t.test("same version", async () => {
    assert.deepEqual(classify(pinned, await latest(changedSameVersion())), {
  ...
  await t.test("minor version", async () => {
    assert.deepEqual(classify(pinned, await latest(changedVersion("0.3"))), {
      state: "review-pending",
  ...
  await t.test("major version", async () => {
    assert.deepEqual(classify(pinned, await latest(changedVersion("1.0"))), {
      state: "review-pending",
  ...
  await t.test("downgrade", async () => {
    assert.deepEqual(classify(pinned, await latest(changedVersion("0.1"))), {
      state: "review-pending",
...
test("successful same-version adoption returns only final adopted truth", async () => {
  ...
  assert.equal(
    loadAndVerifyPinned(box.referencePath).lock.adoption.status,
    "adopted",
  );
});
...
test("successful minor-version adoption returns adopted after Wiki compatibility", async () => {
  ...
  assert.equal(
    loadAndVerifyPinned(box.referencePath).lock.adoption.okf_version,
    "0.3",
  );
});
  ...
  for (const [name, candidate, flags, pattern] of cases) {
    await t.test(name, async () => {
      ...
      assert.throws(
        () =>
          acceptLatest(
            loadAndVerifyPinned(box.referencePath),
            value,
            classify(pinned, value),
            {
              root: box.root,
              referencePath: box.referencePath,
              runTests: () => {},
    ...
    () =>
      acceptLatest(
        ...
        adoptionOptions(box, {
          ...
          runTests: () => {
            throw new Error("strict fixture failed");
          },
        }),
      ),
...
test("malformed source and immutable metadata mismatches fail closed", async (t) => {
  await t.test("malformed version", async () => {
    const candidate = Buffer.from(
      source.toString("utf8").replace("**Version 0.2**", "**Version next**"),
    );
    await assert.rejects(latest(candidate), /exactly one parseable/);
  });
  await t.test("path mismatch", async () => {
    await assert.rejects(
      latest(source, { metadataPath: "other/SPEC.md" }),
      /mismatched OKF source metadata/,
    );
  });
  await t.test("blob mismatch", async () => {
    await assert.rejects(
      latest(source, { blobSha: "0".repeat(40) }),
      /does not match GitHub metadata/,
    );
  });
  await t.test("size mismatch", async () => {
    await assert.rejects(
      latest(source, { size: source.length + 1 }),
      /does not match GitHub metadata/,
    );
  });
});
...
test("embedded lock/body/source mismatches are rejected", async (t) => {
  await t.test("body mismatch", () => {
    const box = sandbox();
    const text = readFileSync(box.referencePath, "utf8").replace(
      "OKF is an open,",
      "OKF is a changed,",
    );
...
test("check and unknown lifecycle states never mutate the atom", async (t) => {
  await t.test("check reports review-pending read-only", async () => {
    ...
    const result = await synchronize({
      mode: "check",
      referencePath: box.referencePath,
      fetchImpl: fakeFetchFor(changedSameVersion()),
    });
...
test("single-atom failure and crash injection always restart from one coherent state", async (t) => {
  await t.test("rename failure retains old adopted atom", async () => {
    ...
    assert.throws(
      ...
      (error) =>
        error.message.includes("pre-rename crash") &&
        error.publication === "not-published",
    );
    const restarted = loadAndVerifyPinned(box.referencePath);
    assert.equal(
      restarted.lock.upstream.source_sha256,
      before.lock.upstream.source_sha256,
    );
  });
  await t.test(
    "crash after rename restarts from new adopted atom",
    async () => {
      const box = sandbox();
      const before = loadAndVerifyPinned(box.referencePath);
      const candidate = changedSameVersion();
      ...
      assert.throws(
        ...
        (error) =>
          error.message.includes("post-rename crash") &&
          error.publication === "published-coherent",
      );
      const restarted = loadAndVerifyPinned(box.referencePath);
