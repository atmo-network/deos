...
export const defaultReferencePath = join(
  skillDir,
  "references/okf-reference.md",
);
...
export function parseVersion(source) {
  ...
  if (matches.length !== 1)
    throw new Error(
      "OKF source must contain exactly one parseable **Version major.minor** declaration",
    );
  return {
    text: `${matches[0][1]}.${matches[0][2]}`,
    major: Number(matches[0][1]),
    minor: Number(matches[0][2]),
  };
}
...
function requireString(value, label, pattern) {
  if (typeof value !== "string" || (pattern && !pattern.test(value)))
    throw new Error(`Malformed OKF reference ${label}`);
}
...
export function loadAndVerifyPinned(referencePath = defaultReferencePath) {
  ...
  if (
    ...
    lock.standard !== "Open Knowledge Format"
  )
    throw new Error("Malformed OKF reference lock metadata");
  ...
  if (
    !Number.isInteger(upstream.source_trailing_newlines) ||
    upstream.source_trailing_newlines < 0
  ) {
    throw new Error("Malformed OKF source newline identity");
  }
  if (!lock.reference || lock.reference.embedding !== embedding)
    throw new Error("Malformed OKF reference embedding");
  requireString(lock.reference.body_sha256, "body SHA-256", /^[0-9a-f]{64}$/);
  const body = Buffer.from(split.body);
  if (sha256(body) !== lock.reference.body_sha256)
    throw new Error("Pinned OKF reference body SHA-256 mismatch");
  const marker = Buffer.from("# Open Knowledge Format (OKF)");
  const offset = body.indexOf(marker);
  if (offset < 0)
    throw new Error("Pinned OKF reference does not embed the upstream source");
  const source = Buffer.concat([
    body.subarray(offset),
    Buffer.from("\n".repeat(upstream.source_trailing_newlines)),
  ]);
  if (sha256(source) !== upstream.source_sha256)
    throw new Error("Pinned OKF source SHA-256 mismatch");
  if (gitBlobSha(source) !== upstream.blob_sha1)
    throw new Error("Pinned OKF source blob SHA-1 mismatch");
  if (parseVersion(source).text !== lock.pinned_version)
    throw new Error("Pinned OKF source version does not match lock metadata");
  if (
    !lock.adoption ||
    lock.adoption.bundle !== "wiki/index.md" ||
    lock.adoption.okf_version !== lock.pinned_version ||
    lock.adoption.status !== "adopted"
  ) {
...
export async function fetchLatestUpstream({
  ...
  const commitResponse = await request(
    fetchImpl,
    `${api}/commits?path=${encodeURIComponent(path)}&sha=${encodeURIComponent(ref)}&per_page=1`,
  ...
  if (
    ...
    commits.length !== 1 ||
    !/^[0-9a-f]{40}$/.test(commits[0]?.sha ?? "")
  )
    throw new Error("GitHub returned ambiguous OKF revision metadata");
  const commit = commits[0].sha;
  const contentResponse = await request(
    fetchImpl,
    `${api}/contents/${path}?ref=${commit}`,
    { headers: headers(token) },
    "GitHub source metadata lookup",
  );
  ...
  if (
    ...
    throw new Error("GitHub returned mismatched OKF source metadata");
  const rawUrl = `https://raw.githubusercontent.com/${owner}/${commit}/${path}`;
  const sourceResponse = await request(
    fetchImpl,
    rawUrl,
    { headers: headers(token) },
    "GitHub immutable source fetch",
  );
  const source = Buffer.from(await sourceResponse.arrayBuffer());
  if (source.length !== metadata.size || gitBlobSha(source) !== metadata.sha)
    throw new Error("Immutable OKF source does not match GitHub metadata");
  return {
    commit,
    revisionDate:
...
export function classify(pinned, latest) {
  ...
  if (latest.sourceSha256 === pinned.lock.upstream.source_sha256) {
    return {
      state: "current",
      change_kind:
        latest.commit === pinned.lock.upstream.commit
          ? "none"
          : "metadata-only",
      adoptable: false,
    };
  ...
  if (order === 0)
    return {
      state: "review-pending",
      change_kind: "same-version",
      adoptable: true,
    };
  if (latest.version.major === pinnedVersion.major)
    return {
      state: "review-pending",
      change_kind: "minor-version",
      adoptable: true,
    };
  return {
    state: "review-pending",
    change_kind: "major-version",
    adoptable: true,
  };
}
...
export function atomicWrite(path, content, options = {}) {
  ...
  const temporary = join(
    directory,
    `.${basename(path)}.${process.pid}.${(temporarySequence += 1)}.tmp`,
  );
  let published = false;
  let fileDescriptor;
...
function adoptedWikiVersion(root) {
  ...
  if (!match)
    throw new Error(
      "DEOS Wiki root has no parseable okf_version adoption declaration",
    );
  return match[1];
}

function runStrictTests(directory) {
  const result = spawnSync("npm", ["test", "--prefix", directory], {
    cwd: projectRoot,
    stdio: "inherit",
  });
  if (result.status !== 0)
    throw new Error("Strict Wiki/OKF tests failed; adoption refused");
}
...
export function acceptLatest(pinned, latest, classification, options = {}) {
  ...
  if (!classification.adoptable)
    throw new Error("Upstream OKF downgrade refused");
  if (!options.reviewed)
    throw new Error(
      "Reference update requires --reviewed after human/agent semantic review",
    );
  if (
    classification.change_kind === "same-version" &&
    !options.allowSameVersionRevision
  ) {
    throw new Error(
      "Changed content under the same OKF version requires --allow-same-version-revision",
    );
  }
  if (
    classification.change_kind === "minor-version" &&
    !options.allowVersionChange
  ) {
    throw new Error(
      "New OKF minor version adoption requires --allow-version-change",
    );
  }
  if (
    classification.change_kind === "major-version" &&
    !options.allowBreakingVersion
  ) {
    throw new Error(
      "New OKF major version adoption requires --allow-breaking-version",
    );
  }
  if (adoptedWikiVersion(options.root ?? projectRoot) !== latest.version.text) {
    throw new Error(
      `DEOS Wiki has not adopted OKF ${latest.version.text}; migrate schema and index.md before reference adoption`,
    );
  }
...
export async function synchronize(options = {}) {
  ...
  let latest;
  try {
    latest = await fetchLatestUpstream({
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const mode = argv.includes("--sync")
    ...
    : argv.includes("--check")
      ? "check"
      : null;
  if (!mode || (argv.includes("--sync") && argv.includes("--check")))
    throw new Error("Choose exactly one of --check or --sync");
