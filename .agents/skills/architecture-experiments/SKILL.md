...
An active redesign MAY add one `Machine-Checkable Current Lineage` table to its existing track index instead of migrating historical records. Each lineage row names one current lineage identity, one or more materialized source claims, an explicit applicability qualification, exact current `BACKLOG.md` consumers, the required proof class, and a current closure owner. An adjacent `Machine-Checkable Numerical Authority` table names each new semantic/profile identity and whether it has numerical authority. The validator checks missing transfers, unknown records/tasks, source claims used by the adjacent deliberate-import section without qualification, and absent or malformed profile authority. `Semantic oracle`, `Round/wake proof`, `Physical closure`, and `Resource closure` classify what evidence must eventually close the transfer; none means design acceptance, connected implementation, executable closure, or resource binding. Numerical authority is exactly `None` unless a labelled exact-identity bridge supplies a 40-hex source commit and 64-hex artifact hash as `Exact bridge: <commit>/<hash>`; applicability prose or a newer profile cannot silently inherit historical numbers. Validation proves only graph/reference and bridge-identity integrity, never semantic equivalence, wake completeness, physical soundness, artifact applicability, or Weight sufficiency.
...
Accepted may be scoped to physical architecture: state `Decision scope: physical architecture only` and distinguish Architecture Freeze from later production/release Geometry Freeze. One materially distinct hypothesis has one experiment owner. A scoped acceptance neither proves Weight soundness nor meets a throughput target. A frozen architecture has two independent evidence routes: a structural/soundness falsifier reopens mandatory correction regardless of campaign allowance, while a qualified performance falsifier may admit one bounded same-semantics comparison without claiming the historical decision was invalid. A cost observation alone is insufficient: the route requires a predeclared missed criterion or protected regression, complete relevant work, one causally implicated physical owner, one concrete alternative and its smallest falsifier. A stale artifact, unreachable fixture, provisional Weight, or unrelated old coefficient cannot open either route. The owning campaign bounds candidates and refinement before decision measurement; selecting an alternative records a Superseded transition with preserved historical validity. Corrections preserving frozen invariants belong to that successor, and micro-optimizations require a measured production owner plus a dedicated falsifiable question.
