...
An active redesign MAY add one `Machine-Checkable Current Lineage` table to its existing track index instead of migrating historical records. Each row names one current lineage identity, one or more materialized source claims, an explicit applicability qualification, exact current `BACKLOG.md` consumers, the required proof class, and a current closure owner. The validator checks missing transfers, unknown records/tasks, and source claims used by the adjacent deliberate-import section without qualification. `Semantic oracle`, `Round/wake proof`, `Physical closure`, and `Resource closure` classify what evidence must eventually close the transfer; validation proves only graph/reference integrity, never semantic equivalence, wake completeness, physical soundness, or Weight sufficiency.
