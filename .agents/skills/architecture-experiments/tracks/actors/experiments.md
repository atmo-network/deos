...
This is an intake routing ledger, not proof that a hazard is closed. `Applicable` means the current design carries the obligation now. `Conditional` activates only if the named mechanism or old quantitative premise is consumed. `Historical` preserves a rejected result without turning it into a current defect. N5.3 closes the composed ledger only after each applicable route has evidence from its named implementation and review owners.
