...
run_audit() {
    ...
    check_rule "\\.iter\\(\\)|for .* in .*::iter\\(\\)" "Potential unbounded state iteration detected. DEOS prefers bounded/O(1) mechanics." "tests.rs|benchmarking.rs|mock.rs|deos-bypass: bounded-iter" "O(N) Iteration Anti-Pattern" "docs/staking.architecture.en.md" "FATAL" "hallucination" "The violation is a concrete false move against the O(1) contract"
