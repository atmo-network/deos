...
use pallet_aaa::{
  ...
  InputLimit, LiquidityOps, Mutability, OutcomeTotals, ProgramInput, RetryClass, Schedule, ScheduleOf,
...
fn dex_exact_out_adapter_rejects_unfunded_input_with_explicit_error() {
  seeded_test_ext().execute_with(|| {
    ...
    let result = <crate::configs::aaa_config::TmctolDexOps as DexOps<
      ...
      u128,
    >>::swap_exact_out(
      ExecutionContext::new(&unfunded, AaaType::User),
      AssetKind::Native,
      AssetKind::Local(ASSET_A),
      crate::EXISTENTIAL_DEPOSIT,
...
fn remove_liquidity_minimum_failure_preserves_each_error_policy_path() {
  seeded_test_ext().execute_with(|| {
    ...
    for policy in [
      ...
      let aaa_id = create_system(ALICE, manual_schedule(), None, plan);
      let actor = aaa_account(aaa_id);
      fund_native(aaa_id, crate::EXISTENTIAL_DEPOSIT.saturating_mul(2));
      ...
      assert!(!has_aaa_event(|event| matches!(
        event,
        Event::LiquidityRemoved { aaa_id: id, .. } if *id == aaa_id
      )));
      assert!(
        has_aaa_event(|event| matches!(
          event,
          Event::StepFailed { aaa_id: id, .. } if *id == aaa_id
        )),
      ...
      match policy {
        ...
        StepErrorPolicy::ContinueNextStep => {
          ...
          assert!(has_aaa_event(|event| matches!(
            event,
            Event::CycleStopped { aaa_id: id, step_index: 1, .. } if *id == aaa_id
          )));
          assert!(AAA::continuation_state(aaa_id).is_none());
        }
        StepErrorPolicy::RetryLater { .. } => {
          ...
          let continuation =
            AAA::continuation_state(aaa_id).expect("temporary minimum failure suspends");
...
fn repeated_same_block_transfers_coalesce_to_one_actor_execution() {
  seeded_test_ext().execute_with(|| {
    ...
    fund_native_via_call(ALICE, aaa_id, 100_000_000_000_000);
...
fn circular_actor_graph_cannot_reexecute_an_actor_in_the_same_block() {
  seeded_test_ext().execute_with(|| {
    ...
    let actor_a_account = aaa_account(actor_a);
    ...
    let actor_b_account = aaa_account(actor_b);
...
fn dynamic_asset_producers_notify_directly_with_balance_only_provenance() {
  seeded_test_ext().execute_with(|| {
    ...
    let mint_sovereign = aaa_account(mint_actor);
...
fn stress_10k_actors_queue_scheduler() {
  ...
  new_test_ext().execute_with(|| {
    ...
    let aaa_ids = setup_mixed_inert_actors(actor_count, initial_balance);
