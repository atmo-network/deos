...
impl<T: Config> Pallet<T> {
  ...
  fn finalize_zero_step_on_legacy_fifo(
    ...
    let (_, status, _) =
      Self::finalize_next_residence_on_legacy_fifo(actor_id, next_residence, admission, now)?;
  ...
  fn finalize_next_residence_on_legacy_fifo(
    ...
    match next_residence {
      NextResidence::Close { state, reason } => {
        Self::finalize_actor_from_consumed_state(actor_id, state, admission, reason)
          .map_err(|_| AttemptTransactionError::Invariant)?;
        Ok((
  ...
  fn finalize_effectful_step_on_legacy_fifo(
    ...
    let (placement, residence_status, closed_for_exhaustion) =
      Self::finalize_next_residence_on_legacy_fifo(actor_id, next_residence, admission, now)?;
    if matches!(residence_status, AttemptDisposition::Closed(_)) {
      attempt.status = residence_status;
  ...
  /// Builds one generation-bound due-retry plan directly from canonical Service authority.
  /// The synthetic ticket carries immutable execution coordinates only; no legacy FIFO ticket or
  /// control cell participates in admission.
  #[allow(
    dead_code,
    reason = "canonical Service execution remains staged behind the atomic publication cutover"
  )]
  pub(crate) fn build_due_retry_service_plan(
    actor: ActorRef,
    kind: ServiceResidenceKind,
    now: BlockNumberFor<T>,
  ) -> Option<(
    ...
    let semantic = Self::load_service_actor_semantic_state(actor, kind).ok()?;
    let encounter = polkadot_sdk::frame_support::storage::with_transaction_unchecked(|| {
      polkadot_sdk::frame_support::storage::TransactionOutcome::Rollback(
        Self::consider_service_head(now).ok(),
      )
    });
    if semantic.hot.cycle_state != CycleState::Suspended
      || encounter != Some(ServiceRoundEncounter::Eligible(actor))
    {
      return None;
    }
    let head = ActorContractHeads::<T>::get(actor.actor_id)?;
    let (state, admission, loaded_step) = Self::load_actor_service_state_with_head(
      actor.actor_id,
      semantic.identity,
      semantic.hot,
      semantic.admission,
    ...
    .ok()?;
    let plan = StepExecutionPlan {
      identity: state.identity.clone(),
      hot: state.hot.clone(),
      run: state.run_state.clone(),
  ...
  pub(crate) fn prime_frame_actor_schedule(actor_id: ActorId) -> Result<(), EnqueueOutcome> {
    // A canonically published Actor already owns exactly one process residence; legacy priming
    // would create a parallel physical control cell for the same work.
    if ActorProcesses::<T>::contains_key(actor_id)
      && !ActorControlLocators::<T>::contains_key(actor_id)
      && !ActorUnsignaledControlCells::<T>::contains_key(actor_id)
    {
      return Ok(());
