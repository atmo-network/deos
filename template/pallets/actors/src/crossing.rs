...
impl<T: Config> Pallet<T> {
  ...
  pub(crate) fn do_try_state_crossing() -> Result<(), polkadot_sdk::sp_runtime::TryRuntimeError> {
    ...
    while let Some(feed) = current {
      ...
      if queue
        .iter(/* deos-bypass: bounded-iter */)
        .any(|transition| transition.revision == 0 || transition.previous == transition.current)
      {
        return Err(invalid());
      }
  ...
  fn classify_fire_activation(
    ...
    match plan.map(|plan| plan.publication) {
      Some(CanonicalOccurrencePublication::PublishPending { .. }) => {
        Ok((CrossingWorkPlan::FireCohortPlaced, false, None))
      }
  ...
  pub(crate) fn prepare_crossing_rearm_hot(
    ...
    opening_observation: Option<crate::CanonicalObservationState>,
    ...
    let current_observation =
      opening_observation.unwrap_or_else(|| T::ObservationProvider::current(&crossing.feed));
    let (current, installed_at_revision) = match current_observation {
