...
#[test]
fn latched_contract_replacement_preserves_temporal_trigger_authority() {
  for zero_step in [false, true] {
    new_test_ext().execute_with(|| {
      ...
      replacement.trigger = RuntimeTrigger::at_time(10);
      ...
      assert!(
        ...
        "Opening consumes AtTime even when replacement preserves another family's latch: {hot:?}",
      );
      assert!(!hot.pending_signal);
      assert!(hot.trigger_wakeup_pointer.is_none());
      assert_eq!(hot.cycle_state, CycleState::Idle);
      assert_eq!(Actors::active_actor_view(actor_id).unwrap().cycle_nonce, 1);
      #[cfg(feature = "try-runtime")]
      Actors::do_try_state().expect("ordinary replacement and Opening preserve temporal authority");
    });
  }
}
...
fn window_expiry_cancels_while_failure_cutoff_finalizes_before_close() {
  new_test_ext().execute_with(|| {
    ...
    run_canonical_round_at(105, Weight::MAX);
    frame_system::Pallet::<Test>::reset_events();
    run_canonical_round_at(107, Weight::MAX);
