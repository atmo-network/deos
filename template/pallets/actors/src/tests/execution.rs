...
#[test]
fn control_invocation_receipts_are_independent_of_actor_fee_exemption() {
  for actor_type in [ActorType::User, ActorType::System] {
    for phase in [
      crate::StepControlPhase::Opening,
      crate::StepControlPhase::Running,
      crate::StepControlPhase::Suspended,
    ] {
      for result in [0, 1, 2] {
        new_test_ext().execute_with(|| {
          ...
          if phase == crate::StepControlPhase::Running {
          ...
          let steps =
            crate::ContractSteps::<Test>::try_from(steps).expect("small phase witness fits");
          let pipeline_fee = pipeline_opening_fee(&steps);
          let actor_id = if actor_type == ActorType::User {
            create_user_with(ALICE, Mutability::Mutable, manual_schedule(), None, steps)
          } else {
            create_system_with(ALICE, manual_schedule(), None, steps)
          };
          ...
          if phase != crate::StepControlPhase::Opening {
            ...
            let run =
              Actors::actor_run_state(actor_id).expect("real prefix or failure retains Run");
            assert_eq!(
              run.cursor,
          ...
          set_fail_transfer_to((result == 2).then_some(BOB));
          System::reset_events();
          ...
          Actors::execute_cycle(Weight::MAX);
          let execution =
            last_step_control_execution().expect("committed Step reports Control evidence");
          ...
          assert_eq!(
            execution.action_fee_collected,
            actor_type == ActorType::User && result != 0
          );
          let receipts = System::events()
            ...
            .collect::<Vec<_>>();
          let action_fee = if actor_type == ActorType::User && result != 0 {
            user_step_fee(&step)
          } else {
            0
          };
          ...
          let opening_fee =
            if actor_type == ActorType::User && phase == crate::StepControlPhase::Opening {
              pipeline_fee
            } else {
              0
            };
          assert_eq!(
            ...
            opening_fee + action_fee
          );
          assert_eq!(
            asset_balance(&BOB, TestAsset::Local(1)) - recipient_before,
            u128::from(result == 1)
          );
