...
- `StepControlWeight`: Typed authority returning its production Weight identity, maximum Actor-control Weight for an exact Step/context, and valid actual Weight for the committed `StepControlExecution { outcome, placement }`. The context distinguishes the inline head from the populated C6 tail fragment, charges exact authored Opening tail-chunk reconstruction, and bounds predicate/Opening/funding geometry. Missing or component-wise greater-than-reserved actual evidence rolls back the complete attempt; a host may conservatively report the admitted maximum until atomic generated branch owners justify reclaim.
