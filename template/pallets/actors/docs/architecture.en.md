...
Generic activation preflight derives its frame admission from whichever authority owns the Actor: a pre-cutover Actor keeps the validating legacy primary-control-cell read, while a canonically published Actor (no `ActorControlLocators`) derives admission and identity from the generation-bound semantic owner and fails closed when it is absent or mismatched. The generic plan entry itself (`preflight_activation_loaded`/`request_activation`) is now a test/benchmark fixture with no production caller, and temporal occurrence processing fails closed for any carrier that is not canonically published, so canonical publication is the sole temporal placement owner.
