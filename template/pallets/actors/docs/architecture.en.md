...
Opening and running StopCycle execution still construct their semantic results inline, but both now delegate auto-close, scheduler-exhaustion closure, unsignaled restoration and republication to `finalize_stop_cycle_residence_on_legacy_fifo`; their distinct resource envelopes remain explicit. Inline attempt evidence reports the actual outcome and finalizer result without becoming persisted Run authority.
