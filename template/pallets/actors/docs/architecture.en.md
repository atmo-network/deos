...
Activation preflight now always consumes an admission certificate supplied by its caller's canonical service load, and the fixture-only generic plan entry (`request_activation`/`preflight_activation_loaded`/`test_activation_plan_kind`) is retired. Benchmark occurrence fixtures latch through the canonical Trigger owner using the actor's declared Trigger family. Temporal occurrence processing fails closed for any carrier that is not canonically published, so canonical publication is the sole temporal placement owner.
