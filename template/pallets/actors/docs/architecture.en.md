...
- `ServiceHeader` / `ServiceNodes`: inert canonical service-ring storage with one generation-bound node per Actor and transaction-required append/remove helpers; process residence must match, legacy authority must be absent, and no production scheduler path reaches this carrier
...
- `ServiceHeader` / `ServiceNodes`: inert persistent-ring header and actor-keyed nodes; transaction-local append/remove verifies matching process residence, generation, local links, singleton/interior/cursor structure and legacy-authority exclusion, while enclosing rollback restores the prior ring
