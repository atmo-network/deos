...
- `ActorSemanticStates`: canonical lifecycle semantic and Contract-generation owner; stores dormant identity plus last generation or active Identity/generation/Hot/admission independently of physical placement. Initial Contract publication uses generation one, replacement checked-increments it, dormancy preserves it, and ordinary Hot updates do not change it. The legacy loader cross-validates the physical primary; the staged Service loader instead requires matching semantic/process/node generation and residence with no legacy authority
...
- `ActorProcesses`: inert generation-bound process storage for the pending service-carrier cutover; only isolated helper tests populate it, while the legacy paged FIFO remains sole production scheduler authority
- `ServiceHeader` / `ServiceNodes`: inert canonical service-ring storage with one generation-bound node per Actor and transaction-required append/remove helpers; process residence must match, legacy physical scheduler authority must be absent, and staged direct Hot mutation updates only `ActorSemanticStates`. No production scheduler path reaches this carrier
- `DeadlineHeaders` / `DeadlinePages` / `DeadlineHandles`: inert retained C32 deadline buckets, linked pages, and exact generation-bound reverse handles; transaction-required insert/remove/move helpers reject legacy authority, require matching process residence, reuse fragmented slots, remove empty pages, and keep production Waiting/wakeup paths unchanged
- `ActorControlCell`: active physical mirror of Identity/Hot/admission plus failure counter, independent Block/Tick pointers, terminal block, schedule anchor, admission-derived Pipeline service identity, cursor/eligibility service projection and resource envelope; `ActorControlLocators` resolves its Unsignaled/Ready/Waiting owner, and strict loading rejects any mismatch with `ActorSemanticStates`
...
`PipelineMachineEnvelope` stores the complete bounded User control/cleanup quote in the certified Contract head; scheduler admission reads that O(1) authority only when Idle paid readiness is consumable. Zero-Step machine work currently uses generated `scheduler_inner_zero_step_complete`; EXP-0037 accepts a scoped Manual System/User partition and complete maximum-head User owner, EXP-0038 accepts the mutually exclusive AtTime/Cadenced persistence/authored-close owners, and EXP-0039 accepts available Crossing persistence/authored-close envelopes plus a state-preserving unavailable refusal owner. Neither temporal nor Crossing Opening charges a second Trigger fee; unavailable Crossing collects no Pipeline fee.

EXP-0040 accepts one component-wise Observation authored-close owner across dense/sparse subscription pages, middle subscriber/feed unlink, dirty-cursor repair and existing/new free-page publication. That inclusive owner already contains Pipeline collection, User hold reconciliation and terminal cleanup. Outer source/Waiting composition and complete production binding still require G4/G5 closure before numerical release acceptance.

EXP-0041 accepts the attained reference maximum-Step Crossing plus singleton future-window Pipeline-apoptosis atom at `709,110,000 / 86,870` before database additions, with 70 reads and 65 writes. It includes canonical Ready consumption, three cold Contract-tail chunks, Crossing cleanup, Block-reference invalidation and applicable User hold release, with no Pipeline receipt or custody movement. This complete branch must not receive a second generic source or cleanup charge. Populated Waiting/heap composition remains EXP-0063 work, and EXP-0065 owns exact regeneration.

EXP-0042 accepts the admitted joint maximum-Step Suspended Run/Crossing public-close atom at `(O=4, P=12)`: `742,998,816 / 38,096` before database additions, with 34 reads and 32 writes. It includes Contract/tail and Run deletion, a singleton future Waiting removal, real deferred-latch/live-cursor repair and ordinary User Account/Hold release, while preserving custody and charging no new fee. Observation cleanup is a mutually exclusive branch. Populated Waiting/heap alternatives remain EXP-0063 work and must not duplicate this atom's Contract/Run/lifecycle cleanup; EXP-0065 owns exact regeneration.

EXP-0058 retains corrected generic Ready-consume and Queue-publication primitives whose fixtures begin from loaded or consumed authority. The per-entry settlement seam replaces provisional discovery/probe/source accounting when admission selects a complete apoptosis, terminal-close, or Crossing-unavailable refusal owner, while retaining unrelated earlier and later FIFO work.

Crossing availability is selected only after terminal and User Pipeline-capacity precedence; an available value/revision is carried into transactional rearm without a second provider read. Pause already demotes its Ready primary to Unsignaled atomically, so later service sees only a stale tombstone; breaker refusal leaves the live Ready owner and exact FIFO position unchanged after discovery/probe rather than consuming and republishing it. The experiment remains Measuring because executed-Step Run/placement fallbacks and Waiting input are open. No synthetic coefficient may be derived by subtraction, and production regeneration remains EXP-0065 work.

EXP-0043 accepts the unsignaled Immutable zero-Step Manual User singleton-Block expiry atom at `261,001,000 / 6,367` before database additions, with 24 reads and 21 writes. It includes canonical Block-source and Waiting authority removal, loaded expiry classification, lifecycle/process/deadline cleanup and applicable User hold release while preserving custody and producing no Pipeline receipt. Outer clock selection and populated Waiting/heap alternatives remain EXP-0063 work; generic source or terminal cleanup must not be charged again, and EXP-0065 owns exact regeneration.
...
Suspended resource refusal first reads the Run head and current certified Step to preserve retry exhaustion and Action liability. A coherent solvent retry needs neither Funding nor Run payload; User liability still reads its native balance. Insolvency and exceptional paths retain full classification and cleanup. `suspended_weight_refusal_avoids_run_payload_and_funding_reads` records trie reads for Temporary/FundingUnavailable, User/System, head/tail cursor and independently scarce aggregate/control/effect RefTime and ProofSize before the last allowed attempt, with unchanged state and later progress controls. Expired-window tests retain cleanup precedence under pause and insolvency, while the breaker preserves Run. These native witnesses do not establish a complete production Weight envelope.
...
Opening prepares Trigger rearm within the consumed-source execution transaction. Cadenced registers a fresh Tick Trigger deadline in the shared `TriggerDeadlineHandles` carrier for canonically published Actors, using the legacy waiting reference only for pre-cutover Actors; Crossing rebuilds membership from current observation while preserving the incoming latch until the common core consumes it; ObservationChange removes its disabled marker. AtTime sets its consumed flag, including when Contract replacement retained another Trigger's paid latch. Manual and AddressEvent require no rearm mutation. The execution successor publishes the resulting hot state; no separate retained-primary rearm publisher intervenes.
...
Activation plans carry the validated admission certificate and original source state as mandatory values. Cancellation likewise always carries admission, but retains distinct retained-source and consumed-source variants because publication must respect whether the original primary still exists. Loaded cancellation uses the supplied Run rather than selecting it again from backing storage; no Run is a no-op only for Idle state, while non-Idle without Run fails before writes. Canonical cancellation republishes from the durable admitted Contract geometry rather than the bounded current-Step projection, because the control loader's reconstructed Contract body commitment differs from the stored full body for a multi-Step Actor. No mode selector chooses between scalar and canonical authority.

Canonical occurrence publication is synchronous and transactional, with no intervening Actor mutation: the stored semantic record is the publication authority, and the caller supplies only the bounded current-Step service envelope that the identity/Hot/Run-store/legacy-absence checks bind against. A publication refuses rather than publishing a partial successor, and its caller's transaction rolls the whole occurrence back.
...
- `actor_prepass` inherent:
  ...
  - freezes `PrepassExecutionCutoff`, performs one bounded saturated-FIFO stale-cleanup quantum, services the rotated Crossing and broad-fanout materialization families against reserved Actor Control (the rotated family-0 slot is retired and yields zero), and keeps every newly materialized ticket behind that cutoff
...
- Pipeline-admission insufficiency selects generated `pipeline_admission_apoptosis`; other terminal reasons and the economically collected cleanup upper select `close_actor`. Actual cleanup retains the Actor's Contract, detector, placement, and owner-hold geometry. The one-Step Manual apoptosis fixture alone does not establish an all-context cleanup bound; the Crossing diagnostic includes retained threshold membership and ordinary owner hold access.
- Close removes the exact primary and invalidates remaining Block/Tick references transactionally. A canonically published Actor (no `ActorControlLocators`/unsignaled cell) is closed through `remove_actor_publication_and_finalize`, which releases its generation-bound `ActorProcesses` publication and Service/Deadline residence exactly once before converging on the custody-neutral finalizer; the legacy retained-frame path remains for pre-cutover Actors. Ready removal leaves an ordered empty slot until bounded head advancement; Waiting removal releases its slot and unlinks an empty page.
- Deactivation of a canonically published Actor detaches its generation-bound publication through `detach_actor_publication`, which releases the Service/Deadline/Trigger residence and removes `ActorProcesses` while preserving the durable identity, so the resulting dormant record retains no scheduler work and no legacy authority; the legacy queue/wakeup cleanup remains for pre-cutover Actors. `remove_actor_publication_and_finalize` reuses the same detach owner before finalizing.
...
EXP-0057 accepts the current reference-Wasm useful-occurrence matrix: Manual `207,502,000 / 15,106` with 14 reads/7 writes; AddressEvent `230,410,000 / 8,394` with 14/7; ObservationChange `153,933,000 / 8,299` with 13/8; ObservationCrossing `618,734,000 / 164,188` with 90/81; AtTime `301,091,000 / 8,451` with 20/9; and Cadenced `469,271,000 / 8,450` with 27/16. Values are pre-database RefTime/estimated ProofSize. Physical host, fanout, Crossing and Tick workers retain their inclusive source/traversal owner, so these economic coefficients are not added again. The temporary fragments remain uninstalled; EXP-0065 owns complete regeneration.
...
Production `service_live_queue_entry` dispatches the consumed-frame zero-Step owner and routes StopCycle/effectful attempts through `execute_current_step_and_place`, shared with inner benchmarks. For an effectful Step, `execute_effectful_step_transition` produces the carrier-neutral semantic result from `execute_loaded_single_step_core`; production still uses `finalize_effectful_step_on_legacy_fifo`, while the staged canonical Service adapter commits the narrow successful completion-and-retain outcome through generation-bound semantic storage before ring advancement. Retry, continuation, deadline and terminal effectful destinations remain fail-closed until each has an atomic canonical residence commit.

Empty Contracts follow the same boundary: `execute_zero_step_transition` completes the semantic cycle; production uses `finalize_zero_step_on_legacy_fifo`, while staged canonical Service tests cover retained completion and terminal cleanup. Rollback simulation uses these same semantic owners for one selected Ready Actor, without consuming earlier peers or advancing a virtual block.

Opening and running StopCycle have distinct semantic constructors that produce one carrier-neutral `StopCycleTransition`; running construction removes persisted Run authority before summary, while Opening preserves `CycleStarted` before Stop/Skip. `finalize_stop_cycle_on_legacy_fifo` alone owns legacy close or republication, actual Weight, fee and hold reconciliation, and `StepCommitEvidence`; the transition preserves Opening-current versus Running-cursor-0 resource envelopes.
...
A canonically published Actor takes a third publication mode: non-close cancellation routes through `cancel_actor_publication`, which releases the generation-bound Service or Deadline residence and any independent temporal Trigger deadline, clears the open Run, and republishes one Idle successor through `publish_actor_publication` instead of mirroring a legacy control cell; the legacy frame prime is retired, so canonical publication is the sole process-placement owner for a replaced or cancelled canonical Actor.

A canonical occurrence publication that cannot complete fails closed: the trigger-occurrence owner rolls back the whole occurrence on any invariant, admission, or capacity refusal, so no successor is published and the retained deadline re-attempts on a later service pass. Terminal closure is never inferred from a placement or capacity mode; it requires an explicit canonical classification.

Temporal progression refreshes the canonical service tuple after placement and state-hold reconciliation, then classifies the loaded canonical Actor before publishing. A terminal classification closes through the retained-state finalizer, an underfunded AtTime occurrence closes through prepaid fee-free minimal apoptosis, and a nonterminal occurrence publishes through the canonical trigger-occurrence owner. No synchronous activation plan or second preflight remains.

The generic activation plan is retired: `ActivationPlan`, `ActivationAction`, `PrimeSchedulePlan`, `preflight_activation_from_authority`, `commit_activation_plan`, their paged-enqueue/deferred-wakeup helpers, and the fixture producers `request_activation`/`preflight_activation_loaded`/`test_activation_plan_kind` no longer exist. Activation consumes a certificate supplied by its caller's canonical service load, benchmark occurrence fixtures latch through the canonical Trigger owner using the actor's declared Trigger family, and temporal occurrence processing fails closed for any carrier that is not canonically published.

Canonical Crossing fire classification resolves only the placed or coalesced cohort; the retired terminal-cohort close plan and its separate final-member membership cleanup were unreachable legacy producer paths, so no activation preflight path recreates a legacy primary. The ObservationChange reaction owner instead fails closed inline when canonical observation authority is absent or incoherent, and delegates to the canonical trigger-occurrence owner when authority is present; no legacy compact activation producer remains.
...
Each live FIFO entry snapshots the cycle and Actor Control meters before its discovery. Generic nonterminal service retains discovery, loaded-state probe, Ready consumption and one inner/destination owner. Admission-time apoptosis, terminal close, or state-preserving Crossing-unavailable refusal instead evaluates and settles from that snapshot, replacing the provisional entry prefix with one complete owner without coefficient subtraction. Breaker refusal is distinct: it retains the live Ready source in place and charges only its discovery/probe. A coherent paused Actor has already moved to Unsignaled, so its old Ready slot is only a tombstone; a live paused Ready cell is malformed rather than a benchmarkable transition. Tombstones completed before an entry and later next-head or empty-queue probes remain separately charged physical operations.
...
The production `execute_cycle_to_cutoff_with_resources` path pre-reserves the complete pass Actor Control maximum and enforces that same component-wise limit through a dedicated control meter across queue scan, actor probe, Opening, Step, cleanup, and placement work, then settles the full reconciled pass control afterward. Canonical Service step control is owned by that enclosing pass reservation: the prepass calls `service_canonical_round_head_with_reserved_control` with the phase-owned effect domain, so the round reserves only Action effect capacity and never double-books Actor Control, while direct service-round callers without an enclosing pass reservation still reserve both lanes through `service_canonical_round_head_with_resources`.

The canonical Service round classifies the selected head before deciding. A due schedule window or exhausted cycle nonce closes the Actor through the atomic `finalize_actor` owner and reports `Closed` instead of advancing an Idle resident, so a terminal deadline returned to Service cannot be discarded as idle-no-work while the Actor stays active past its window.

Actual effects settle transactionally and the pass reconciles its control once. Prepass admission withholds the generated `scheduler_on_idle_base + block_resource_finalize` control envelope so ordinary head-of-line exhaustion cannot strand the mandatory ExternalPhase-to-Finalizable transition.
...
| Minimal Pipeline-admission apoptosis | `4,971,257,226 / 45,716` | `0 / 0` |

The current runtime fixture includes fixed on-idle work, one complete apoptosis entry owner, and the later empty-queue probe. Per-entry replacement removes the formerly additive discovery/state-probe/Ready-consume prefix (`1,666,703,613 / 17,991`) from this exact telemetry without deriving a coefficient. The generated narrow `pipeline_admission_apoptosis` remains stale pending EXP-0065; the accepted complete replacement value is not yet installed. The transition emits `CycleAdmissionInsufficient`, deletes process semantics, preserves sovereign custody, and invokes no Task effect.
...
Counts are generated owner maxima rather than distinct physical keys or an additive complete-block total. Generic branches retain the shared outer scan/probe/consume frame. Complete admission-close and Crossing-unavailable refusal branches replace that entry frame through the saved meter baseline and a typed complete-result selector.
...
The package retains the pre-cutover paged wakeup substrate and sparse cursor as inert topology:
...
- The cursor-driven overdue worker is a retained pre-cutover worker that benchmarks and legacy fixtures still exercise directly; production no longer invokes it because the coordinator's family-0 slot is retired. It floors consensus milliseconds into the current tick, reselects after every admitted unit, and round-robins simultaneously due block/tick clocks while preserving FIFO order inside each clock. It meters each cursor lookup, page scan, queue append, and possible full-depth cursor removal before mutation; partial progress keeps each clock's same minimum for later resumption.
- The coordinator gives the rotated first family its generated maximum-unit minimum plus the shared lendable remainder, while reserving one complete maximum-unit minimum for each later family. The retained family-0 quantum is released back to the shared remainder by the same lending pass because family 0 consumes no Weight; the pre-cutover wakeup worker consumes only that grant when a legacy fixture drives it and remains independently bounded by `MaxWakeupsPerBlock`; a complete cursor/drain unit must fit before mutation, and returned hook Weight cannot exceed the caller budget.
...
- Opening captures predicate and balance snapshots once; Running/Suspended service reuses them. Q1, the deferred readiness latch, authored error policies, and committed-prefix durability remain independent of placement.
...
- Genesis records an uninitialized temporal anchor and one tick-zero bootstrap wakeup because no consensus timestamp exists during construction. The first ordinary wakeup service resolves that bootstrap marker: a canonically published Actor initializes the anchor in its semantic record and re-places the full-delay deadline through `plan_deadline_destination` plus `insert_trigger_deadline_member`, while a pre-cutover carrier keeps the legacy waiting-substrate transition. Neither path latches readiness or enters FIFO.
...
- Opening atomically rearms Cadenced to the first aligned tick strictly after current consensus time before Pipeline charging, registering the canonical `TriggerDeadlineHandles` member alongside the semantic `trigger_wakeup_pointer` so the shared deadline frontier observes it. Failure rolls back readiness consumption, the member, and fee movement. Only Pipeline-service FIFO membership executes the Actor; any future probabilistic execution requires separate admission policy and secure runtime entropy.
...
Crossing ingress requires a checked successor of the last retained transition revision and rejects exhaustion atomically. TryRuntime checks positive revisions and the same strict succession throughout each retained queue; the certified publisher owns revision history across drained queues.
...
A newly latched nonterminal fire uses generated `crossing_placed_unit` Weight (`558,181,000 / 162,782`, 89 reads and 80 writes) for phase movement, one User occurrence collection, and exactly one canonical FIFO placement without terminal cleanup. A fee transfer that cannot complete rolls back the atomic canonical occurrence commit while the already-committed traversal phase stands: the member remains `WaitingForRearm` with no readiness, placement, occurrence fee, event, or detection-disable latch, and `crossing_fire_collection_failure_advances_without_readiness` pins that refusal.

The current EXP-0057 economic owner `observation_crossing_trigger_occurrence` is `618,734,000 / 164,188` with 90 reads and 81 writes; it begins from an already published transition and retained threshold, so Oracle publication/signaller work is excluded. This staged fragment supersedes the stale installed coefficient as current evidence but remains uninstalled until EXP-0065 regenerates the complete file. A post-installation transition skip uses generated `crossing_skip_unit` Weight (`171,882,000 / 81,886`, 40 reads and two writes), preserving phase and placement while advancing the stale candidate.
...
Dedicated pair probe and execution owners charge and commit both in one transaction. Placed-batch accounting and Weight admission carry exact bounded candidate counts: two uses the generated atomic pair owner, while homogeneous larger cohorts use the generated maximum-batch owner. In `select_crossing_admission`, only a tail-page batch may fall back to the pair; a non-tail batch whose complete owner does not fit falls directly to the scalar single, or refuses without mutation if the scalar reservation also fails. `crossing_admission_fallback_respects_source_geometry_and_weight_dimensions` covers both geometry and independent RefTime/ProofSize limits; `crossing_non_tail_limited_grant_preserves_scalar_progress_without_fault` proves exact scalar state-root equivalence and continued worker service.

Placed-batch production reconstructs one coherent bounded cohort authority. Count two uses scalar-equivalent pair movement: the cursor candidate first, then the tail candidate whose post-compaction locator sequential single-member movement predicts and the commit path asserts, so the atomic pair serves exactly the members sequential scalar admission would serve, in order. A homogeneous contiguous P128 tail prefix may admit up to 128 candidates and uses descending physical movement followed by stable retained-suffix rewrite and locator repair. Both paths commit one queue plan plus one cursor/feed advancement. Exact-root tests reject corrupted generation or compact authority before unsafe grouped mutation. The fixed-depth radix seek has a separate generated owner charged only while `current_threshold` is absent, so candidate service reuses the retained threshold without sparse-work doubling.
...
| --- | --- | --- |
| Actor Control envelope | `302,102,663,242 / 676,150` | Every generated complete branch fits component-wise after fixed-context composition |
...
The 10,000-member single-threshold load profile exhibits no failure mode distinct from the total per-feed horizon, so the runtime intentionally adds no separate leaf cap; exact page and worker bounds already constrain physical service. These are bounded capacity facts, not a latency SLA. The cohort counts assume one homogeneous placed-fire head transition, sufficient Actor Control, solvent Actors, available placement, and no competing materialization pressure. Alternating revisions may occupy all 64 transition slots; admission then fails atomically until service creates capacity.

Under the reference composition's generated component-wise admission and materialization family rotation, the previously released cohort executed `1`, `3` and `8` Crossing candidates across the three-block cycle (mean four), while the retained placed pair-owner fallback executes `2`, `6` and `8` for the steady cycle after the opening leaf — `12 → 16` at unchanged cycle ProofSize with a measured runtime-bound RefTime premium per additional candidate — and never exceeds `MaxCrossingTransitionsPerBlock`.

The executed ceiling is grant-bound rather than counter-bound: one admission reserves worker base, common probe, selection owner, placed owner and fault reserve, and the aggregate owner is reached only where the rotated family grant covers its `608,478` ProofSize reservation plus classification probes (`actors/EXP-0104`, `actors/EXP-0110`). On that executed counter cap, 100 eligible blocks materialize at most 800 candidates, and the implementation does not satisfy the historical performance-assurance 10,000-User/100-block reference profiles; System schedule throughput and broad ObservationChange fanout do not substitute for the User reactive profiles.

`MaterializationFamilyCursor` rotates the block-start family as `0 → 1 → 2 → 0`. Family 0 is the retired pre-cutover overdue-wakeup slot: fresh-genesis canonical publication never populates the paged `WakeupCursor*` substrate, so `materialization_family_has_work` reports no family-0 work and family 0 returns zero Weight, while its reserved maximum-unit quantum flows back into the shared lendable remainder. Families 1 (Crossing) and 2 (broad fanout) remain the only serviceable families.

`MaterializationMinimumReservation::{ReserveAllFamilies, Unavailable}` records whether one minimum quantum can be protected for every family; budget calculation no longer receives an unlabeled boolean. The coordinator services every family in rotated order, may service only the rotated first family once more through the bounded lending pass, preserves each worker's internal deadline/revision order and counters across grants, advances the cursor before the execution cutoff, and continues materialization while the global breaker suppresses Actor execution. The sum of the three configured ceilings is the single canonical materialization envelope. Each family also has one canonical maximum-unit minimum derived from its generated branch Weight, and production integrity requires all three minima to fit together with a positive two-dimensional lendable remainder.
...
A permanent Crossing materialization error rolls back the attempted unit, stores one bounded current fault with feed, revision, threshold, and typed class, and halts later Crossing service after base admission instead of retrying unchanged corrupt state every block. Observation fanout applies the same contract with feed, latest revision, available subscriber-page cursor, and class. The retained pre-cutover wakeup worker records its temporal key, page cursor, and class after rolling back the unit, then stops at the next charged cursor probe. Fault recording remains exposed through bounded pallet projection. Their `GlobalBreakerOrigin` clear calls remove exactly the current record, emit its identity, and permit the preserved obligation to resume; callers repair the underlying invariant before clearing or the deterministic worker records the fault again.
...
Active control has one physical owner: an `ActorControlCell` containing identity, hot lifecycle state, admission-derived Pipeline service identity, current cursor and eligibility service projection, admission, and the current Step's control/effect resource envelope. The Pipeline identity binds membership to the paid admission generation; canonical Run state remains the process owner for an open Cycle, and primary loading rejects stale membership before service. `ActorControlLocation` identifies its Unsignaled, Ready, or Waiting placement. Public projections and lifecycle operations resolve that primary; `ActorIdentities` contains dormant identities only. Activation transfers dormant identity into the primary, deactivation restores its latest identity, and close leaves sovereign custody untouched.
...
`ActorObservationFeeds` stores each active actor's canonical source-derived feed set. TryRuntime compares that complete set with the Contract Trigger through the admission derivation helper; internally consistent missing, foreign or phantom subscriptions are invalid. ObservationChange latching retains membership and disables detection through its existing marker. A reusable dense slot has exact `ObservationSubscriptionSlot` and reverse-owner entries. `ObservationFreeSlotPages` provides one-page LIFO allocation bounded by `MaxActiveActors`, so identity churn reuses slots rather than growing consensus keys.
...
`ObservationSubscriberCount` and `ObservationSubscriptionCount` expose bounded current cardinality. Try-state reconciles actor/feed ownership, free slots, page cells, reciprocal occupied links, list bounds, and counts; a count-bounded head walk rejects disconnected pages and cycles. Owned and free slots must lie below the allocation frontier; free-page lengths must match their exact LIFO stack positions, not just the aggregate free count. Fanout initializes from the feed-local head and follows exact page links, so an isolated subscriber in a historically high global slot costs one occupied-page unit rather than every lower page id.
...
- `ActorSemanticStates`: canonical dormant-or-active semantic and Contract-generation records; lifecycle writers publish, checked-increment, preserve, replace and remove them transactionally. Legacy loading accepts only an exact independently derived physical partition; the staged canonical Service seam accepts only a matching active semantic record, serving process and ring node after legacy authority is absent, and writes Hot directly by whole-record compare-and-replace. Active records expose the nonzero generation-bound `ActorRef` required by process carriers
...
- `ActorProcesses`: generation-bound process record; canonical creation publishes it and canonical Contract replacement rotates it together with the semantic generation after releasing the old residence. Its transaction-required helper rejects legacy physical scheduler coexistence, stale-current replacement and failed planning
- `ServiceHeader` / `ServiceNodes`: inert persistent-ring header and actor-keyed nodes; one composed transaction owner publishes the process and inserts into an empty or populated ring atomically, rolling both surfaces back on insertion failure. Transaction-local append/remove separately verifies matching process residence, generation, local links, singleton/interior/cursor structure and legacy-authority exclusion; canonical creation, lifecycle republishing and Contract replacement reach these owners in production, while sole-drain legacy-FIFO retirement remains staged
- `DeadlineHeaders` / `DeadlinePages` / `DeadlineHandles`: key-addressed bucket headers, retained linked C32 pages, and exact Actor-generation reverse handles published by canonical creation, lifecycle republishing and Contract replacement; transaction-local insert/remove/move preflights destination capacity, validates all three ownership surfaces, and restores source authority on failure
- `DependencyScanSourceListState` / `DependencyScanSourceNodes`: inert bounded circular source selector for retained dependency scans; transaction-local insertion is idempotent, arbitrary completed-source removal preserves the fair cursor, and malformed links, inactive scans, or capacity refusal leave prior authority unchanged. The inert combined publication boundary retains even an empty fixed-horizon scan, preserves one membership while revisions coalesce, and rolls revision plus carrier mutation back together. Production Oracle publication does not call that boundary before its begun/coalesced carrier effects receive generated composed Weight; scan consumption additionally requires generated probe/member/completion/fault Weight and fourth-family integration.
...
- `ActorWaitingHeads` / `ActorWaitingTails` / `ActorWaitingOccupancies` / `ActorWaitingCursorIndices`: exact per-deadline directory for linked nonempty C32 pages. `WakeupCursorPages` / `WakeupCursorLen` retain the shared Block/Tick heaps; `NextWakeupClock` arbitrates simultaneously due domains. This pre-cutover waiting substrate remains as inert topology: fresh-genesis canonical publication writes none of it, and the retired family-0 coordinator slot no longer drains it.
...
Full active-state classification additionally validates each declared Block/Tick slot against the primary's actor and admission identity, including exact locator coordinates when the slot owns the primary. It reads at most two temporal slots and never scans a population; hot head admission remains separate. TryRuntime reconciles the primary's inline cursor and resource envelope with canonical Run and current-Step authority, including zero-Step control cost. Reverse audits reject persisted tail keys outside their Contract head's declared geometry and activation-authority keys without a Contract head owner.
