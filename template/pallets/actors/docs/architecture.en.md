...
Funding accumulators and Run funding snapshots contain positive amounts only: Opening copies certified accumulation and keeps it immutable. TryRuntime checks this value constraint as well as tracked-key membership; a coherent payload commitment alone does not establish reachable funding state. Opening balance snapshots independently retain zero balances for admitted surfaces.
