...
- `DependencyScanSourceListState` / `DependencyScanSourceNodes`: inert bounded circular source selector for retained dependency scans; transaction-local insertion is idempotent, arbitrary completed-source removal preserves the fair cursor, and malformed links, inactive scans, or capacity refusal leave prior authority unchanged. Production dependency publication and materialization do not reach it before generated Weight and fourth-family integration close together.
