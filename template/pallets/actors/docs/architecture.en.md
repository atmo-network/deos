...
- `DeadlineHeaders` / `DeadlinePages` / `DeadlineHandles`: inert key-addressed bucket headers, retained linked C32 pages, and exact Actor-generation reverse handles; transaction-local insert/remove/move preflights destination capacity, validates all three ownership surfaces, and restores source authority on failure
