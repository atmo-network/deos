...
Production `service_live_queue_entry` dispatches the consumed-frame zero-Step owner and routes StopCycle/effectful attempts through `execute_current_step_and_place`, shared with inner benchmarks. For an effectful Step, `execute_effectful_step_transition` produces the carrier-neutral semantic result from `execute_loaded_single_step_core`; `finalize_effectful_step_on_legacy_fifo` alone closes or republishes legacy FIFO/wakeup residence. Empty Contracts follow the same boundary: `execute_zero_step_transition` completes the semantic cycle, then `finalize_zero_step_on_legacy_fifo` owns legacy close or republication. Rollback simulation uses these same owners for one selected Ready Actor, without consuming earlier peers or advancing a virtual block.
