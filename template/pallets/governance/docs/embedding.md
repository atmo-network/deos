...
- `ProposalOpeningFee` and `ProposalFeeRecipient` must use a deposit-capable host account and preserve fee transfer atomicity with proposal admission.
...
- Enable try-state in upgrade and release validation. It reconciles bounded proposal indexes, epoch buckets and phase ownership, reward windows, retained outcomes, locks, and host-reported aggregate custody.
