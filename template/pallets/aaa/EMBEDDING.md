# `pallet-aaa` Runtime Embedding Guide

> Package-owned contract for reusing `pallet-aaa` outside the current DEOS/TMCTOL reference runtime.
...
Use this guide with the normative [AAA Specification](https://github.com/atmo-network/deos/blob/main/docs/aaa.specification.en.md) and the shipped [DEOS AAA Architecture](https://github.com/atmo-network/deos/blob/main/docs/aaa.architecture.en.md). The specification defines portable semantics; this package document defines the host contract; the architecture document describes the current DEOS reference binding.
