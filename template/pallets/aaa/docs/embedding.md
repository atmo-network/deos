# `pallet-deos-aaa` Runtime Embedding Guide

> Package-owned contract for reusing `pallet-deos-aaa` outside the current DEOS/TMCTOL reference runtime.
...
`pallet-deos-aaa` is a bounded deterministic actor kernel. A downstream runtime can embed it without inheriting DEOS governance policy, TMCTOL bucket topology, native staking design, or the current System AAA catalog.

Use this guide with the normative [AAA Specification](./specification.en.md) and shipped [DEOS AAA Architecture](./architecture.en.md). The specification defines portable semantics; this package document defines the host contract; the architecture document describes the current DEOS reference binding.
...
- Governance/system origins, a two-dimensional hook weight meter, runtime-reserved `AaaOnIdleReserve`, owner-slot/queue/wakeup/active/total-identity/sweep bounds, `MaxOpeningSnapshotEntries`, `MaxIdleStarvationBlocks`, `MaxCacheRevalidationUnitsPerBlock`, and fee constants.
- Cache identity: `CurrentCacheEpoch`, the per-actor `cache_epoch` stamp, and the optional durable `CacheRevalidation` progress gate. `CacheRevalidationNoAdmitDisposition` names the migration-specific disposition for actors that no longer admit; `None` rejects activation of a cache-affecting runtime change, so pre-launch hosts ship `None` until a concrete migration names its disposition.
- Canonical FIFO configuration: non-zero page size, bounded `MaxQueueLength`, `MaxQueueEntriesScannedPerBlock`, and `MaxExecutionsPerBlock`. One `NextQueueTicket`, one cutoff, one actor-local ticket, one scheduler, one wakeup substrate, and one Continuation owner govern every actor. Actor class, AAA id, execution share, and priority policy never change service order.
...
- `Runtime owns`: Asset ledgers, caller-aware DEX pricing, liquidity pool and donation policy, staking topology/share mapping, atomic fee-collection policy and sink depositability, ingress producers, governance origins, genesis System AAA definitions, and task weight calibration.
...
Business policy belongs in runtime adapters or genesis actor configuration, not in `pallet_aaa` core.
