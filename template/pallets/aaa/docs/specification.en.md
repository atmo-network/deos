...
- User creation pays only on committed creation; User executable steps are attempt-priced; System execution is AAA-fee-exempt.
- AAA has no recurring rent, task-fee refund, implicit balance discovery, or background solvency scan.

### 1.1 Terms

| Term | Contract |
| --- | --- |
| Rejected control transition | The AAA call returns `Err`; AAA state/events, subscriptions, scheduler state, and creation-fee movement equal pre-state. Host fee/nonce accounting is excluded. |
...
AAA defines no ownership transfer, arbitrary dispatch, loops, same-block actor-graph continuity, priority lane, actual-Weight refund, readiness-bypassing simulation, signal-payload amount resolution, adaptive rent/deposit pricing, generic migration engine, close-time asset enumeration, implicit multi-asset refund, or Router route semantics. Uncertified balance movements are not retroactively interpreted as AAA ingress.
...
struct OutcomeTotals {
  ...
  skipped_conditions: u32, skipped_resolution: u32,
...
Economic apoptosis applies to Mutable and Immutable User actors. Only scheduler classification and permissionless sweep evaluate its predicates; AAA performs no background solvency scan.
...
- `create_system_aaa` allocates a fresh locator with `system_sovereign_id = aaa_id`, records `Occupied(aaa_id)`, and consumes lifetime locator capacity.
- `create_system_aaa_at_sovereign_id` requires an existing `Vacant` locator, assigns a fresh `aaa_id`, and records `Occupied(fresh_aaa_id)`; unknown and occupied locator cases are distinct errors.
...
- `SovereignIndex` covers current Active or Dormant ownership only. Nonzero balance or host provider state at an otherwise unindexed derived account is custody, not a live AAA collision, and MUST NOT block creation or reattachment.
...
- Every AAA-declared ordinary debit other than a Section 3.4 terminal custody drain obeys the protected minimum in Section 3.3.
...
- After close, no AAA authority exists over preserved balances until successful reattachment; knowledge of the deterministic account alone grants no control.
...
required_attempt_fee =
  attempt_fee_envelope(execution_plan, cursor, User).total
```
...
- A `RetryLater` suspension candidate increments `unsuccessful_attempts_at_cursor` and `consecutive_failures` exactly once before terminal-bound evaluation; it commits `CycleSuspended` only when neither bound is reached.
- A non-retry terminal `Failed` attempt increments `consecutive_failures` exactly once.
...
- Fresh Active installation and semantic execution-plan or completion-policy replacement reset the global streak.
...
`activate_aaa`, `deactivate_aaa`, semantic `pause_aaa`, semantic `resume_aaa`, semantic `update_schedule`, semantic `update_execution_plan`, semantic `update_funding_source_policy`, and `cancel_continuation` are limited to one committed call per actor per block. Exact no-op returns before checking or updating the clock. `manual_trigger`, auto-close target calls, explicit close, sweep, global controls, ingress, and internal scheduler/cleanup transitions are exempt. Expiry-substituted close does not update a clock it immediately deletes. Rejection uses `ControlMutationRateLimited`.
...
`Immediate` has no cadence phase or timer jitter. Its temporal eligibility uses only cooldown and window arithmetic. `phase_window`, `worst_case_phase`, `cadence_phase`, `cadence_origin`, and `cadence_at_or_after` are defined only for `Cadenced`.
...
if let TriggerPolicy::Cadenced { every_blocks, .. } = schedule.trigger {
  ...
  let phase_window =
    min(every_blocks / 4, MaxTimerJitterBlocks);

  let worst_case_phase =
    phase_window.saturating_sub(1);

  let composed_delay =
    checked_add(every_blocks, worst_case_phase)
      .ok_or(Error::SchedulerIndexExhausted)?;

  ensure!(
    composed_delay <= MaxExecutionDelayBlocks,
    Error::ExecutionDelayTooLong
  );
}
```
...
fn cadence_at_or_after(
  ...
  let cadence_phase = if phase_window == 0 {
    0
  } else {
    u64_le(Blake2_256(SCALE(aaa_id))[0..8])
      % u64::from(phase_window)
  };

  let cadence_origin =
    checked_add(wide(schedule_anchor), wide(cadence_phase))?;

  if wide(lower) <= cadence_origin {
    return checked_narrow(cadence_origin);
  }

  let delta =
    checked_sub(wide(lower), cadence_origin)?;
...
Missed cadence points coalesce; they do not create catch-up cycles. Jitter shifts phase once and does not change the cadence period.
...
| SwapOut | credit exactly the resolved output; AAA supplies one finite input-cap ceiling and `DexOps` owns executable quote/tolerance cap |
...
A plan is a terminal custody drain exactly when all conditions hold:
...
- If `terminal_drain_balance == 0`, the sole step follows ordinary zero-resolution semantics: it emits `StepSkipped(ResolutionSkipped)`, increments `skipped_resolution`, completes the cycle with no committed effectful task, does not close with `ProductiveCycleCompleted`, and retains the actor under ordinary post-cycle placement. AAA defines no empty-drain close reason.

No other plan may debit `protected_minimum`. One terminal custody drain names one asset. Recovering multiple known assets requires repeated successful reattachment and terminal-drain cycles; AAA performs no custody asset scan.
...
| Step result | Policy / bound state | Step event | `OutcomeTotals` delta | Transition and boundary event |
| --- | --- | --- | --- | --- |
| conditions false | any | `StepSkipped(ConditionsNotMet)` | `skipped_conditions += 1` | advance |
| resolution `Skipped` | any | `StepSkipped(ResolutionSkipped)` | `skipped_resolution += 1` | advance |
| `FundingUnavailable` | `ContinueNextStep` or `AbortCycle` | `StepSkipped(FundingUnavailable)` | `skipped_funding_unavailable += 1` | advance |
| `FundingUnavailable` | `RetryLater`, neither unsuccessful bound reached | none | none | remain at cursor; apply one local/global unsuccessful increment; emit `CycleSuspended(FundingUnavailable)` |
| `FundingUnavailable` | `RetryLater`, a local/global unsuccessful bound reached | none | none | finalize `Failed`; emit `CycleSummary(Failed)`, then close with the Section 2.4 bound reason |
| successful effectful task | any | the task success event | `executed_steps += 1`; `committed_effectful_tasks += 1` | advance; at plan end finalize `Completed` |
| successful `StopCycle` | any | `CycleStopped` | `executed_steps += 1` | finalize `Completed` immediately |
| Temporary task failure | `ContinueNextStep` | `StepFailed(Temporary)` | `failed_steps += 1` | advance |
| Temporary task failure | `AbortCycle` | `StepFailed(Temporary)` | `failed_steps += 1` | apply one global unsuccessful increment; finalize `Failed`; emit `CycleSummary(Failed)` and close with `ConsecutiveFailures` when that bound is reached |
| Temporary task failure | `RetryLater`, neither unsuccessful bound reached | `StepFailed(Temporary)` | `failed_steps += 1` | remain at cursor; apply one local/global unsuccessful increment; emit `CycleSuspended(Temporary)` |
| Temporary task failure | `RetryLater`, a local/global unsuccessful bound reached | `StepFailed(Temporary)` | `failed_steps += 1` | finalize `Failed`; emit `CycleSummary(Failed)`, then close with the Section 2.4 bound reason |
| Permanent task failure | `ContinueNextStep` | `StepFailed(Permanent)` | `failed_steps += 1` | advance |
| Permanent task failure | `AbortCycle` or `RetryLater` | `StepFailed(Permanent)` | `failed_steps += 1` | apply one global unsuccessful increment; finalize `Failed`; emit `CycleSummary(Failed)` and close with `ConsecutiveFailures` when that bound is reached |
...
Quote, tolerance arithmetic, System guard, Router call, actual validation, paired ingress, rollback, and success event belong to one task Weight class and one task transaction. AAA exposes no competing quote surface.
...
New cycles start at cursor `0`. Retries start at the Continuation cursor. `executions(aaa_id, block) <= 1`.
...
The full step envelope leaves reservation exactly once regardless of condition, resolution, failure class, or charged amount. The decrement occurs after that step's conditions/resolution and before any later step's conditions/resolution.
...
User creation charges `AaaCreationFee` to the signed caller/owner inside the control transaction. The sovereign account never pays the creation fee. Inability to collect returns `InsufficientFee`; rejected creation restores every fee effect. System creation, System attempts, and terminal cleanup charge no AAA execution fee. Task-defined asset effects and Router fees still apply.
...
AAA uses explicit storage transactions; FRAME dispatch failure alone does not provide the required rollback semantics.
...
A classification or structural transactional invariant failure at a live head yields `InvariantStalledLiveHead`, preserves the head, and performs no actor mutation. AAA defines no call-level quarantine, demotion, reticketing, skip, or forced cleanup for that head; persistent structural repair is available only through Section 9.4. Fee-collection failure yields `FeeCollectionStalledLiveHead`, preserves the head, and retries only through later ordinary actor service. A starved actor-service pass is `WeightBlockedLiveHead`, `FeeCollectionStalledLiveHead`, or `InvariantStalledLiveHead` with no committed attempt. Starved passes saturating-increment `consecutive_blocks`; `PassExhausted` does not. Weight/pass deferral changes no actor state or event. Liveness requires recurring conforming budget, finite stale churn, and eventual placement capacity.
...
Only a certified producer creates AAA AddressEvent semantics. The generated certified-producer inventory is the sole producer authority: it names every runtime movement path that claims AAA ingress and records provenance, source availability, atomic rollback owner, preflight/notify integration, retry mapping, and Weight bound.

AAA Transfer, every SplitTransfer leg, and Mint are certified producers whenever their destination resolves to an AAA sovereign account.
...
Preflight is read-only and covers lifecycle, funding, trigger, and required placement. Notify executes exactly once after movement. Any failure restores movement and every AAA effect. Certification is atomic, not advisory: after preflight, a certified movement MUST NOT fall back to balance-only when its complete AAA consequence cannot commit.

A certified third-party movement to an Active actor MAY fail when its complete AAA consequence cannot commit. Recoverable queue/wakeup capacity or placement unavailability is Temporary. Monotonic ticket/index exhaustion, topology corruption, invalid provenance, and invariant failure are Permanent. AAA tasks preserve the classification through `TaskFailure`; non-AAA producers map it to their outer dispatch error.

A movement not named in the certified inventory is **balance-only**: it cannot latch readiness, update `funding_accumulated`, or emit `FundingAccumulated`, even when destination is an AAA sovereign account. It is not rejected by AAA scheduler pressure. No event scan, balance-diff scan, inbox, or implicit producer discovery is permitted.
...
- Fee movement belongs only to one certified `FeeCollector` path and notifies exactly once. If `FeeSink` resolves to an Active AAA sovereign account, trigger matching and funding acceptance are evaluated independently under that actor's authored schedule/funding policy and the inventory-defined source/provenance. A funding-rejected fee credit remains balance-only for funding but MAY still latch readiness when its trigger matches. Failure of the complete FeeSink consequence rolls back the fee movement and the enclosing payer attempt under Section 4.3. No second ordinary AddressEvent notification is permitted.

### 5.4 Hooks, Breaker, and Reserved Actor Service

`on_initialize` performs no AAA work and returns zero AAA Weight.

`AaaOnIdleReserve` is an embedding guarantee: immediately before AAA `on_idle` in every conforming block, both remaining Weight dimensions are at least this value. Runtime MUST enforce the guarantee through block dispatch limits, mandatory-hook maxima, and hook order. AAA meters itself against `min(actual_remaining, AaaOnIdleReserve)` and runs before lower-priority idle consumers.
...
```rust
struct ExecutionContext<'a, A> { actor: &'a A, aaa_type: AaaType }
...
trait FundingAuthority<A> {
  fn permits(aaa_id: AaaId, owner: &A, source: Option<&A>,
...
Liquidity success uses positive amounts within supplied caps/minima and no undeclared debit. For DonateLiquidity, AAA derives `max_b`; `LiquidityOps` may choose actual amounts within both caps but MUST NOT derive or exceed another cap.
...
AAA supplies resolved intent, class, finite authored/custody caps, tolerance, and failure policy. `DexOps` owns current quote acquisition, quote protection, the AAA-specific System reference guard, Router invocation, and actual validation. Router owns route discovery, path validation, Router fees/protection, Oracle publication, execution, and route outcome. AAA neither defines nor alters route semantics.
...
```text
cycle_weight_upper =
  derive_weight(actor_class, execution_plan, current Weight/adapter bindings)

cycle_fee_upper =
  derive_fee(actor_class, execution_plan, current WeightToFee bindings)

suffix_weight_upper(cursor) =
  derive_weight(actor_class, execution_plan[cursor..], current bindings)

suffix_fee_upper(cursor) =
  derive_fee(actor_class, execution_plan[cursor..], current bindings)
```

No attempt may admit, reserve, charge, simulate, or report from stored derived Weight or fee state.

A generated Weight or adapter change that would make a previously admitted semantic Weight class exceed its owning execution envelope MUST preserve that class's admission or apply Section 9.4 before ordinary execution resumes. Compatibility is determined from the complete finite semantic Weight-class domain, not by unbounded actor iteration. AAA defines no permanent generic cache-revalidation state.

`WeightToFee`, `MinUserBalance`, and fee changes induced by an admission-compatible `WeightInfo` update are economic-policy changes. They require no actor iteration and apply immediately to every User actor, including Immutable actors, unless a Section 9.4 migration defines a grace, subsidy, or alternate disposition. Any resulting close transitions with `BalanceExhausted` or `FeeBudgetExhausted`, including mass economic apoptosis, are canonical. System actors remain AAA-fee-exempt.
...
User Dormant and Active creation charge `AaaCreationFee` to the signed caller/owner.
...
```text
ActiveAaaCount <= limit <= min(MaxActiveActors, MaxQueueLength)
```
...
1. The same signed owner calls `create_user_aaa_at_slot` for the released exact slot, creating a fresh Mutable Dormant or Active actor;
...
Recovery of a non-fee asset requires prior funding of the same deterministic sovereign account with sufficient `FeeNativeAssetId` for Active admission and the current attempt. AAA provides no sponsored recovery path.
...
enum SimulationStatus { Completed, Failed, Suspended, Closed(CloseReason) }
enum SimulationStepOutcome {
  Executed, Skipped(StepSkippedReason), Failed(RetryClass),
  FundingUnavailable, Stopped,
}
...
struct SimulationResult {
  status: SimulationStatus, cycle_nonce: u64, attempt: u32, start_cursor: u32,
...
trait AaaSimulationApi<Program> {
  fn simulate_current_program(
    aaa_id: AaaId, expected_type: AaaType, expected_mutability: Mutability,
    expected_program: Program, mode: SimulationMode,
  ) -> Result<SimulationResult, SimulationError>;
}
```

One call simulates exactly one fresh or Continuation attempt; `steps.len() <= execution_plan.len() - start_cursor <= MaxExecutionPlanSteps`.
...
Simulation step records project Section 3.4 exactly:

| Production step transition | `SimulationStepOutcome` |
| --- | --- |
| successful effectful task | `Executed` |
| successful `StopCycle` | `Stopped` |
| conditions/resolution/funding skip that advances | `Skipped(reason)` |
| `FundingUnavailable` selected for `RetryLater`, whether it suspends or reaches a bound | `FundingUnavailable` |
| task failure, whether it advances, suspends, or terminates | `Failed(retry_class)` |

The enclosing `SimulationStatus` distinguishes suspension, terminal failure, and close. The simulated `cumulative_outcomes` applies the same row delta as production, including `failed_steps += 1` for a Temporary task failure whose attempt status is `Suspended`.
...
| Case | `cycle_nonce` | `attempt` | `start_cursor` |
| --- | --- | --- | --- |
| fresh attempt | `stored_cycle_nonce + 1` | `0` | `0` |
| Continuation attempt | stored cycle nonce | current attempt + 1 | stored cursor |
| pre-attempt close | stored cycle nonce | current stored attempt or `0` | stored cursor or `0` |
...
```rust
enum AaaEligibilityPhase {
  NotRegistered, Dormant, Ready, GlobalCircuitBreaker, CloseDue(CloseReason),
  Paused, WaitingSignal, WaitingRetry, WaitingTemporal,
}
struct AaaEligibilityProjection<BlockNumber> {
  phase: AaaEligibilityPhase,
  next_eligible_block: Option<BlockNumber>,
}
enum AaaEligibilityError { ActorInvariant, ContinuationInvariant, ComputationOverflow }
trait AaaEligibilityApi<BlockNumber> {
  fn aaa_eligibility(aaa_id: AaaId)
    -> Result<AaaEligibilityProjection<BlockNumber>, AaaEligibilityError>;
}
```
...
AaaPaused { aaa_id: AaaId }
AaaResumed { aaa_id: AaaId }
AaaClosed { aaa_id: AaaId, reason: CloseReason }

CycleStarted { aaa_id: AaaId, cycle_nonce: u64 }
CycleSummary { aaa_id: AaaId, cycle_nonce: u64, result: CycleResult, outcomes: OutcomeTotals }
...
Event durability follows Section 4.4. `ManualTriggerSet` emits only on latch `false -> true`. `FundingAccumulated` emits only for tracked policy-accepted credit. Cycle key is `(aaa_id, cycle_nonce)`; attempt adds `attempt`.

Active creation emits exactly one `AaaCreated { initial_lifecycle: Active }` and does not emit `AaaActivated`. `AaaActivated` belongs only to Dormant -> Active transition of an existing identity.
...
- `FundingUnavailable` selected for `RetryLater` emits no `StepSkipped`; it emits `CycleSuspended(FundingUnavailable)`, or `CycleSummary(Failed)` then `AaaClosed(bound_reason)` when an unsuccessful bound is reached;
- Temporary task failure selected for `RetryLater` emits `StepFailed`, then `CycleSuspended(Temporary)`, or `CycleSummary(Failed)` then `AaaClosed(bound_reason)` when an unsuccessful bound is reached;
- Terminal task failure emits `StepFailed`, then `CycleSummary(Failed)`, then any failure-driven `AaaClosed`;
- Successful completion emits `CycleSummary(Completed)`, then any productive or auto-close `AaaClosed`;
- A successful terminal custody drain emits `CycleStarted`, `TransferExecuted`, `CycleSummary(Completed)`, then `AaaClosed(ProductiveCycleCompleted)`;
...
`CycleSuspended` and `CycleSummary` expose cumulative outcomes after the current step delta. Cancellation emits `CycleCancelled`, then `CycleSummary(Cancelled)`. If caused by close, `AaaClosed` follows. If caused by semantic update or deactivation, the corresponding event follows. Pure close emits only `AaaClosed`. Weight/pass deferral emits nothing. Unbounded history is off-chain.
...
For this `pre-1.0.0` revision, metadata MUST expose only canonical shapes; reserved compatibility indices, stale aliases, dual names, and compatibility storage are forbidden. Canonical terminology is `Cycle`, `CloseAfterProductiveCycle`, and `ProductiveCycleCompleted`.
...
```rust
enum Error {
  AaaIdOverflow, AaaNotFound, ActiveAaaCapacityExceeded, ActiveAaaCountInvariant,
  ActorIdentityCapacityExceeded, ActorIdentityCountInvariant, ActorInvariant,
  AaaAlreadyActive, AaaDormant,
  ActiveAaaLimitExceedsQueueCapacity, ActiveAaaLimitTooHigh,
  ActiveAaaLimitTooLow, ActiveAaaLimitBelowCurrent,
  AaaPaused, EmptyExecutionPlan, ExecutionPlanExceedsOnIdleBudget,
  ExecutionDelayTooLong, GlobalCircuitBreakerActive, ImmutableAaa,
  ...
  SelfTransferNotAllowed, MintNotAllowedForUserAaa,
  NotGovernance, NotOwner,
  OwnerSlotCapacityExceeded, OwnerSlotOccupied, InvalidOwnerSlot, AaaIdOccupied,
  ...
  ExecutionPlanTooLong, SnapshotUnavailable, FundingAccumulatorOverflow,
  ...
  RetryLaterNotAllowedForImmutableAaa, ContinuationNotFound, ContinuationInvariant,
...
| `ActorClassificationError` | Dispatch `Error` | `SimulationError` | `AaaEligibilityError` |
| --- | --- | --- | --- |
| `ActorInvariant` | `ActorInvariant` | `ActorInvariant` | `ActorInvariant` |
| `ContinuationInvariant` | `ContinuationInvariant` | `ContinuationInvariant` | `ContinuationInvariant` |
| `ComputationOverflow` | `ComputationOverflow` | `ComputationOverflow` | `ComputationOverflow` |

Every dispatch path that invokes classification MUST preserve this mapping. `permissionless_sweep_many` rolls back its complete transaction on a classification error. Certified ingress carries the corresponding pallet `Error` inside Permanent `IngressFailure`. A classification error MUST NOT map to `AaaNotFound`, `AaaDormant`, `NotReady`, `SchedulerIndexExhausted`, `AdmissionBoundOverflow`, or a waiting phase.
...
Migration state is specific to the concrete upgrade and MUST be absent or inert after completion. AAA defines no permanent generic migration subsystem.

## 10. Runtime Configuration

### 10.1 Relations

Required bindings include `AaaPalletId`, `FeeNativeAssetId`, `SystemOrigin`, `GlobalBreakerOrigin`, adapters/services, `FeeSink`, `AaaCreationFee`, `WeightToFee`, one `WeightInfo`, `AaaOnIdleReserve`, generated base/cleanup bounds, `MaxSweepBatch`, independent queue/wakeup/observation processing bounds, System reference parameters, and `TargetBlockTime`.
...
15. Runtime guarantees `remaining_weight_at_AAA_on_idle >= AaaOnIdleReserve` in both dimensions.
...
25. `MaxIdleStarvationBlocks > 0`; `MaxTimerJitterBlocks` MAY be zero.
26. Simulation records are bounded by the same `MaxExecutionPlanSteps`.
27. Every loop and storage bound appears in metadata or generated descriptors.
28. `MaxSplitTransferLegs >= 2`.
29. No Config binding or storage item exists for `StepBaseFee`, `ConditionReadFee`, stored derived Weight/fee bounds, cache epochs, or generic cache revalidation.
...
`QueuePageSize`, `WakeupPageSize`, `ObservationPageSize`, `MaxQueueEntriesScannedPerBlock`, `MaxExecutionsPerBlock`, `MaxWakeupsPerBlock`, `MaxObservationFanoutPagesPerBlock`, `WakeupWeightLimit`, `ObservationFanoutWeightLimit`, `AaaOnIdleReserve`, base/cleanup bounds, and adapter worst cases are measured runtime outputs.

Generated runtime and Weight descriptors define the values. Count ceilings do not guarantee throughput.

## 11. Conformance

A runtime conforms to this specification iff all conditions hold:

1. AAA runtime metadata exposes only the calls, public types, events, errors, bounds, and runtime APIs defined here.
...
8. Certified producer and publisher inventories contain every path that claims AAA ingress or observation authority; paths outside them have no AAA semantic effect.
