...
- `Architecture / Experiment Proof Graph`: Added Leaf/Synthesis ownership, frozen obligations and separate proof graphs with globally unique IDs. Decomposed integrated Actor evidence into independently scoped outcome, resource, stop and proof owners without benchmark reruns or renumbering. Preserved C1/A0 decisions and exact extraction provenance in Git; retired the validated working manifest. Measured results have independent statuses; production and release acceptance remain open.
- `Actors / Control Integrity`: Closed G3 schema, cardinality and sole-owner reconciliation. TryRuntime rejects shifted Opening cursors, orphan cold records and invalid subscriptions; Crossing ingress rejects revision exhaustion atomically. Suspended retries preserve User liability, terminal precedence and the last attempt. AtTime Opening consumes retained-latch replacements without a second Trigger charge. Release acceptance remains open.
...
- `Actors / Production Binding`: Bound funded User completion to generated maximum-header zero/tail owners without overlap or new context. Regenerated Weight/Wasm and consumers, corrected Cadenced detector/backpressure authority, accepted G6–G8 evidence and froze C1. Published that four Crossing candidates per block cannot meet the 10,000-User/100-block target; full exact-tree validation and local release assurance pass without a fairness claim.
