...
- `Actors / Introspection Contract`: Defined bounded local cause, topology, host-effect, resource, System-role, API and materializer semantics with per-claim truth classes, finalized identities, explicit gaps and no new scheduler authority; retained the design outside the Wiki, with implementation deferred until after 0.7.25 Geometry Freeze.
- `Dependencies / Native Wasm Toolchain`: Updated Wasmtime and its coupled compiler family to the patched 36.0.14 line for RUSTSEC-2026-0269; refreshed dependency provenance against the September advisory database without adding an exception.
- `Validation / Provenance and Integration`: Bound Actors source identity to content and executable modes across all worktree states. Restored package-architecture audit routing without project dependencies on Skills. Runtime fixtures preserve monotonic time and distinguish production fee/security/voting policy from benchmark substitutions. Validated Actors corruption checks with try-runtime alone, production and benchmark runtime profiles, and independent embedding with DEX, TryRuntime and no-std.
- `Architecture / Experiment and Package Boundaries`: Froze C1 physical architecture in EXP-0025 and assigned Weight, reachable state, admission, heap, bindings and W0–W9 to EXP-0028–0033 with validated causal relations in one track entrypoint. Kept package identifiers and fee-vector paths generic, included test fixtures, and removed migration diaries, mandatory Skill references and fixed 32-Step claims. Production and release acceptance remain open.
- `Actors / Control Integrity`: Validated temporal references, primary slots, sealed anchors and reclamation. Trie evidence excludes discovery/future-head cold reads and Running/Idle payload on capacity refusal. Idle User checks native Pipeline liability before Funding/Run and reuses the result for insolvency closure. Preserved cancellation/readiness, retry-local and terminal precedence; Suspended User insolvency precedes effect deferral. Full pre-cold admission and production acceptance remain open.
- `Actors / Canonical Transitions`: Completed sole-owner Ready/Waiting, lifecycle and Run substitution; retained dormant, funding and cold Run owners. Removed duplicate reload paths and reused temporal authority within callback freshness boundaries. Corrected Waiting fallback's actual-Weight classification. Simulation shares the Step owner with explicit budgets and one-record rollback. Staking bootstrap uses bounded eligibility. Final-tree, artifact and integrated acceptance remain open.
- `Actors / Benchmark Boundaries`: Validated User Running holds/retries; measured funded 24/48 captures, independent-source Opening and C1 placement. Proved retry heap variation and upward/tail removal; downward bounds two sampled upward branches on one Wasm. Withdrew underbound maximum-Opening shortcuts. Collection stays prepaid Control with effect-only fees; resumed pricing uses retained funding count. A0 unchanged. Full envelopes, artifacts, admission constraints and integrated acceptance remain open.
