...
- [ ] **N6.2 / Minimal Necessary Physical Choices.** `EXP-0137` closes the design-selection exit with one implementation baseline: stable process placement, actor-keyed intrusive Live/Pending ring, exact revisioned Park lookup, retained C32 deadline pages plus paged heaps, and sealed exact-handle reclamation. Each choice has one concrete structural falsifier and bounded local refinement allowance; admit at most one alternate only when its falsifier survives one correction. Cost alone transfers to N6.5/N6.3 and cannot reopen the architecture. Implement and test the selected choices without ritual prototypes, node-local cache speculation or bitmap sweeps. **Exit:** Every selected operation is executable over its complete legal domain, or one named falsifier records the bounded correction/alternate disposition; one implementation remains and its claims separate removed work, admission, lifecycle cost and changed semantics.
