...
The first code prerequisite is complete: `plan_next_work_loaded` selects typed `Service(kind)`, exact-block `Wakeup` or `Disabled` process intent from current state, supplied Run authority, time and cutoff without touching storage. `plan_process_destination` pairs that intent with the active generation-bound `ActorRef`, preflights an absent/capacious structurally linked Service destination or the exact canonical Deadline slot, and constructs a complete valid `ActorProcess` plus its concrete insertion authority. `plan_actor_publication` composes that exclusive process residence with the independent optional Tick-trigger deadline, updated semantic Hot pointer and generated current-Step resources; one suspended Cadenced Actor is proven to own Block-process and Tick-trigger destinations simultaneously without mutation. Process/Park and Trigger deadlines now have distinct reverse owners. The composite preflight rejects zero/stale generation, semantic/Contract/admission disagreement, mismatched Run presence, stale generated current-Step resources and any partial canonical process, Service or deadline publication before entering the future transaction. Paused-without-window retains exact Owner/OwnerPaused basis; active-unlatched retains Protocol/Idle disablement rather than an invalid residence-less Serving process. The temporary `schedule_next_work_loaded` legacy adapter remains the sole production committer and preserves existing capacity rollback and public behavior; no canonical production committer was exposed. The remaining seam is to commit semantic Hot, process residence and optional Trigger membership transactionally, then consume that publisher while replacing every listed publication root and the mandatory consumer before removing the temporary adapter.
