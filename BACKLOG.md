...
| Deadline/Park/Pending mechanisms | **Decided and partially implemented/tested/resource-bound; not integrated end to end.** Canonical Service transfer derives generation/plan Park authority from the admitted current Step and rejects incoherent Run, Step or resource state before mutation. Exact positive events and due reviews consume their Pending authority only with a successful generation/plan/Park-bound wake. One concrete bounded Oracle-availability review reads the complete retained source plan once, chooses wake or re-arm, and preserves Pending/Park work on stale evidence, corrupt source mappings, revision races and occupied Service destination. Its one-source due worker now traverses the earliest due block bucket through the shared C32 deadline index under a benchmark-generated positive-wake/negative-rearm envelope. One bounded dispatcher classifies the earliest C32 member as a sleeping retry or Park timed review without parallel scans. Its generated Block selector (`25,353,000` ps / `4,078` ProofSize) precedes inspection; the selected retry extraction (`89,678,000` ps / `4,078` ProofSize) or existing review envelope is then admitted independently, so branch refusal consumes neither member. The independent Tick frontier now admits its generated selector (`26,261,000` ps / `4,078` ProofSize) before routing only the retained timed-Park-review semantics through that same complete review envelope; Block and Tick roots cannot consume one another, and Tick retry residence fails closed. One bounded Block-then-Tick mandatory-service composition pre-admits both complete branch maxima before either selector reads, then gives each frontier one attempt through the shared meter, so absence or refusal on one cannot starve the other. That pass now runs from real mandatory `on_idle` service under the shared Actor Control meter, derives one-step re-arm deadlines from the current Block/Tick clocks, fails closed when either next clock is unavailable, and settles only generated selector/worker Weight; hook/resource refusal preserves both roots. | Complete the remaining paid scan/check consumers and connect ordinary Step/Park publication through real execution round trips. |
