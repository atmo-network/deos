...
| --- | --- | --- | --- | --- |
| Canonical service V1 | One supported create/activate/progress/retry/complete path under only `ActorProcesses` plus `ServiceNodes` authority | N2.4, N3.5–N3.6, N5.1; the shared semantic/carrier round trace now closes partial-round and refusal order only | No generated carrier owners exist yet: create/publication, ring insert, round begin/probe/admit, current Step/retry, completion and unlink must be benchmarked before connection | Every edge is transactionally single-authority; legacy Ready/Waiting is unreachable for that profile |
