...
- [ ] **N5.1 / Old-Path Reachability and Removal.** Hard-cut the supported build from legacy placement, tickets, future-Cycle latches, historical Crossing duties, Opening snapshots and LastFunding accumulation. Preserve effect/custody/credit-authentication boundaries. A newly selected parked-credit adapter may reuse a verified producer boundary, but not its old scheduling authority. Inspect dispatch, genesis, hooks, simulation, recovery, embedding and feature builds; no compatibility reader or dormant fallback may recreate legacy service. The canonical B+1 cutover left most pre-cutover scheduler tests on legacy same-block timing: at the current tree the `pallet-deos-actors` lib suite reports 362 passed / 530 failed of `897` (5 ignored), with production-effect tests such as `tests/market_tasks.rs` needing canonical timing migration and legacy-substrate tests needing deletion rather than preservation. `tests/market_tasks` is now migrated to `42 passed / 3 failed`: 18 single- and multi-Step effect witnesses, the retry/deadline path, the four-Step dispatch witness, the liquidity adapter-outcome loop and the user swap-plus-transfer witness run on canonical B+1 Service timing. The remaining three are not timing bugs: `liquidity_tasks_reject_adapter_outcomes_outside_authored_bounds` and `split_transfer_rounding_skips_zero_distribution_and_allows_one_effective_leg` block on the retained-Idle revisit gap below, and `cadence_rearm_capacity_failure_rolls_back_pipeline_opening` still drives the legacy `WakeupCursorLen`/`drain_overdue_wakeups_cursor` capacity substrate and must be deleted or rebuilt once that substrate is removed. Migrate or delete them so ordinary-path tests agree before removal. **Exit:** V1/V2 and every promised release profile are served by one engine; absence assertions and ordinary-path tests agree.
