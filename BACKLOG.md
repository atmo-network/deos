...
>
> Next planned framework line: `DEOS 0.7.8 — Reactive Delivery and Feedback Analysis`.

## 0.7.8: Reactive Delivery and Feedback Analysis

> Release objective: turn the `0.7.7` bounded reactive mechanism into explicit end-to-end service, observability, configuration, and closed-loop analysis contracts without adding arbitrary callbacks, dynamic control flow, synchronous subscriber execution, or a second scheduler. Feedback analysis is accepted release scope, not a conditional possibility.

- [ ] `AAA Reactive Delivery Contract`: Define the complete observation-publication → dirty-ingress → deferred-fanout → queue/wakeup-admission → actor-attempt contract. State exact guarantees and exclusions for latest-state reconsideration, revision coalescing, at-most-once execution, freshness, pause/breaker behavior, and the boundary at which System/User service reservations begin. Derive production RefTime/ProofSize and block-count delivery envelopes for dense, sparse, fragmented, multi-feed, revision-race, and queue-saturated cases at the 10,000-actor bound; do not translate benchmark-host timing into an unconditional execution deadline.
- [ ] `AAA Reactive Topology / Slot-Grid Tax`: Measure and materially reduce delivery work caused by global or historical subscription-slot and dirty-slot high-water geometry, including empty subscriber pages and recycled-slot fragmentation. Compare evidence-backed alternatives such as compact per-feed occupied-page ownership, a bounded active-dirty queue, or separate empty-probe and dense-page weights, then select the smallest mechanism whose sparse-feed cost follows live/occupied topology rather than unrelated historical slots. Preserve bounded mutation, exact cleanup, slot reuse, try-state reconciliation, latest-revision restart semantics, the existing scheduler, and the prohibition on synchronous subscriber iteration.
- [ ] `AAA Feedback Analysis / Closed-Loop Model`: Add a deterministic static model over typed observation provenance, trigger sources, condition reads, task effects, actor-to-actor signal surfaces, shared assets, and future typed parameter actuators. Detect structural self-feedback and cross-actor feedback paths while distinguishing endogenous, exogenous, and unknown observations. Cover representative TMCTOL loops such as price → swap/liquidity action → reserve observation, fee or treasury policy → market behavior → price, and actor funding → downstream actor activation. Treat a detected loop as a review surface, not proof of instability; keep economic stability, probability, and causal strength explicit unknowns unless an identified model supplies them.
- [ ] `AAA Static Analysis / Reactive Findings`: Extend the manifest-authoritative analyzer with factual, unscored findings including `FreshnessWindowBelowEstimatedDeliveryEnvelope`, `EndogenousObservationFeedback`, `ReactiveSelfCycle`, `ReactiveCrossActorCycle`, `ThresholdChatterRisk`, `MissingHysteresisOrPersistence`, `HighGainActuation`, `CooldownFeedRateMismatch`, `SharedObservationActuatorContention`, and `SystemActorWithoutReactiveIngressPriority`. Bind every delivery-relative finding to an identified runtime/weight/cadence snapshot, preserve possibility-versus-proof language, and add a falsifiable reactive scenario corpus without introducing a second semantic model or consensus rule.
- [ ] `AAA Configuration IR / Tokenomic Genome`: Define a convenient format-neutral typed internal representation for an AA-actor configuration, treating structure rather than file extension as canonical. The IR must preserve actor type, mutability, completion policy, trigger/admission policy, typed observation feeds, funding policy, ordered linear steps, exact `ConditionSet`, task parameters, amount resolutions, and failure policy. Provide lossless syntax adapters for `.json`, `.toml`, and structured `.md`; raw document bytes, comments, presentation prose, and chosen serialization must not alter the lowered metadata-bound `ProgramInput` or existing `planId`. Require deterministic normalization, structural diagnostics/diff, cross-format semantic-equivalence fixtures, and `parse → IR → lower → decode` round trips. Keep this DNK-like tokenomic genome as a control-plane representation, not another runtime language.
- [ ] `Oracle → AAA Transactional Availability Contract and Inspector`: Formalize the atomic publication-hook boundary: hook weight ownership, capacity and invariant failures, Oracle/Router rollback propagation, subscriber-independent ingress, recovery expectations, and operator-visible failure states. Add an exact bounded dirty-age source rather than inferring age from unrelated observation timestamps. At one finalized state, expose feed-level latest/fanout revision, dirty age, dirty slot and scan cursor, next subscriber page, occupied/page span, estimated remaining fanout pages and blocks under identified production budgets, plus bounded selected-actor pending-signal, queue lane/ticket or wakeup state, and queue-admission status. Estimates must state assumptions and never claim an exact future execution block or enumerate all subscribers synchronously.
- [ ] `0.7.7 Documentation Drift Closure`: Correct the normative AAA specification header so its release focus and source basis describe the accepted `0.7.7` typed-observation/reactive line rather than the earlier intent/failure baseline. Add a narrow release audit that keeps specification line, release focus, source basis, package navigation, and current framework boundary synchronized without broad documentation churn.
