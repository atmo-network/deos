...
- [ ] **N2.4 / Loaded Transition Context and Economic Owners.** Complete the bounded validated Attempt context across persistence and residence choice. Preserve the consumed-frame boundary: public callbacks cannot mutate the current Actor while its canonical primary is absent, host-effect failure rolls back locally, and any later failure rolls back the whole source/effect/outcome/placement transaction. Reuse only structural authority that remains fresh inside that boundary; make future blocks read canonical current state. Liquidity adapter outcomes now fail closed against authored debit caps and output minima inside task-local rollback. Repeated same-block scheduler passes preserve the exact successor ticket and cannot commit a second Step; future-block service consumes canonical Run authority. A Ready Crossing-to-Manual owner update atomically removes detector membership, preserves the exact latched ticket, rebinds its Contract commitment, and executes only replacement authority. A future-window Waiting ObservationChange replacement preserves exact wake authority, moves subscription and activation authority to the replacement feed, ignores the removed feed, and later executes only the new feed's latch; late transactional failure restores the exact source root, Waiting admission, old subscription and activation authority, removes the destination subscription, and permits later old-feed service. Finish all remaining mandatory outcomes; a storage-free selector still consumes RefTime. **Exit:** One production execution path with rollback-safe reuse, complete resource/payment owners and no dependence on node-local cache survival.
