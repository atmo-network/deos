...
- [ ] **N2.1 / Minimal Canonical Continuation.** Implement one current Cycle, Actor/generation binding, cursor, bounded retry state and required outcomes under stable canonical ownership. Decouple process data from service membership. Start checks apply only while idle; an open Cycle follows its current Step regardless of changes to its old start condition. Build a minimal multi-Step and retry vertical slice before final carrier selection. Manual, AddressEvent, observation and temporal occurrences are already ignored before Trigger-fee admission while Running or Suspended; AddressEvent still accounts independently authorized funding, Observation Crossing still advances its detector to the re-arm phase, Cadenced still advances its recurrence deadline, and AtTime still consumes its one-shot deadline. Run authority now binds the exact admission-derived paid Pipeline service identity and rejects stale service state; remaining work must eliminate duplicated process/service ownership without weakening the current vertical slice. **Exit:** Current-balance programs progress across blocks with one continuation owner, atomic Steps and durable committed prefixes, independent of a second future-Cycle machine.

- [ ] **N2.2 / Live Amount and Predicate Evaluation.** Implement exactly `Fixed` and current-Available `Percent` across valid typed balance/share/output surfaces; `Percent(100%)` replaces `AllAvailable`. Specify reading points relative to fees, protected minima, ledger restrictions and Task effects. Use coherent current-read sets, checked/widened arithmetic and declared rounding; never silently shrink a fixed debit. Reevaluate on retry and after relevant mutations. Remove Opening/LastFunding consumers, snapshots and accumulation, not independent host custody/credit verification. Reject removed encoded forms rather than reinterpret them. **Exit:** Tests cover top-ups between Steps/retries, zero/dust, maximum amounts, shared-asset multi-field resolution and freshness; no hidden historical balance basis or unsafe spending shortcut.
