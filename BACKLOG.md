...
| --- | --- | --- | --- | --- |
| Canonical service V1 | Split identity, hot state, admission and cursor from legacy placement: create/activate currently publishes them only inside an Unsignaled control cell, removes scalar `ActorIdentities`, and every active loader reads that cell before initial scheduling; deleting legacy authority before service publication would therefore delete the sole semantic owner | N2.4, N3.5–N3.6, N5.1; the source-backed cutover blocker regression forbids production publication/retirement calls while this ownership remains, and atomic publication/retirement plus the semantic/carrier round trace cover inert carrier behavior | Generated composed extrema cover atomic process publication plus empty/populated insertion and singleton/pair-cursor/interior retirement; round begin/probe/admit remain standalone, while semantic-state publication, current Step/retry and completion are unbound | One supported profile transactionally publishes a non-placement semantic owner, removes its legacy cell/locator, then publishes `ActorProcesses` plus `ServiceNodes`; every loader and Weight is converted together with no dual-authority intermediate |
