...
- [ ] **N2.4 / Loaded Transition Context and Economic Owners.** Complete the bounded validated Attempt context across callbacks, host mutations, persistence and residence choice. Reuse only authority still fresh after each mutation boundary; cache structure rather than future values, and make future blocks read canonical current state. Commit only changed partitions without hiding tentative state from canonical adapters. Finish explicit freshness/reload reasons and price negative checks, scheduler turns, membership mutations and all remaining mandatory outcomes; a storage-free selector still consumes RefTime. **Exit:** One production execution path with explicit freshness/reload reasons, rollback-safe reuse, complete resource/payment owners and no dependence on node-local cache survival.
