...
| Canonical semantic/process/Service authority | **Implemented locally and production-reachable for Manual, AddressEvent, ObservationChange, AtTime and Cadenced publication and scalar ObservationCrossing fire/rearm/coalesced/placed fire plus homogeneous pair rearm/coalesced/placed fire and all placed-batch page shapes; generated owners exist for those occurrence boundaries; no complete end-to-end V1 proof.** | Convert remaining initial/resumed and post-attempt publication, then retire the FIFO consumer atomically. |
...
| Lifecycle and cleanup | **Decision accepted and partially implemented; neither production closure nor final generated-resource evidence is complete.** | Convert all lifecycle publications and prove generation-safe cleanup/debt bounds after V1. |
