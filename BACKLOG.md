...
- [ ] **N2.2 / Live Amount and Predicate Evaluation.** Finish exactly `Fixed` and current-Available `Percent` across valid typed balance/share/output surfaces. Specify reading points relative to fees, protected minima, ledger restrictions and Task effects. Use coherent current-read sets, checked/widened arithmetic and declared rounding; never silently shrink a fixed debit. Reevaluate on retry and after relevant mutations. Remove Opening/LastFunding consumers, snapshots and accumulation, not independent host custody/credit verification. Reject their removed encoded forms rather than reinterpret them. Execute the remaining coupled removal in this order: current-attempt freshness now covers prior-Step mutation, between-Cycle top-up and retry replenishment, and non-funding core/execution regressions now use current or fixed semantics; funding-module test preparation is complete: accumulator-only regressions are deleted while independent trigger-source, latch, canonical custody/credit, hold reconciliation and failure atomicity coverage remains on fixed/current semantics; browser authoring, amount editing, forecasting, local simulation, Actor status projection, semantic manifest and analysis no longer expose LastFunding semantics or accumulator balances; the production pallet API no longer admits or resolves the encoded LastFunding amount form, no contract can derive accumulator-tracked assets, and the canonical Run payload no longer retains a funding snapshot; field-level consumer audit proves the remaining `ActorFunding` owner has no independent metadata contract—tracked-asset derivation is unconditionally empty, accumulation is therefore unreachable, and funding-source policy remains in the Contract—so next remove the whole obsolete storage/type in one compile-coherent cohort rather than preserving a marker or reduced shell: first atomically detach it from loaded plans/state together with every scheduler/execution transition (a type-first cut leaves 70 errors; even a combined type/storage/loader cut leaves 52 production-library errors and 123 all-test errors, so use the library boundary as the first falsifier rather than mixing fixture conversion into that compile), then remove creation/lifecycle/try-runtime/state-hold ownership, and finally reconcile tests, runtime integration, embedding, control-weight geometry and benchmarks before validation; finally remove Opening amount/predicate snapshots and rename `PercentageOfCurrent` to `Percent`. **Exit:** Tests cover top-ups between Steps/retries, zero/dust, maximum amounts, shared-asset multi-field resolution and freshness; no hidden historical balance basis or unsafe spending shortcut.

- [ ] **N2.3 / Error Policy and Retry Continuity.** Retain Abort/Continue/Retry distinctions and exact temporary/permanent/funding/skip outcomes. Keep retries at the same cursor/Cycle; update bounded counters only for their specified outcomes. Service deferral, notifications, parking-index maintenance and ordinary ring revisit cannot reset bounds or count as an executed failure. Due-by-next-round retry may stay live; later retry sleeps without changing its meaning. Do not repeat previously committed economic effects. **Exit:** A closed transition table and independent model/property/fault tests prove fresh retries, exact limits and prefix durability under sleep/wake, mutation and contention.

- [ ] **N2.4 / Loaded Transition Context and Economic Owners.** Carry one bounded validated Attempt context through paid classification, planning, admission, effect, outcome, persistence and residence choice. Reuse only authority still fresh after fees, callbacks, host mutations and rollback. Cache structure rather than future values; future blocks read canonical current state. Commit only changed partitions, without hiding tentative state from canonical adapters. Price negative checks, scheduler turns, membership mutations and all remaining mandatory outcomes; a storage-free selector still consumes RefTime. **Exit:** One production execution path with explicit freshness/reload reasons, rollback-safe reuse, complete resource/payment owners and no dependence on node-local cache survival.

- [ ] **N2.5 / Certified Step and Wake Plans.** Reuse or extend admission certificates for immutable Step structure and idle start dependencies: exact typed read surfaces, relevant mutation/time boundaries, outcome shapes and resource selectors. Bind Contract/body, generation where applicable, semantic/physical versions, bounds and Weight identity. Derive on create/update or bounded recertification. Park evidence may refer to this plan; do not duplicate the authored program or add a universal dependency VM. Unsupported host wake domains select timed review or explicit refusal. **Exit:** The current execution and wake/check paths consume one qualified plan; stale plans cannot authorize service or indefinite parking, and certification cost is measured.
