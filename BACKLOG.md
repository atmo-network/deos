...
The semantic map, compare-and-replace writer, lifecycle callers, loader and try-state guards have crossed over, but active physical cells deliberately retain mirrored Identity/Hot/admission until the service carrier and generated resource owners replace that projection coherently. Effectful Step execution now returns a carrier-neutral typed transition containing its evidence and close-or-continue residence intent before the legacy FIFO finalizer owns physical placement, Weight and settlement. Zero-Step and opening/running StopCycle retain the same carrier-neutral boundary; their semantic event and resource distinctions remain explicit. The remaining executable cut is process/ring publication → mandatory service and all residence mutations → legacy scheduler retirement → physical-field removal → benchmarks and generated bindings. Intermediate helper-only states do not close N2.1 or N3.5.
