...
The first code prerequisite is a plan/commit split at `schedule_next_work_loaded`, not another publication adapter. That function currently appears to classify Queue/Wakeup/None, but its paused, unsignaled-window and later-eligibility branches call `schedule_window_expiry_with_authority` or `defer_wakeup_with_authority` and therefore publish legacy Waiting authority during classification. Canonical publication cannot call it and then atomically choose Service/Deadline/Park without first creating the forbidden legacy owner. Extract one storage-free typed residence plan from current state and time, then let the joint caller transaction commit exactly one canonical destination. Preserve the legacy adapter only as a temporary consumer of that pure plan until every listed composition root moves in the same retained cohort; do not commit a production state in which both committers are reachable.
