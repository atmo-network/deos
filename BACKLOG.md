...
The first code prerequisite is complete: `plan_next_work_loaded` now selects a typed `None`, `Queue` or exact-block `Wakeup` residence from current state, supplied Run authority, time and cutoff without touching storage. The temporary `schedule_next_work_loaded` legacy adapter is the sole production consumer and commits that plan to one FIFO/Waiting destination, preserving capacity rollback and the existing public behavior; focused tests prove the wakeup plan is storage-free before the adapter commits it. No canonical production committer was exposed, so authority remains singular. Cutover inspection exposed one remaining type hole before this plan can drive canonical publication: `None` currently conflates paused-without-window and active-unlatched states, while `ActorProcess` permits no residence-less `Serving` process and `Disabled` requires an exact cause, revival authority and suspended basis. The planner must therefore return an explicit generation-bound process destination—Service, Deadline or typed Disabled/other contract-authorized non-service state—rather than asking the committer to infer process semantics from `None`. Refine that result from existing lifecycle/trigger authority and prove every branch constructs a valid `ActorProcess`; then consume it inside the joint transaction while replacing the listed publication roots and mandatory consumer. Remove the temporary adapter in the same retained cut rather than leaving both committers reachable.
