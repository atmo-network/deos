...
The first code prerequisite is complete: `plan_next_work_loaded` now selects typed `Service(kind)`, exact-block `Wakeup` or `Disabled` process intent from current state, supplied Run authority, time and cutoff without touching storage. Paused-without-window retains an exact Owner/OwnerPaused Idle, Running or Suspended basis; active-unlatched uses Protocol revival with Idle basis instead of an invalid residence-less Serving process. The temporary `schedule_next_work_loaded` legacy adapter remains the sole production consumer and erases this richer result only at its FIFO/Waiting boundary, preserving capacity rollback and existing public behavior; focused tests prove Disabled, Pending Service and Wakeup planning leave storage unchanged. No canonical production committer was exposed, so authority remains singular. The remaining seam is destination materialization: pair the pure intent with the active `ActorRef`, preflight the concrete Service insertion or Deadline slot, and construct the complete generation-bound `ActorProcess` inside the joint transaction while replacing the listed publication roots and mandatory consumer. Remove the temporary adapter in the same retained cut rather than leaving both committers reachable.
