...
| --- | --- | --- | --- | --- |
| Canonical service V1 | One supported create/activate/progress/retry/complete path under only `ActorProcesses` plus `ServiceNodes` authority | N2.4, N3.5–N3.6, N5.1; multi-Actor resident integration witness | Generated create, round probe/admit, current Step/retry, completion and unlink owners | Every edge is transactionally single-authority; legacy Ready/Waiting is unreachable for that profile |
