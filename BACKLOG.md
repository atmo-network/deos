...
| Current inputs | Each admitted Attempt reads the required authoritative current state. | Preserve `Fixed`, `PercentageOfCurrent` and `AllAvailable` on valid typed Task surfaces, including available shares where appropriate. |
...
| Opening snapshots | Current reads are the default. Decide explicitly whether optional `PercentageAtOpening` and frozen Opening predicates remain or are removed. The last conversation did not independently approve every such removal. Unsupported old forms must be rejected, not reinterpreted. |
...
- [ ] **N2.2 / Live Amount and Predicate Evaluation.** Preserve valid fixed/current-percent/all-available forms and typed balance/share/output surfaces. Specify reading points relative to fees, protected minima, ledger restrictions and Task effects. Use coherent current-read sets, checked/widened arithmetic and declared rounding; never silently shrink a fixed debit. Reevaluate on retry and after relevant mutations. Remove LastFunding-only consumers and accumulation, not independent host custody/credit verification. Apply the explicit Opening-snapshot decision. **Exit:** Tests cover top-ups between Steps/retries, zero/dust, maximum amounts, shared-asset multi-field resolution and freshness; no hidden historical balance basis or unsafe spending shortcut.
