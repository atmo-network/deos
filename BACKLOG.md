...
The first atomic carrier cut has one implementation seam, not a publish-only precursor. Replace the `ActorReadyTail` prepass snapshot and `live_queue_head` discovery loop with `begin_service_round` plus bounded `consider_service_head` encounters; adapt `service_live_queue_entry` to load authoritative semantic/process state from an `ActorRef` and consume the captured ring head rather than an `ActorControlCell`; route its `NextResidence` result to process/ring retention, deadline transfer or retirement; and switch initial/resumed publication through the same transaction before deleting the FIFO path. The existing service function currently derives Identity/Hot/admission, eligibility, resource envelopes and queue-consumption authority from the legacy cell, so changing publication alone would create inert ring members and lose executable resource ownership. Generated Weight must cover ring probe/advance and every retained publication/removal suffix before the mandatory hook can advertise the carrier.
