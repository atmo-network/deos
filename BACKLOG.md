...
- [ ] **N2.4 / Loaded Transition Context and Economic Owners.** Start the joint N2.1/N2.4/N3.5 atomic cutover here: introduce the stable Active semantic owner and generated lifecycle publication/removal owners before routing continuation service. Make that semantic owner the checked source of `ActorGeneration`: initial Contract publication creates the first generation, every authorized Contract replacement increments it or fails closed on exhaustion, dormancy preserves the last generation, and ordinary hot-state updates never change it. Expose the resulting `ActorRef` to every process, ring, deadline, park and cleanup publication; never derive generation from cycle nonce, admission identity or placement. Finish the read/write cutover begun by `load_actor_semantic_state`. Inventory every semantic Publish/Replace/Remove owner and convert create, activate, update, deactivate, cancel, completion and terminal paths together with their resource composition. Use one stable semantic owner; derive cursor/resources from their selected owners rather than adding synchronized copies. Carry validated loaded authority until a relevant callback/mutation invalidates it. **Exit:** ordinary callers no longer recover semantic state from legacy placement; rollback restores the entire owned transition; every canonical carrier is bound to the current semantic generation; no copied authority, unowned selector or stale cached balance exists.
