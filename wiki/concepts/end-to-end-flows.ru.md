...
1. Изменяемый план выполняет `SwapIn`, затем пытается выполнить `AddLiquidity`, после чего должен сделать `Transfer`.
2. Обмен успешно фиксируется. Интерфейс ликвидности сообщает явно временную ошибку.
3. `RetryLater` откатывает только неудачную задачу и сохраняет разреженное продолжение на шаге `AddLiquidity`.
4. После периода ожидания повтор входит в те же очередь и систему пробуждений без нового внешнего сигнала. Допуск и комиссия охватывают только `AddLiquidity → Transfer`.
5. Успех удаляет продолжение и создает одно суммарное событие для исходного номера логического запуска. Отмена оставляет обмен совершенным и не выполняет компенсацию.

Текущие шаг и попытка — ограниченная правда цепочки. Длительная история попыток принадлежит материализованному индексу событий.

## Границы намерения обмена

- `SwapIn` фиксирует заданный расход; исполнимая котировка в момент попытки определяет результат, а допуск ограничивает ухудшение относительно этой котировки.
- `SwapOut` фиксирует заданный результат; исполнимая котировка в момент попытки определяет расход.
- `InputLimit::LiveQuote` осознанно принимает будущее движение рыночной цены в пределах сохраняемого баланса, а `InputLimit::Absolute(nonzero)` добавляет фиксированный предел расхода.
