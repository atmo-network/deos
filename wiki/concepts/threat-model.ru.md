...
| --------------------------------- | ---------------------------------- | ---------------------------- | --------------- |
| Governance выводит TOL            | перенаправление резервов           | domain payloads + protection | governance/spec |
| Обход роутера                     | обход fee/burn                     | шлюз роутера                 | router          |
| Неправильное использование bucket | потеря происхождения               | сегментированные каналы      | TMCTOL + AAA    |
| Путаница с indexer                | архив выдается за правду           | значки происхождения         | client/docs     |
| Доверенная фаза коллатора         | доверие выдается за permissionless | launch-line ограничение      | runtime/ops     |
| Зависание графа акторов           | сбой/простой/пробел оракула        | retry + cooldown             | AAA             |
| Параметрический гриффинг          | параметры нарушают предположения   | bounded settings             | runtime/gov     |
| Атака на оценку LP                | LP переоценён или учтён дважды     | консервативное хранение      | staking/gov     |
| Ложное происхождение во фронтенде | UI скрывает класс данных           | read-model contract          | web client      |
