...
The unlaunched reference chain keeps fresh-baseline Actors storage version `9` and ships no historical migration. This baseline includes canonical Trigger runtime state, exact User Crossing counts, refundable bond state, and the materialization-family cursor. The independent embedding gate executes absent and bounded present Precondition, `StopCycle`, and Continuation behavior without a DEOS/TMCTOL helper or actor-topology dependency.
