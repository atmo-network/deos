...
Actor balances can function like trigger messages: an asset arriving on an actor account can wake the Actor Contract, and that pending signal must retain a bounded path to eventual eligibility. Manual and matched address events coalesce through the single `pending_signal` latch; admitted execution clears it atomically while deferral, pause, and scheduler movement preserve it. Pause atomically moves Ready authority to Unsignaled, leaving its former FIFO slot as an ordinary tombstone; while the global breaker is active, a live Ready head remains at the same ticket instead of being consumed and republished. `ObservationChange { feed }` now declares reconsideration against one exact typed feed without threshold, callback, payload, or per-revision execution semantics; Actors derives a duplicate-free bounded subscriber index from that source, and the Oracle hook now coalesces only the latest changed revision into reusable dirty-feed state without reading subscribers. An independently metered deferred worker traverses bounded subscriber pages, binds that state to the same latch, and leaves execution to the existing queue/wakeup scheduler. Observation, Manual, periodic-only, and mixed source sets cannot author `PercentageOfTrigger`; only applicable AddressEvent-only sources establish its exact cycle-start balance snapshot contract.

`ObservationCrossing` is the sparse threshold trigger. It requires distinct fire and rearm thresholds, initializes from current canonical observation without retroactive activation, and retains every committed `previous -> current` revision transition until bounded ordered processing completes. One feed admits at most 9,000 User memberships and 10,000 total memberships, reserving 1,000 positions exclusively for System Actors. Active User Trigger state is backed by an exact refundable runtime-policy bond; replacement adjusts it atomically and deactivation or terminal close releases it. Rising and Falling memberships live in separate exact-threshold radix pages. Manual calls, address movement, broad observation changes, sparse crossings, and cadence deadlines therefore specialize detection but converge through one activation sink, one pending latch, one placement contract, and the same FIFO. On the previously released cohort, Crossing materialization executed `1`, `3` and `8` candidates across its three-block cycle (mean four); the retained placed pair-owner fallback executes `2`, `6` and `8` for the steady cycle after the opening leaf, moving the cycle `12 → 16` at unchanged cycle ProofSize with a measured runtime-bound RefTime premium per additional candidate. The executed per-block ceiling remains eight, so 100 eligible blocks bound Crossing materialization at 800 candidates and the historical performance-assurance profile of 10,000 User crossings within 100 eligible blocks stays unsatisfied; System schedules and broad observation fanout are not substitutes.
...
A generic live FIFO entry charges discovery, the loaded-state probe, source consumption, and one transition owner. If admission closes the actor, or a paid Crossing lacks its required current observation, one complete close/refusal owner replaces that entry's provisional prefix rather than being added to it. The refusal preserves the head and performs no Pipeline collection. Work on earlier tombstones or the next queue head remains separate. This accounting boundary changes neither FIFO order nor actor state semantics.
...
FIFO progress also requires available observations for Trigger rearm and successful fee collection when payment capacity exists. If a latched Crossing cannot read its required current value, Opening commits no Pipeline fee or cycle and preserves the paid latch and live head; later Actors cannot bypass it despite spare Weight. In DEOS, paused feeds and old retained values remain available to rearm, but deactivated feeds do not. Observation loss grants no new close authority: recovery depends on restored prerequisites or an already-authorized lifecycle transition.
...
External runtimes can reuse `pallet-deos-actors` without inheriting the DEOS/TMCTOL System actor catalog. The host runtime provides bounded adapters for assets, caller-aware DEX quotes, staking shares, liquidity donation, funding authority, atomic fee collection, fallible ingress, and two-dimensional task weights. Actors owns scheduling, lifecycle, policy-aware amount resolution, fee reservation, and task orchestration. After read-only evaluation, each attempted User step calls `FeeCollector` at most once: non-executable outcomes charge evaluation-only, while executable outcomes charge evaluation plus execution together. The collector transfers the full charge into Fee Sink; downstream allocation remains outside Actors. The DEOS reference Fee Sink currently applies its 50/50 staking/liquidity Actor Contract.
...
The shipped runtime reserves fifteen deterministic System addresses but enrolls only three active Actor Contracts at genesis: Burn Actor, Fee Sink, and BLDR Splitter. These contracts react to verified inbound value rather than periodic polling. Twelve System identities start dormant with no Actor Contract, funding, fee, queue, wakeup, or cycle state. Ten are Mutable activation candidates. First-order Bucket A and BLDR Anchor are sealed Immutable identities: every control path rejects activation or close, while the runtime LP freezer admits incoming LP but exposes zero reducible balance to ordinary, admin-forced, internal transfer, burn, and LP-class destruction paths. Native staking LP provisioning can activate only after the receipt asset, staking pool, dormant identity, and non-empty `$NTVE/stNTVE` AMM are ready.
