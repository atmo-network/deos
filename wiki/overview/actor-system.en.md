...
`ObservationCrossing` is the sparse threshold trigger. It requires distinct fire and rearm thresholds, initializes from current canonical observation without retroactive activation, and retains every committed `previous -> current` revision transition until bounded ordered processing completes. Rising and Falling memberships live in separate exact-threshold radix pages. Manual calls, address movement, broad observation changes, sparse crossings, and cadence deadlines therefore specialize detection but converge through one activation sink, one pending latch, one placement contract, and the same FIFO.
...
The unlaunched reference chain keeps fresh-baseline storage version `2` and ships no historical migration. The independent embedding gate executes absent and bounded present Precondition, `StopCycle`, and Continuation behavior without a DEOS/TMCTOL helper or actor-topology dependency.
