...
`ObservationCrossing` is the sparse threshold trigger. It requires distinct fire and rearm thresholds, initializes from current canonical observation without retroactive activation, and retains every committed `previous -> current` revision transition until bounded ordered processing completes. One feed admits at most 9,000 User memberships and 10,000 total memberships, reserving 1,000 positions exclusively for System Actors. Active User Trigger state is backed by an exact refundable runtime-policy bond; replacement adjusts it atomically and deactivation or terminal close releases it. Rising and Falling memberships live in separate exact-threshold radix pages. Manual calls, address movement, broad observation changes, sparse crossings, and cadence deadlines therefore specialize detection but converge through one activation sink, one pending latch, one placement contract, and the same FIFO. Current production admission processes four Crossing candidates per block, so the 9,000-User population needs at least 2,250 uncontended detector blocks. It does not satisfy the assurance target of 10,000 User crossings within 100 eligible blocks; System schedules and broad observation fanout are not substitutes.
