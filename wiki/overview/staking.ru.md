...
Спецификация резервирует отдельные пути получения нативных наград для Phase 2. Общий расчет награды в том же активе отвергает нативный staking asset, чтобы награды `$NTVE` не уходили через ненативный путь автоматического реинвестирования.
...
Для ненативных активов расчет награды в том же активе по-прежнему может автоматически выпускать новые квитанции. Текущее распознавание использует ограниченный прием событий только из текущего блока с явными лимитами сканирования и Weight; при усечении эпоха становится недоступной для получения наград, а историческое воспроизведение не заявляется. Награды `$NTVE` за номинацию остаются отдельным, привязанным к этапу потоком и не действуют в стартовой линии Phase 1 с доверенными коллаторами.
