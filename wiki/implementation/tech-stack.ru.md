...
Внешние активы приходят через XCM v5 и затем привязываются к локальным `AssetId` через реестр активов.
...
Веб-клиент DEOS в первую очередь опирается на данные блокчейна, но явно отделяет производные данные сеанса и материализованные данные. Он построен на:
