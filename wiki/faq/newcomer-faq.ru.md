...
**Чем Actors отличается от AA-Актора?** `DEOS Actors` — вся система акторов среды исполнения: планировщик, правила жизненного цикла, `Actor Contracts`, счета акторов и пошаговое исполнение. `AA-Актор` — один конкретный экземпляр в среде исполнения этой системы. Подробнее: [Система Actors](../overview/actor-system.ru.md) и [AA-Актор](../overview/actor.ru.md).
