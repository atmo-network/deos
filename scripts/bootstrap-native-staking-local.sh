...
NATIVE_STAKING_LIQUIDITY_ACTOR_ID="${NATIVE_STAKING_LIQUIDITY_ACTOR_ID:-14}"
...
Options:
  ...
  --actor-id ID               Native Staking Liquidity Actor id for check mode (default: 14)
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --actor-id)
                ...
                NATIVE_STAKING_LIQUIDITY_ACTOR_ID="$2"
                shift
                ;;
            --operator-address)
...
check_prerequisites() {
    ...
    require_uint "$NATIVE_STAKING_LIQUIDITY_ACTOR_ID" "Actor id"
...
print_plan() {
    ...
    if [[ "$MODE" == "check" ]]; then
        echo "  Staking liquidity actor: $NATIVE_STAKING_LIQUIDITY_ACTOR_ID"
    else
...
run_node_flow() {
    ...
    (
        ...
        NATIVE_STAKING_LIQUIDITY_ACTOR_ID="$NATIVE_STAKING_LIQUIDITY_ACTOR_ID" \
        OPERATOR_ADDRESS="$OPERATOR_ADDRESS" \
...
const actorId = parseEnvBigUint("NATIVE_STAKING_LIQUIDITY_ACTOR_ID");
...
function checkPhase(checks) {
  ...
  if (!checks.stakingLiquidityActorExists) return "missing-native-staking-liquidity-actor";
...
function checkRecommendation(phase) {
  switch (phase) {
    ...
    case "missing-native-staking-liquidity-actor": return "Ensure genesis System Actors configuration includes the Native Staking Liquidity Actor skeleton before activation";
    default: return "Run the guarded Native Staking Liquidity Actor activation path if it is not active yet; otherwise bootstrap is ready";
  }
}
...
function prepareRecommendation(phase) {
  switch (phase) {
    ...
    case "needs-operator-stntve-acquisition": return "Submit the emitted operator stake call for NTVE, then rerun this helper after finality";
    ...
    default: return "The canonical pool is non-empty; run check mode and then the guarded Native Staking Liquidity Actor activation path if needed";
  }
}
...
try {
  ...
  const [securityViewResult, boundaryDiagnostic, exchangeRate, pool] = await Promise.all([
    api.view.Staking.native_security_view({ at: block.hash }),
  ...
  if (!securityViewResult.success) {
    throw new Error(`Native security view rejected bounded state: ${securityViewResult.value.type}`);
  }
  const securityView = securityViewResult.value;
  const securityState = securityView.value;
  const securityMode = { type: securityView.type };
  const securityReadiness = securityView.type === "LpBackedSelection"
    ? securityState.readiness
    : null;
  const securityEpoch = securityState.current_epoch;
  if (mode === "check") {
    ...
    const payload = { wsUri, block: block.number, nativeAssetId, stakedNativeAssetId, actorId: Number(actorId), phase, recommendedAction: checkRecommendation(phase), securityMode, securityReadiness, securityEpoch, boundaryDiagnostic, checks, exchangeRate, pool, stakingLiquidityActor };
    if (jsonOutput) console.log(stringify(payload));
      ...
      console.log(stringify({ block: payload.block, nativeAssetId, stakedNativeAssetId, actorId: payload.actorId, securityMode, securityReadiness, securityEpoch, boundaryDiagnostic, checks, exchangeRate, pool, stakingLiquidityActor }));
    }
  } else {
    ...
    if (checks.exchangeRateAvailable !== checks.stakedAssetExists) {
      throw new Error("Native staking registration and stNTVE receipt state disagree; repair the runtime-owned registration atomically");
    }
    const calls = [];
    if (checks.nativeAssetExists && !checks.exchangeRateAvailable) {
      calls.push(await encoded(
        "register native staking asset and stNTVE receipt",
        "Root/governance staking AdminOrigin",
        api.tx.Staking.register_staking_asset,
        { asset_id: nativeAssetId },
      ));
    }
    if (checks.stakedAssetExists && !checks.operatorHasStakedLiquidity) {
      calls.push(await encoded("stake NTVE into stNTVE for bootstrap liquidity", "Signed operator", api.tx.Staking.stake, { asset_id: nativeAssetId, amount: stakeAmount }));
    }
    ...
    const payload = { wsUri, block: block.number, nativeAssetId, stakedNativeAssetId, operatorAddress, requestedAmounts: { stakeAmount, liquidityNative, liquidityStaked, minNative, minStaked }, phase, recommendedAction: prepareRecommendation(phase), securityMode, securityReadiness, securityEpoch, boundaryDiagnostic, checks, exchangeRate, pool, operatorStakedBalance, calls, submissionPolicy: "Plan-only: submit emitted calls through the stated authority; this helper never signs or submits. LP-backed security actions require LpBackedSelection mode and are not synthesized here" };
    if (jsonOutput) console.log(stringify(payload));
    else {
      console.log(`Phase: ${payload.phase}`);
      console.log(`Recommended action: ${payload.recommendedAction}`);
      console.log(`Submission policy: ${payload.submissionPolicy}`);
      console.log(stringify({ block: payload.block, nativeAssetId, stakedNativeAssetId, operatorAddress, securityMode, securityReadiness, securityEpoch, boundaryDiagnostic, checks, exchangeRate, pool, operatorStakedBalance, calls }));
    }
  }
} finally {
  client.destroy();
