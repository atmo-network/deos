...
- [actors-assurance.sh](./actors-assurance.sh)
  Shared current-tree Actors proof contract for semantic-manifest and fee-envelope-vector freshness, cross-language semantics, scheduler stress, capacity, and independent-runtime embedding. `--exact-heavy-profile NAME` runs one profile from the gate's declared runtime/package heavy inventory, verifies that it resolves exactly once, and preserves the same source/artifact identity boundary without replaying unrelated heavy predecessors. `--production-reference-replay` separately runs the one ignored full-block StorageProof replay against the currently pinned production Wasm rather than allowing an ordinary filtered test command to report a skipped test as evidence. By default the broad gate reports and preserves exact source, production Weight, production Wasm, and metadata identities. A caller that intentionally cannot build Wasm may set `REQUIRE_WASM_IDENTITY=0`; the script reports that narrower evidence boundary while retaining every non-Wasm check. Historical transition replay is release history rather than a routine validation dependency.

  `--integrated-w0-w1` selects EXP-0066's bounded preparation gate instead of the broad suite. It authors complete W0 empty/one-Step blocks and a 128-Actor W1 falsifier through runtime-created inherents, full Executive hooks and finalization under Actor-only and continuously saturated valid user demand, then independently replays the accepted production Wasm with recorded StorageProof and CompactProof. This proves the measurement path and identity binding, not closure of the historical 10,000-Actor reference horizon. `--integrated-w1-actor-only` and `--integrated-w1-continuous-user` preserve the reference preset's 15 System Actor identities (three active and twelve dormant), fill the remaining identity capacity with 9,985 mixed Manual/Cadenced workload Actors, author 100 linked finalized blocks, and exact-Wasm proof-replay every block, reporting the retained historical 10,000-Cycle and 100-distinct-Actors-per-block reference horizon; shortfall inside that horizon is reported, not gated. The continuous-user cohort also fills every User base turn with valid signed remarks and carries the signer nonce monotonically across blocks. `--integrated-w2-schedules` runs separate Actor-only Manual-only and Cadenced-only populations through the same 100 linked full-Executive blocks and exact-Wasm proof contract, isolating execution from temporal scheduling cost without changing the accepted binding. `--integrated-w3-opening-matrix` runs 14 equal repetitions of every 0/2/4 Opening-predicate by 1/2/3-Step Manual-only cell through linked full-Executive blocks until all 126 Actors and 252 Steps complete, reporting Opening, strict-middle and final Steps separately and exact-Wasm proof-replaying every retained block. `--integrated-w4-heterogeneous-effects` preserves the reference preset's 15 identities and fills the remaining production capacity with a deterministic interleaving of 9,485 Transfer, 400 Router SwapOut, and 100 StopCycle Actors (the nominal 9,500 Transfer slots less the 15 retained production identities), then measures 100 linked blocks under both Actor-only and continuously saturated valid-user demand with exact-Wasm proof replay. `--integrated-w5-lifecycle-retry-cleanup` runs one five-Actor Manual matrix through zero-Step completion, successful Opening/middle/final Running, temporary retry and exhaustion, productive cleanup, minimal admission apoptosis, and uncompensated committed-prefix durability across linked native and exact-production-Wasm blocks. `--integrated-w6-mixed-arrival-lifecycle` runs deterministic dense Manual, sparse block-window/AtTime, and seeded Cadenced arrivals across both scheduler clocks while signed pause/resume, owner close, Oracle publication and Crossing Contract replacement produce and safely consume normal tombstone and stale-generation churn; all retained blocks use the same native/full-Executive and exact-Wasm proof contract. `--integrated-w7-due-only-active-frontier` compares a 100-due-Actor control against the same FIFO frontier at the exact 10,000-identity production cap. The maximum profile keeps 4,943 workload Actors behind one future block-clock window and 4,942 Manual Actors unsignaled; together with the 15 retained reference identities they form the declared 9,900 non-due population, while both profiles must complete the same 100 due Actors through native and exact-Wasm blocks. `--integrated-w8-tombstone-prefix-chunk-pressure` places legal closed-Actor tombstone prefixes of 1/4/8/16/32/64/128 physical Ready entries ahead of 100 due Manual Actors, then measures one complete block per profile. Every profile must reclaim the exact tombstone prefix, preserve the live FIFO/Q1 committed prefix, and stop pending live work only at the Actor Control frontier without comparing alternative page layouts. `--integrated-w9-resource-independence` compares one identical 100-due-Actor FIFO frontier under Actor-only, proof-saturated signed remarks, and RefTime-heavy signed Router swaps. It requires identical Actor progress and accounting, then records the honest fallback when the highest retained valid business-call profile still exhausts User ProofSize before RefTime. `--integrated-control-attribution` compares matched 100-Actor Manual and cadence-one fixtures for nine linked blocks, records mandatory-Prepass and finalized Actor Control separately, and proves temporal occurrence materialization is Prepass-owned while the captured FIFO cutoff defers newly materialized service. The same gate pins the generated production-Weight read/write ledger for fixed coordination, dual-clock probes, retained/removed wakeup consumption, the Cadenced rearm-topology delta, and reserved close/fault contingencies; these additive admission envelopes remain distinct from actual complete-block proof.
...
- [benchmarks.sh](./benchmarks.sh)
  Run pallet benchmarking flows and weight generation helpers. `--check` compiles benchmark surfaces and rejects generated Weight files that name Actors storage absent from the current pallet; `--extra` enables Actors diagnostics, `--extrinsic NAME --output FILE` produces focused evidence that must not replace complete production weights, and `--skip-build` reuses a freshly built benchmark runtime. Add `--json-file FILE` to an exact extrinsic run to preserve raw component samples alongside its generated model; this requires frame-omni-bencher. Unbound User zero-Step and empty/large-head Pipeline collection profiles require exact `--extrinsic` selection and remain excluded from default production generation. The [`architecture-experiments` skill](../.agents/skills/architecture-experiments/SKILL.md) owns case selection, controlled comparison, evidence interpretation, decision lineage, Weight handoff, and claim boundaries without duplicating this command surface.

  Use paired `--low VALUES --high VALUES` for selected component ranges, with comma-separated unsigned values in the benchmark's component order. Bounds require an exact extrinsic, explicit output file and frame-omni-bencher; the bencher validates ranges and samples each axis with other components at their upper bounds. This does not exhaust a Cartesian grid. `--min-duration 0` makes quick reachability probes use only the requested repeat count; omitted duration retains the bencher default. Restricted sampling cannot establish complete production coverage.

- [ci-local.sh](./ci-local.sh)
  Reproduce local CI or select one compact check with `--only`; narrow Cargo work further with `--package NAME`, `--test-filter NAME`, and explicit feature mode. Use `--only tests --release` for optimized large-fixture tests; other checks reject `--release`. Apply Rust formatting with `--only format --fix`. Agents should prefer this entrypoint over raw Cargo commands.

The native schedule-accounting test defaults to 100 added System Actors over nine Cadenced blocks. Set `DEOS_SCHEDULE_LEDGER` to `manual`, `mixed` or `cadenced` to select one full identity-bound, 100-block Actor-only profile; set `DEOS_PRODUCTION_WASM` to the accepted production Wasm file. Run `ci-local.sh --only tests --release --package deos-runtime --test-filter full_executive_schedule_ledger` with `RUST_TEST_NOCAPTURE=1 DEOS_VERBOSE=1`. The Wasm is genesis code only: the test authors and reconstructs native execution, emits code identity and block hashes for retained-evidence comparison, and does not execute Wasm or certify a new production artifact. Unset the profile (or select `cadenced-small`) for the small regression; other values are rejected. Workload and reference occurrences remain distinct.

`DEOS_SCHEDULE_LEDGER=mixed-user` selects the same full Mixed population with continuous valid signed remarks, reports User usage and the next candidate's preflight budget deficit, and preserves signature verification and nonce progression. Fresh signing does not promise retained header identity: compare the available evidence honestly rather than treating summary equality as exact-state equality. The workload Actors remain System Actors; this mode does not cover User Actor fees.

`actors-assurance.sh --integrated-funded-user-action` executes both the native monetary/resource witness and its ignored exact-production-Wasm replay under one pinned identity gate. It proves the selected funded User Transfer's Trigger, Pipeline and Action fee separation; it is not broader User Task or retry coverage.

`actors-assurance.sh --backpressure-audit` executes EXP-0100's focused runtime and package falsifiers for DEX/Router, XCM, observation, funding/resource, scheduler-capacity and adapter rollback ownership. It validates existing atomicity and sole Actor retry authority without introducing a new queue, registry or production-Wasm claim.

The W5 lifecycle test runs through `ci-local.sh --only tests --package deos-runtime --test-filter full_executive_w5_lifecycle_retry_cleanup_fixture`. It also checks charged Control/effect ownership for zero-Step, Running, retry, terminal cleanup and User admission apoptosis. Optional `DEOS_PRODUCTION_WASM` pins accepted genesis code for retained-header comparison without executing Wasm; unset it for the standalone native fixture.

`ci-local.sh --only tests --package deos-runtime --test-filter full_executive_funded_pipeline_collection_witness` compares funded System/User StopCycle fee ledgers and reports charged Control. It verifies monetary deltas and receipts; a passing witness is not proof of complete User Weight coverage. The companion filter `full_executive_user_completion_header_domain_witness` checks ordinary admission and execution of OwnerOnly/maximum-allowlist heads sharing a source-derived completion context; it is a domain-alias witness, not a numeric Weight bound.

The extra `scheduler_inner_opening_user_complete_single` and `scheduler_inner_opening_user_complete_tail` Actors benchmarks measure User StopCycle after real source consumption, including Pipeline collection and hold reconciliation but not outer FIFO work. Zero tails and positive tails use separate models because funding-hold release changes I/O. Native construction uses `ci-local.sh --only tests --package pallet-deos-actors --all-features --test-filter scheduler_inner_opening_user_complete_`. Reference measurement requires fresh benchmark Wasm and `benchmarks.sh --extra --extrinsic NAME --output /tmp/user-completion.rs pallet_deos_actors`; both diagnostics remain outside default production generation and do not qualify arbitrary User Contract geometry.
...
- [export-papi-metadata.sh](./export-papi-metadata.sh)
  Export native runtime metadata through the committed `deos-runtime` metadata example in an isolated Cargo target, regenerate PAPI descriptors, and project observation-inspector runtime evidence from the exact metadata, compressed production runtime-code Wasm, runtime constants, production Actors weights, and descriptor identity. The exporter initializes the canonical reference ParaId before reading storage-backed metadata constants, so its bytes match Metadata API output from the reference composition. Isolation prevents metadata compilation from replacing the production Wasm artifact. This replaces ad hoc metadata export and independently maintained inspector constants.

- [bootstrap-native-staking-local.sh](./bootstrap-native-staking-local.sh)
  Consolidated native staking bootstrap helper. `check` reads the mode-shaped canonical `NativeSecurityView`, pool readiness and bounded `ActorEligibilityApi` state at one finalized block. JSON `stakingLiquidityActor` contains the Active activation projection, or null for Dormant/NotRegistered; classification errors fail closed. Matching runtime metadata/descriptors are required. `prepare-calls` emits plan-only registration, pool and liquidity calls without mode-inactive LP-security actions. It never signs or submits transactions; the staking specification and package architecture own sequencing and authority boundaries.
