...
- [network-assurance-local.sh](./network-assurance-local.sh)
  Prepare pinned tools/artifacts, spawn the canonical two-validator/two-collator topology, verify both collators prepare blocks, pause one collator while the other preserves finality, restart the second collator from its existing base path, and execute signed finalized transfers before and after restart. `SESSION_TRANSITION=1` adds the multi-hour finalized session proof, while `COMPOSED_PATH=1` adds the mutating Router/Oracle/Burn Actor proof; the default local composition and build path remain unchanged. Candidate mode records only successful steps in exact smoke/authors/failover/pre-transfer/persisted-restart/post-transfer/composed order, routes the final transfer and composed proof through restarted Dave, then terminates and verifies every owned process before the summary consumes the exact ledger. Failure or ineffective cleanup removes the ledger and permits no summary. No mode claims runtime-upgrade acceptance.
...
- [validate-local.sh](./validate-local.sh)
  Run the canonical `fast`, `heavy`, or fail-closed `full` release-validation profile. The entrypoint remains the sole stage, order, repetition, and pass/fail owner. Clean whole-profile successes may reuse exact local evidence through `validation-evidence.mjs`; dirty local `fast|heavy` runs stay uncached, every CI profile and local `full` require a clean candidate, `--fresh` forces execution, and `DEOS_VALIDATION_CACHE=0` disables lookup and storage. Inherited semantic environment controls fail before lookup, and test-only numeric controls accept complete unsigned safe-integer literals only.
...
`validation-evidence.mjs` owns clean tracked/staged/untracked/index/tree identity, authority/environment/invocation/immutable-commit identity, append-only reached-boundary verification, Git-common-dir v2 records, owner-checked locking, atomic publication, and full-artifact manifests. It binds every existing effective Cargo and npm configuration layer unless the exact candidate-relative path is a clean regular file whose bytes and executable mode match the candidate Git tree; ignored, untracked, absent-from-tree, type-mismatched, and symlinked configurations remain path/type/mode/content-bound environment inputs. Absent paths, including nonexistent `TMPDIR` roots, add no identity. Existing unreadable or unstable layers fail closed, while pre/post and final-hit appearance, disappearance, path, type, mode, symlink-target, or content changes invalidate reuse. Before every internal child, the helper and `validate-local.sh` replace `DEOS_PROJECT_ROOT` and `DEOS_BINARY_DIR` with the resolved candidate repository and its `bin/` directory, so caller values cannot redirect validation or hydrate alternate binaries. It rejects inherited semantic tool overrides and rechecks the complete identity immediately before a hit returns. It runs only the repository's fixed `scripts/validate-local.sh` path. `validation-authority.v1.json` is loaded from the candidate Git tree and declares constrained authority roots, the exact immutable Actors baseline commit, and required nonempty Wasm/metadata/descriptor/runtime-evidence outputs. Missing, empty, invalid, or drifted `full` artifacts invalidate and remove the stale hit before ordinary canonical execution; only a successful rerun publishes replacement evidence. Records remain under `<git-common-dir>/deos-validation/v2/`; they are local same-user reuse evidence, not CI, network, release, publication, or mutable-freshness evidence. The test suite falsifies the inspected controls and races but does not claim host-compromise resistance: `node --test scripts/test-validation-evidence.mjs`.

`release-evidence.mjs` derives the candidate allowlist from candidate-tree `validation-authority.v1.json` and requires every full-record artifact entry to belong exactly once to all eight current output families. Candidate bytes, sizes, and SHA-256 values must equal the successful `full` record; only its canonical record and summary are added. Physical and manifest-listed extra, hidden, missing, empty, symlinked, traversal-shaped, or substituted members fail closed. `github-release-artifact.mjs` independently verifies repository, run attempt, successful producer job, tag head, exact nonexpired artifact ID/name/GitHub digest, raw archive SHA-256, and bounded regular-file-only candidate or single-file ZIP inventory before extraction.

The final package owner accepts only those exact same-run candidate and successful network-summary handoffs. It performs no runtime, metadata, descriptor, network build, or filesystem package scan. `deos-release-evidence-v1` generates the canonical SPDX directly from isolated exact tag-tree lock copies: one deterministic package per exact Cargo owner/location/name/version/source/checksum row and npm owner/location/name/version/resolved/integrity/dev/optional/devOptional/peer/inBundle/link row, including duplicate nested instances and the release validator lock. Every package is explicitly described by the document and carries its complete canonical release-provenance row. Pinned AJV validates against the pinned SPDX 2.3 JSON schema before root, relationship, and independent one-to-one lock reconciliation; generation from two absolute staging roots must be byte-identical and host-path-free.

The release manifest persists one exact 13-file inventory: 11 payload assets with size/SHA-256, one self-describing manifest entry without a recursive self-digest, and one checksum-control entry excluded only from itself. `SHA256SUMS` hashes the other 12 members, physical inventory and attestation subjects equal all 13 exactly, and the final workflow artifact is retained for 90 days. No workflow receives release-creation or `contents: write` authority.

`release-tools.v1.json` pins Polkadot and Omni Node 1.24.1 plus chain-spec-builder 19.0.0 to `polkadot-stable2606-1` commit `8ae9775dc43c0d8cdd0f6d87700596e14278b1e1`, Zombienet 1.3.138, and the SPDX 2.3 schema at commit `aadf3b0b8dbbabdb4d880b0fc714255fea436ff7` with its exact hash. `scripts/release-tooling/package-lock.json` directly owns exact AJV 8.20.0 schema-validation dependencies and joins lock/SBOM authority. Existing PATH runtime tools are accepted only when the locked path, binary digest, and version identity agree. Zombienet's independent pin authenticates the orchestrator but does not infer SDK compatibility; only the ordered network run supplies that evidence. GitHub Release creation and upload remain separately authorized external acts. Run `node --test scripts/test-release-evidence.mjs` for the acceptance, provenance, malicious-archive, bundle, SPDX, archive, workflow, proof, cleanup, and tool falsification suite; set `DEOS_SPDX_SCHEMA` to execute the pinned-schema cross-root case.

`validation-ci-evidence.mjs` is the narrow GitHub provenance bridge. Same-repository pull requests run canonical `--fresh fast` and upload one strict envelope containing the exact local semantic record plus repository, workflow authority, event, run, attempt, job, head, tree, and conclusion identity. A main push uses a 90-second overall discovery budget and 15-second AbortController request bounds to find one unambiguous successful job and attempt-bound artifact, then accepts exactly one bounded regular `evidence.json` and compares it with a freshly calculated main identity. Any timeout, API/output/download failure, fork, duplicate or stale artifact, workflow bootstrap change, corrupt payload, failed run, or semantic mismatch leaves no reusable output and falls back to canonical `--fresh fast`. The workflow defines one stable check name, `validation-gate`; repository branch protection or a ruleset must separately require exactly that check and remains an external gate until verified. Caches are acceleration only and never include `.git/deos-validation`. Tag-bound `full`, network, security freshness, SBOM, signing, publication, and release evidence remain separate. CI behavior is covered by `node --test scripts/test-validation-ci-evidence.mjs`.
