...
check_prerequisites() {
    ...
    BASELINE_COMMIT="$(node -e 'const m=require(process.argv[1]); const i=m.immutableRefInputs.find((x)=>x.name==="deos-actors-0.7.17-baseline"); if(!i) process.exit(1); process.stdout.write(i.commit)' "$SCRIPT_DIR/validation-authority.v1.json")" || {
        log_error "Immutable 0.7.17 baseline authority is missing"
        exit 1
    }
...
run_executable_contract() {
    ...
    report_validation_boundary "actors.semantic-manifest.baseline-generate"
    run_shell_step \
    ...
    report_validation_boundary "actors.semantic-manifest.current-generate"
    run_shell_step \
    ...
    report_validation_boundary "actors.semantic-manifest.exact-compare"
    ...
    report_validation_boundary "actors.golden-equivalence.baseline-reactive-corpus"
    run_shell_step \
    ...
    report_validation_boundary "actors.golden-equivalence.current-reactive-corpus"
    ...
    report_validation_boundary "actors.golden-equivalence.baseline-semantic-anchor-family"
    report_validation_boundary "actors.golden-equivalence.current-semantic-anchor-family"
