...
- `Skill Workflow Ownership`: Domain-specific agent strategy, mode selection, evidence interpretation, coordination, and handoff belong in project skills; co-located scripts remain agent-specific leaves rather than shared project automation.
- `Shared Script Placement`: Any executable used by humans, GitHub Actions, CI, root compositions, or multiple skills belongs in root `/scripts`; skills and workflows call that public implementation instead of making root automation delegate into a skill directory.
- `Script Skeleton`: Full named/admin implementations follow `usage -> parse_args -> check_prerequisites/plan -> main` on `_common.sh`; agent-specific skill leaves follow their owning skill contract and compose public root scripts when shared execution is required.
...
- `Validation Routing`: Owning project skills declare the narrowest route from changed scope to existing script entrypoints, including intentional exclusions and escalation triggers; agents run that route rather than defaulting to full-workspace, full-client, build, benchmark, network, or release gates.
