...
`Framework Mechanism vs Instance Policy`: DEOS provides reusable economic mechanisms and safety contracts, not a mandatory moral/business configuration for every fork. Framework-owned surfaces include primitives, invariants, execution safety, bounded read models, configuration seams, validation gates, and reference patterns. Instance-owned surfaces include brand, launch policy, dApps, founder economics, labor culture, invoice norms, concrete bucket percentages/names, marketing, and demand strategy. Builder invoices, bucketed capital flows, governance protection, protocol-owned liquidity, and AAA execution are mechanisms; a downstream instance may keep, rename, reparameterize, disable, or replace policy choices as long as it preserves the contracts it claims to implement. Repeated treasury/vault/drip/buyback/zap flows SHOULD be lifted into bounded configurable AAA execution plans rather than multiplying bespoke pallets.
...
- `/simulator/`: Historical economic hypothesis lab for TMCTOL and still-authoritative mathematical reference capsule. It proves formulas, tokenomic thresholds, conservation, floor/compression scenarios, and parameter behavior before or alongside runtime implementation. It is not a shadow runtime or parity target for pallets, AAA, governance, XCM, client flows, storage, or weight semantics. Consult it when changing tokenomics, formulas, thresholds, or invariants; do not treat it as the default entrypoint for routine runtime, frontend, or tooling work.
...
- `Block-Cadence Flexibility`: DEOS should be positioned as a flexible runtime-feature framework on top of standard Substrate / Polkadot SDK foundations, not as a framework fixed to one block speed. Future V3 scheduling / block-bundling work is interesting when it helps downstream forks choose a cadence range from conventional ~6s blocks toward sub-second / ~500ms profiles, but cadence support must remain explicit, benchmarked, and configuration-driven rather than silently hardcoded into protocol assumptions.
...
- `Simulation (Mathematical Truth)`: `/simulator`, JavaScript/BigInt, `PPB`; verifies TMCTOL formulas, tokenomic hypotheses, thresholds, conservation, and scenario behavior before or alongside runtime logic changes. It is not a shadow runtime for pallet/storage/weight/AAA/governance/XCM/client parity.
...
- `SDK Version Tracking`: `substrate-wasm-builder` version must match the polkadot-sdk umbrella (`2606.0.0 -> 34.0.0` on the current line). Current operator binary tag is `polkadot-stable2606` (node v1.24.0). `system_version` in `RuntimeVersion` must remain `3` or higher to activate the staged `:pending_code -> :code` runtime-upgrade path from RFC-123.
...
- `Economic-Claim Falsifiability`: Architecture documents MUST NOT state absolute economic guarantees (`eliminates`, `prevents`, `guarantees`, `protects against`, `resists`) beyond shipped runtime behavior. Every load-bearing economic claim MUST carry `(a)` a code anchor to the symbol that establishes it (fn, storage, invariant test, constant) and `(b)` a falsification test that would actually fail if the claim were violated. Architecture docs MUST distinguish `shipped mitigation` (what the runtime enforces, e.g. bounded EMA deviation on direct routes) from `claimed guarantee` (e.g. `eliminates flash-loan attacks`); the latter requires the former plus evidence. A claim whose test passes on any implementation is tautology and counts as rot, not proof. This rule exists because lexical-only gates (terminology, cross-reference, stale-alias) catch vocabulary drift but cannot detect a doc asserting a guarantee the code no longer upholds.
...
- `Autonomous Backlog Sync`: Autonomous execution loops MUST stay anchored to `BACKLOG.md`. If implementation discovers a new in-scope slice that is not represented there, add or retarget a concrete BACKLOG entry in the same pass, and close/remove stale entries immediately once their exit criteria land.
...
- `DAP-Inspired Budget Pools`: Upstream DAP/non-minting reward-pot concepts are useful as design inspiration for DEOS framework evolution even when the upstream pallets are not adopted directly. Future reward or treasury work SHOULD consider explicit externally funded budget pots, budget recipients, epoch snapshots, and unclaimed-reward handling as reusable framework mechanisms compatible with AAA/staking/governance, while avoiding a direct DAP dependency unless it clearly improves forkability and bounded execution.
...
4.  `Repo-Local Completion Gate`:
    - When a pass changes repository state and you are operating autonomously, run `./.agents/skills/alignment/scripts/completion-gate.sh` after local validation and knowledge sync.
