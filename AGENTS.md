...
`Product Boundary`: This repository is the framework layer, not the finished ecosystem product. Product value is expected to crystallize downstream when a fork defines its own user-facing philosophy, launches concrete dApps, and turns the shared protocol substrate into a live ecosystem.
...
- `Artifact-Name Tail Policy`: Low-level artifact identifiers such as `tmctol-runtime`, descriptor/module/file names, generated namespaces, and similar technical handles MAY remain as conscious legacy implementation names until a separate migration epic explicitly targets them. Do not infer top-level framework identity from those identifiers, and do not rename them opportunistically in otherwise unrelated passes.
...
- `Unified Native Staking`: `pallet-staking` remains the single staking pallet. Non-native assets stay generic and economic-only. Native `$NTVE` is a special nomination-aware path: direct passive native staking is disallowed, users must choose a trusted collator before `stNTVE` is minted, and effective operator backing must follow the account's live `stNTVE` position so transfers or unstakes cannot leave stale votes behind. The default target is read-derived backing from live `stNTVE` balances rather than eager cached counters until benchmark evidence proves otherwise. `stNTVE` stays the only native staking receipt; no separate `nomXXX` token tier or standalone `pallet-delegation` is introduced unless later evidence proves it necessary. Higher-level automation SHOULD mirror this distinction explicitly: keep generic stake tasks non-native and use a dedicated operator-aware native surface (for example `StakeNative { amount, operator }`) instead of bolting optional operator context onto the generic stake task.
...
- `Generated Wiki Locale Contract`: Human-facing files under `/wiki` SHOULD use explicit locale suffixes (`index.<locale>.md`, `log.<locale>.md`, and taxonomy directories directly under `/wiki` such as `overview/<slug>.<locale>.md` or `concepts/<slug>.<locale>.md`) with `en` as the default compatibility locale on the current line. Avoid an extra `/wiki/pages/` wrapper unless a future renderer contract proves it necessary. Non-default locale pages SHOULD mirror the same page ids and relative path topology as the default locale, while `/wiki/_meta/*.json` SHOULD stay shared and express localization through language-keyed field objects plus an explicit locale map such as `_meta/locales.json` when frontend/agent consumers need stable discovery.
- `Trusted Wiki Render Contract`: When the browser renders repo-local wiki markdown directly, treat `/wiki/**/*.md` as trusted reviewed repository content rather than as user input. The safety boundary MUST then move to repository validation: reject raw HTML tag blocks, dangerous URL schemes, and inline DOM event-handler attributes pre-merge (currently via `./.agents/skills/wiki-sync/scripts/validate-wiki-trust.sh`) instead of pretending runtime sanitization is the canonical defense.
...
- `Subsystem Doc Versioning`: Some subsystem documents carry their own `Version` field. When such a document is materially updated, its embedded version MUST be aligned to the current monorepo release line rather than to a stale value copied into `AGENTS.md`. If that subsystem document is not touched in a delivery pass, its embedded version MUST remain unchanged. This intentional drift is a signal of document staleness relative to the monorepo.
...
- `Frontend Slice Honesty`: In `/web-client/src/lib`, do not keep heterogeneous stateful slices hidden under a generic umbrella once their roles diverge. Promote them to first-class top-level modules when that makes shell, layout, domain state, and execution-feedback boundaries more truthful.
...
- `Sparse Reward Snapshot Line`: The current staking reward-weight path SHOULD remain one-epoch-lagged and touch-driven. Keep the active per-account snapshot sparse, aggregate same-block reward touches/inflows in memory before touching staking storage, ingest touches from staking-owned balance changes plus runtime-visible `stXXX` transfer/governance events, fix the epoch denominator when reward inflow is recorded, and emit an explicit truncation signal if the bounded event scan cap is exceeded. The bounded ingress scan SHOULD run from a weight-accounted `on_idle` path rather than a hidden `on_finalize` side effect. Truncated epochs SHOULD be treated as ineligible for reward settlement rather than allowing claims against incomplete ingress data. On already-live chains with pre-existing untouched holders, require an explicit bootstrap or warm-up step before enabling reward ingress; the bootstrap path may materialize the live snapshot only before that epoch's denominator is frozen. Reward settlement SHOULD follow the same-asset auto-compound direction already landed as `claim_reward(asset_id, epoch)` and bounded `claim_reward_batch(asset_id, epochs)` rather than introducing a separate liquid reward leg.
- `Reserve-Aware Zap Slippage`: Builders for the current TMCTOL standard on DEOS SHOULD derive Zap Manager foreign→native swap tolerance from current native reserve depth and clamp it between explicit lower/upper bounds. Avoid reusing the generic System AAA 5% swap ceiling as a flat Zap policy once pools become deep enough that tighter bounds materially reduce MEV exposure.
