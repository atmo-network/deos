...
- `Adjacent Resize Isolation`: Render consecutive same-axis tile splits as one ordered resize group; dragging or resetting a handle changes only the two neighboring rows or columns, preserves every non-adjacent pane's rendered size, enforces recursive minima, and maps the resulting group weights back into the persisted binary tree without exposing tree nesting as asymmetric UI behavior.
- `Intrinsic Widgets`: Treat widget responsiveness as width-and-height information design rather than breakpoint rearrangement alone; define useful intermediate and minimum viable phases that remove secondary detail under pressure, preserve the primary task and essential failure/safety state, and route any remaining horizontal or vertical overflow through `PaneWidgetHost` rather than widget-local scroll containers.
- `Intrinsic Widget Scale`: Let `PaneWidgetHost` provide every widget with tight/regular/open root units in a bounded 14px/15px/16px range and scoped Tailwind spacing/typography variables; keep pane chrome, borders, scrollbars, touch minima, D3 geometry, container thresholds, and the minimum scrollport in physical units so visual scaling cannot destabilize workspace mechanics.
- `Widget Interior Freedom`: Preserve the tile manager, domain contracts, safety, and provenance boundaries, but freely restructure a widget's internal information architecture, navigation, and visual composition when the legacy layout obstructs a clearer intrinsic task flow.
- `Layout Before Reskin`: During intrinsic maturation, prioritize widget information architecture, task order, sizing, spacing, grouping, disclosure, and width/height composition; refine control sizing or contrast when usability requires it, but defer broad palette, effects, and visual-theme redesign until the layout system reaches a coherent baseline.
- `Backbone vs Skin`: The framework reference client owns the responsive/intrinsic UI backbone—workspace topology, widget task hierarchy, geometry, minimum phases, accessibility, safety, provenance, and interaction logic—while downstream forks may apply branded colors, gradients, effects, typography, and visual character without rewriting that skeleton or weakening its states.
- `Active State Opacity`: Render active controls and available state with solid colors; reserve reduced opacity or translucent treatment for disabled, inactive, hidden, loading, or transient drag/overlay states where attenuation communicates meaning.
- `Tailwind-First Styling`: Express meaningful presentation and responsive layout through Tailwind atomic/arbitrary utilities when readable; use the Tailwind v4 trailing important modifier (`bg-black!`, including after variants) rather than legacy leading `!`; retain component CSS for D3 geometry, trusted Markdown descendants, dynamic runtime values, or semantic container rules that become less clear when encoded as dense class strings.
