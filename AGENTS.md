...
- `SDK Version Tracking`: `substrate-wasm-builder` version must match the polkadot-sdk umbrella (`2603.0.0 -> 32.0.0` on the current line). Current operator binary tag is `polkadot-stable2603-2` (node v1.22.2); the 2603 patch release is represented by targeted `Cargo.lock` patch updates rather than a new umbrella crate version. `system_version` in `RuntimeVersion` must be `3` or higher on SDK 2603 to activate the staged `:pending_code -> :code` runtime-upgrade path from RFC-123.
