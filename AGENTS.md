...
- `Actor Close and Custody`: Minimal apoptosis and every other close delete process semantics, release lifecycle metadata/slot/state resources, and perform no economic unwind or custody transfer. Deterministic sovereign custody survives close and exact-slot recreation; exit/recovery uses ordinary authored Steps, and `Transfer(Percent(100%))` receives no lifecycle source-exhaustion privilege.
