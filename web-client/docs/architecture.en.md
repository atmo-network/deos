...
The automation widget reads known System actors from `AAA.ActorIdentities`, `AAA.ActorHot`, `AAA.ActorProgram`, and sparse `AAA.ContinuationState` at one finalized block. Current cursor, logical-run attempt, unsuccessful-attempt count at that cursor, and the per-actor `cache_epoch` stamp are canonical-chain truth. Historical attempt timelines remain materialized. The automation authoring, analysis, and local simulation contracts derive `MaxExecutionPlanSteps` and `MaxRetryAttempts` from the generated AAA ABI manifest; RetryLater remains Mutable-only and does not fabricate adapter retryability.

The adapter exposes the public cache-revalidation progress surface (`AAA.CurrentCacheEpoch` and the optional `AAA.CacheRevalidation { target_epoch, cursor, remaining }`) as canonical-chain truth: while a gate exists no FIFO attempt runs, and the actor rows surface the stamp so the UI never presents a stale-cache actor as executable.

The widget also consumes the read-only `AaaEligibilityApi::aaa_eligibility` projection (`automation/eligibility.ts` plus the `aaa-eligibility` blockchain adapter) for each actor: `ready`, the scheduler-owned `phase`, and `nextEligibleBlock` at the same finalized block. The browser never reimplements cadence phase, cooldown, schedule window, retry backoff, breaker, or latch arithmetic; when the runtime API is unavailable the row shows `Unavailable` instead of guessing. The projection never promises service, because queue position and available Weight decide actual admission.

The automation domain validates metadata-bound plan artifacts. It discovers `ProgramInput`, `ActiveProgramInput`, `AaaType`, and `Mutability` from exact runtime metadata, requires SCALE decode/re-encode equality, derives deterministic `planId`, produces lossless JSON-safe projections, and classifies cross-genesis or cross-metadata diffs as incompatible until explicit rebinding.
