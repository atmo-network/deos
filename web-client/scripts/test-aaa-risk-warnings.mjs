/*
Domain: AAA risk and composition warnings validation
...
import {
  AAA_COMPOSITION_WARNING_KINDS,
  projectAaaCompositionWarnings,
...
function artifactFor({ steps, mutability = 'Mutable' } = {}) {
  const programScale = encodeAaaProgramValue(
    metadataBytes,
    activeProgram(steps),
  );
...
test('Immutable actor without a terminal step produces a critical warning', () => {
  ...
  const warnings = projectAaaCompositionWarnings({ artifact, analysis });
...
test('shared single FIFO projects strict head-of-line warning', () => {
  ...
  const analysis = analyze(artifact);
  const warnings = projectAaaCompositionWarnings({
    artifact,
    analysis,
...
test('all warning kinds are declared in the stable enumeration', () => {
  const artifact = artifactFor({ steps: [step()] });
  const analysis = analyze(artifact);
  projectAaaCompositionWarnings({ artifact, analysis });
  assert.ok(AAA_COMPOSITION_WARNING_KINDS.length >= 6);
  assert.ok(
    AAA_COMPOSITION_WARNING_KINDS.includes('ImmutableWithoutReachableTerminal'),
  );
  assert.ok(
    AAA_COMPOSITION_WARNING_KINDS.includes(
