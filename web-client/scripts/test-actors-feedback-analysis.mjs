...
test('price to swap to price is a structural endogenous self-feedback path', () => {
  const model = analyzeActorFeedback({
    actors: [
      {
        id: 'liquidity',
        analysis: analysis({
...
test('exact pool identity separates equal pairs and reports true shared contention', () => {
  ...
  const distinct = analyzeActorFeedback({
    actors: [
      {
        id: 'pool-a-actor',
  ...
  const shared = analyzeActorFeedback({
    actors: [
      {
        id: 'writer-a',
        analysis: swapAnalysis(),
...
test('timing and policy evidence rejects provenance and identity substitution', () => {
  ...
  assert.doesNotThrow(() => analyzeActorFeedback(input));
  ...
  const mismatchModel = analyzeActorFeedback({
    ...input,
...
test('feedback projection is deterministic and fails closed at graph bounds', () => {
  ...
  assert.deepEqual(analyzeActorFeedback(input), analyzeActorFeedback(input));
  assert.throws(
    () => analyzeActorFeedback({ ...input, limits: { maxNodes: 1 } }),
    /node limit exceeded/,
  );
  assert.throws(
    () => analyzeActorFeedback({ ...input, limits: { maxEdges: 1 } }),
    /edge limit exceeded/,
  );
  assert.throws(
    () =>
