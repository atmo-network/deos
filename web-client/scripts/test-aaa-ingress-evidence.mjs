...
async function run() {
  ...
  assert.equal(boundary.typedTrait, 'pallet_aaa::AddressEventIngress');
  ...
  assert.match(
    ingressAdapter,
    /impl\s+pallet_aaa::AddressEventIngress<[^>]+> for RuntimeAddressEventIngress/,
  );
  assert.match(ingressAdapter, /crate::AAA::notify_ingress\(/);
  assert.match(ingressAdapter, /crate::AAA::preflight_ingress\(/);
