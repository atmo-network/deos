/*
Domain: AAA reactive scenario-corpus validation
...
function artifact(program) {
  return createAaaArtifactFromAuthoring({ program, metadataBytes, runtime });
}
...
test('descending buys and ascending sells lower as independent bounded one-shot actors', () => {
  ...
  for (const scenario of [...descendingBuyBuckets, ...ascendingSellBuckets]) {
    const validation = validateAaaAuthoringProgram(scenario.program);
...
test('partial reaction cores lower without inventing reserve-ratio or depth predicates', () => {
  assert.equal(
    AAA_AUTHORING_CONDITION_TYPES.includes('TreasuryReserveRatio'),
    false,
  );
  assert.equal(AAA_AUTHORING_CONDITION_TYPES.includes('LiquidityDepth'), false);
  for (const scenario of partialScenarios) {
    ...
    assert.equal(
      validateAaaAuthoringProgram(scenario.program).valid,
      true,
      scenario.name,
    );
...
test('non-price scalar strategy uses runtime block truth rather than a mislabeled price feed', () => {
  ...
  assert.equal(validateAaaAuthoringProgram(nonPriceScalar.program).valid, true);
  const analysis = analyzeAaaProgram({
...
test('every scenario declares availability, data ownership, provenance, and failure behavior', () => {
  ...
  for (const scenario of scenarios) {
    ...
    assert.equal(scenario.data.reactionOwner, 'pallet-aaa', scenario.name);
    ...
    assert.equal(validateAaaAuthoringProgram(scenario.program).valid, true);
  }
});
