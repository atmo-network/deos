<script lang="ts">
  ...
  function toPoints(input: number[], chartWidth: number, chartHeight: number): string {
    if (input.length === 0) {
      return "";
    }
    ...
    return input
      .map((value, index) => {
        const x = input.length === 1 ? chartWidth / 2 : (index / (input.length - 1)) * chartWidth;
