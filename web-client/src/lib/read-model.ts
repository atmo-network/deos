/*
Domain: Read-model provenance contract
Owns: Canonical-chain vs materialized provenance types, wrappers, labels, and source descriptions.
Excludes: Concrete providers, network fetching, cache storage, and widget layout.
Zone: Foundation contract; may be imported by domains, stores, adapters, and UI Kit badges.
*/
...
export function fromClientBoundedProjection<T>(
  ...
  sourceRef: string,
  scope: Extract<
    CanonicalDerivedProvenance['scope'],
    'live' | 'snapshot' | 'bounded-recent' | 'session'
  > = 'live',
  stampInfo?: ReadModelStamp,
): ReadModelValue<T> {
