<script lang="ts">
  ...
  import { isValidTmctolAddress } from "$lib/adapters/blockchain/signer";
  import { logStore } from "$lib/log/index.svelte";
  import { marketStore } from "$lib/market/index.svelte";
  import { portfolioStore, type TransferAssetKey } from "$lib/portfolio/index.svelte";
  import { systemStore } from "$lib/system/index.svelte";
  import { walletStore } from "$lib/wallet/index.svelte";
  import { fmt, fmtInputAmount, toBigInt, toFloat } from "$lib/shared/format";
  import { Badge, Button, Card, Notice, ReadModelBadge, SectionCard, SelectableTile, StatCard, TextField } from "$lib/shared/ui";
  ...
  const signerBoundaryNotice = $derived.by(() => {
    if (signerStatus === "readonly") {
      return {
        variant: "muted" as const,
        message: "This address is valid but watch-only. Transfers require a signer for the same address from either an injected wallet or a built-in local dev preset",
      };
    }
    if (selectedSource === "dev") {
      return {
        variant: "muted" as const,
        message: "This wallet is using an in-browser local dev signer. Treat it as a local Zombienet/testing convenience, not a general production wallet path",
      };
    }
    if (selectedSource === "injected") {
      return {
        variant: "muted" as const,
        message: "This wallet is using an injected browser signer. Transfers and other live writes will prompt through the connected extension for this same address",
      };
    }
    if (signerStatus !== "available") {
      return {
        variant: "muted" as const,
        message: "No signer is currently attached to this selected address. Open the sidebar account surface to connect an injected wallet or choose a local dev preset before sending funds",
      };
    }
    return {
      variant: "muted" as const,
      message: "A signer is available for this account through the current wallet connection path",
    };
  });
  const knownAssets = $derived.by(() => portfolioStore.knownAssets);
  const knownAssetsProvenance = $derived(portfolioStore.knownAssetsView.provenance);
  const transferAssets = $derived.by(() => portfolioStore.transferAssets);
  const selectedTransferAsset = $derived.by(() => portfolioStore.findAsset(sendAsset));
  ...
  const spendableSendBalance = $derived(nativeTransfer ? safeSendBalance : sendBalance);
  ...
  const sendGuidance = $derived.by(() => {
    ...
    if (sendPendingDraftKey !== null) {
      return {
        variant: "muted" as const,
        message: "A transfer from the previous draft is still in progress. Wait for it to finish before submitting a new draft",
      };
    }
    if (signerStatus === "readonly") {
      return {
        variant: "muted" as const,
        message: "Selected account is watch-only. Open the sidebar and connect an injected signer for the same address before sending funds",
      };
    }
    if (signerStatus !== "available") {
      return {
        variant: "muted" as const,
        message: walletStore.state.signerMessage === "No injected wallet extension detected"
    ...
    if (recipient.length > 0 && !isValidTmctolAddress(recipient)) {
      return {
        ...
        message: "Recipient must be a valid DEOS ss58 address before the transfer can be signed",
      };
    }
    ...
    if (parsedSendAmount.amount != null && parsedSendAmount.amount > sendBalance) {
      return {
        variant: "warn" as const,
        message: `Selected amount exceeds the available ${selectedTransferAsset.symbol} balance`,
      };
    }
    if (nativeTransfer && parsedSendAmount.amount != null && parsedSendAmount.amount > safeSendBalance) {
      return {
        variant: "warn" as const,
  ...
  async function submitTransfer(): Promise<void> {
    ...
    sendErrorDraftKey = null;
    try {
      if (parsedSendAmount.amount == null || parsedSendAmount.rawAmount == null) {
      ...
      if (submittedAmount > sendBalance) {
        throw new Error(`Selected amount exceeds the available ${submittedSymbol} balance`);
      }
      if (nativeTransfer && submittedAmount > safeSendBalance) {
        throw new Error(`Selected amount exceeds the spendable ${submittedSymbol} balance after fee headroom`);
      }
      await portfolioStore.transferAsset(submittedAsset, submittedRecipient, submittedAmount);
      logStore.add(
...
<Card class="min-h-full flex flex-col">
  <div bind:this={rootEl} class="h-full flex flex-col min-h-0">
    <div class="@container grid gap-3 p-3 text-xs">
      <SectionCard title="Selected account" subtitle="Chosen in the sidebar account widget">
        {#snippet actions()}
          <button
            onclick={copyAddress}
            class="inline-flex items-center gap-1 rounded-lg border border-(--mono-border) bg-(--mono-bg) px-2 py-1 text-[10px] text-(--mono-muted) transition-colors hover:border-(--mono-purple) hover:text-(--mono-text)"
          >
            {#if copied}
              <Check size={12} />
              Copied
        ...
        <div class={[
          "grid gap-2",
          densePane ? "grid-cols-1" : "@md:grid-cols-3",
        ]}>
          <StatCard label="Account" value={selectedLabel} />
          <StatCard label="Mode" value={accountModeLabel} detail={signerStatus === "available" ? "signing ready" : signerStatus === "readonly" ? "valid address, no signer" : "signer unavailable"} />
          <StatCard
            label="Receive address"
            value={densePane ? (selectedAddress ? shortAddress(selectedAddress) : "—") : (selectedAddress || "—")}
            detail={densePane && selectedAddress ? selectedAddress : undefined}
            class="break-all"
          />
        </div>
        <Notice variant={signerBoundaryNotice.variant}>{signerBoundaryNotice.message}</Notice>
      </SectionCard>

      <SectionCard title="Known client assets" subtitle="Bounded current-session wallet asset surface">
        {#snippet actions()}
        ...
        <div class={[
          ...
          {#each knownAssets as asset}
            <StatCard
              label={asset.assetId !== null ? `${asset.kind} #${asset.assetId}` : "Native asset"}
      ...
      <SectionCard title="Send assets" subtitle="Receipts appear in Log">
        ...
        <div class={[
          ...
          <div class="grid gap-2">
            <div class={[
              compactPane
                ? "grid grid-flow-col auto-cols-[minmax(7.5rem,1fr)] gap-2 overflow-x-auto pb-1 [scrollbar-width:none]"
                : "grid gap-2",
            ]}>
              {#each transferAssets as asset}
                <SelectableTile
                  onclick={() => asset.transferKey && (sendAsset = asset.transferKey)}
                  selected={sendAsset === asset.transferKey}
                >
                  <div class="font-medium text-(--mono-text)">{asset.symbol}</div>
                  <div class="text-[10px] text-(--mono-muted)">{asset.kind}</div>
                </SelectableTile>
              {/each}
            </div>
            <StatCard
              ...
              detail={selectedTransferAsset.assetId !== null ? `${selectedTransferAsset.kind} #${selectedTransferAsset.assetId}` : selectedTransferAsset.kind}
            />
            <StatCard
              label={nativeTransfer ? "Spendable now" : "Available"}
              value={fmt(toFloat(nativeTransfer ? safeSendBalance : sendBalance))}
              detail={nativeTransfer ? `Leaves ${fmt(toFloat(sendBalance - safeSendBalance))} ${selectedTransferAsset.symbol} for fees` : undefined}
            />
          </div>

          <div class="grid gap-3">
            <TextField label="Recipient" bind:value={sendRecipient} placeholder="5..." />
            <TextField
              label="Amount"
            ...
            <div class="flex justify-end">
              <button
                ...
                class={fillMaxAvailable
                  ? "text-[10px] text-(--mono-purple) hover:underline"
                  : "text-[10px] text-(--mono-muted) opacity-60"}
            ...
            {:else if sendGuidance}
              <Notice variant={sendGuidance.variant}>{sendGuidance.message}</Notice>
            {/if}
            <Button variant="primary" class={sendButtonState.disabled ? "w-full opacity-50" : "w-full"} onclick={submitTransfer} disabled={sendButtonState.disabled}>
